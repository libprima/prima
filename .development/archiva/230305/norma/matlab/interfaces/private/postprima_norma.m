...
if ~options.classical && ~probinfo.infeasible && ~probinfo.nofreex
    ...
    if strcmp(solver, 'cobyla_norma') && (any(chist > hugecon) || any(isnan(chist)))
        % Public/unexpected error
        error(sprintf('%s:InvalidChist', invoker), ...
...
if options.debug && ~options.classical
    ...
    minf = min([fhistf, fx]);
    if (fx ~= minf) && ~(isnan(fx) && isnan(minf)) && ~(strcmp(solver, 'lincoa_norma') && constr_modified) && ~strcmpi(options.precision, 'quadruple')
        % Public/unexpected error
        error(sprintf('%s:InvalidFhist', invoker), ...
    ...
    if strcmp(options.precision, 'double') && ((strcmp(solver, 'lincoa_norma') && ~constr_modified) || strcmp(solver, 'cobyla_norma'))
        ...
        if ~(isnan(cstrv) && isnan(constrviolation)) && ~(cstrv == inf && constrviolation == inf) && ~(abs(constrviolation-cstrv) <= lincoa_norma_prec*max(1,abs(cstrv)) && strcmp(solver, 'lincoa_norma')) && ~(abs(constrviolation-cstrv) <= cobyla_norma_prec*max(1,abs(cstrv)) && strcmp(solver, 'cobyla_norma'))
            % Public/unexpected error
            error(sprintf('%s:InvalidChist', invoker), ...
    ...
    if options.chkfunval % Check the values of fun(x) and con(x)
        ...
        if ~isempty(fhist) && ~isempty(xhist)
            ...
            if any(~(isnan(fhist) & isnan(fhistx)) & ~((fhist == fhistx) | (abs(fhistx-fhist) <= cobyla_norma_prec*max(1, abs(fhist)) & strcmp(solver, 'cobyla_norma'))))
                % Public/unexpected error
                error(sprintf('%s:InvalidFx', invoker), ...
        ...
        if ~isempty(xhist) && (isfield(output, 'nlcihist') || isfield(output, 'nlcehist') || isfield(output, 'chist'))
            ...
            if (strcmp(solver, 'lincoa_norma') && ~constr_modified) || strcmp(solver, 'cobyla_norma')
                ...
                if any(~(isnan(chist) & isnan(chistx)) & ~((chist == chistx) | abs(chistx-chist) <= lincoa_norma_prec*max(1, abs(chist)) & strcmp(solver, 'lincoa_norma') | (abs(chistx-chist) <= cobyla_norma_prec*max(1, abs(chist)) & strcmp(solver, 'cobyla_norma'))))
                    % Public/unexpected error
                    error(sprintf('%s:InvalidFx', invoker), ...
