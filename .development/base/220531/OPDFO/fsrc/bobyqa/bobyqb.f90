...
!
! Last Modified: Tuesday, June 21, 2022 AM09:37:50
!--------------------------------------------------------------------------------------------------!
...
if (ntrits == 0) then
    ...
    if (.not. (denom > HALF * vlag(knew)**2)) then
        !if (.not. (denom > vlag(knew)**2)) then  ! For verifying RESCUE
...
! calculating the next value of the objective function.
    ...
    if (.not. (knew > 0 .and. scaden > HALF * biglsq)) then
        !if (.not. (knew > 0 .and. scaden > biglsq)) then  ! For verifying RESCUE
        if (nf > nresc) then
            goto 190
        else
            info = DAMAGING_ROUNDING
