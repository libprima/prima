subroutine update(n, npt, xpt, bmat, zmat, idz, ndim, sp, step, kopt, knew, vlag, w, freduced)

implicit real(kind(0.0D0)) (a - h, o - z)
implicit integer(i - n)
dimension xpt(npt, *), bmat(ndim, *), zmat(npt, *), sp(*), step(*), vlag(*), w(*)
logical, intent(in), optional :: freduced
...
if (knew == 0) then
    ...
    do k = 1, npt
        ...
        do j = 1, n
            if (.false.) then
                !if (freduced) then
                distsq = distsq + (xpt(k, j) - (xpt(kopt, j) + step(j)))**2
            else
