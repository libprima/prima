function [x, fx, exitflag, output] = cobyla_last(varargin)
...
%
%   x = cobyla_last(fun, x0, Aineq, bineq, Aeq, beq, lb, ub, nonlcon)
%
...
%
%   [x, fx, exitflag, output] = cobyla_last(INPUTS)
%
%   See "3. Outputs" below for explanations on these outputs.
%
%   2. Flexible syntax
%
%   x = cobyla_last(fun, x0) solves
%       minimize fun(x)
%   x = cobyla_last(fun, x0, Aineq, bineq) solves
%       minimize fun(x) s.t. Aineq * x <= bineq
%   x = cobyla_last(fun, x0, Aineq, bineq, Aeq, beq) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq
%   x = cobyla_last(fun, x0, Aineq, bineq, Aeq, beq, lb) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq, lb <= x
%   x = cobyla_last(fun, x0, Aineq, bineq, Aeq, beq, lb, ub) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq, lb <= x <= ub
%   x = cobyla_last(fun, x0, nonlcon) solves
%       minimize fun(x) s.t. cineq(x) <= 0, ceq(x) = 0
%   x = cobyla_last(fun, x0, Aineq, bineq, nonlcon) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, cineq(x) <= 0, ceq(x) = 0
%   x = cobyla_last(fun, x0, Aineq, bineq, Aeq, beq, nonlcon) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq, cineq(x) <= 0, ceq(x) = 0
%   x = cobyla_last(fun, x0, Aineq, bineq, Aeq, beq, lb, nonlcon) solves
...
%       solver: backend solver that does the computation, i.e., 'cobyla_last'
...
%   options = struct();
%   options.maxfun = 50;
%   x = cobyla_last(@cos, -1, 2, 3, options);
%
%   solves
%       min cos(x) s.t. 2 * x <= 3
...
internal_invokers = {'prima_last'}; % Invokers from this package; may have others in the future
...
% PROBINFO in prima_last; that said, in such a scenario, the PROBINFO of this solver
...
elseif ~strcmp(invoker, 'prima_last') && probinfo.feasibility_problem && ~strcmp(probinfo.refined_type, 'nonlinearly-constrained')
    ...
    funcon = @(x) cobyla_last_funcon(x, fun, Aineq, bineq, Aeq, beq, lb, ub, nonlcon);
...
%%%%%%%%%%%%%%%%%%%%%%%%% Auxiliary functions %%%%%%%%%%%%%%%%%%%%%
function [f, constr, m_nlcineq, m_nlceq] = cobyla_last_funcon(x, fun, Aineq, bineq, Aeq, beq, lb, ub, nonlcon)
f = fun(x);
[constr, m_nlcineq, m_nlceq] = cobyla_last_con(x, Aineq, bineq, Aeq, beq, lb, ub, nonlcon);
return

function [constr, m_nlcineq, m_nlceq] = cobyla_last_con(x, Aineq, bineq, Aeq, beq, lb, ub, nonlcon)
