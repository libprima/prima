...
%PREPRIMA preprocesses the input to prima_norma.
%
%   ***********************************************************************
...
if ~validated % options.rhobeg has not got a valid value yet
    ...
    if isfield(options, 'rhoend') && isrealscalar(options.rhoend) && isfinite(options.rhoend) && options.rhoend >= 0 && ~strcmpi(solver, 'bobyqa_norma')
...
if strcmp(solver, 'bobyqa_norma') && ~probinfo.nofreex && options.rhobeg > min(probinfo.refined_data.ub-probinfo.refined_data.lb)/2
...
solver_list = {'bobyqa_norma'}; % Only BOBYQA needs pre_rhobeg_x0 as of 20240121. May have others in the future.
