function [x, fx, exitflag, output] = uobyqa_last(varargin)
...
%
%   [x, fx, exitflag, output] = uobyqa_last(INPUTS)
%
...
%       solver: backend solver that does the computation, i.e., 'uobyqa_last'
...
%   x = uobyqa_last(problem);
%
%   solves
%       min cos(x)
%   starting from x0 = -1 with at most 50 function evaluations.
%
%   See also prima_last, NEWUOA, BOBYQA, LINCOA, COBYLA.
%
...
elseif (nvararg >= 2 && nvararg <= maxarg)
    ...
    args = [varargin(1:2), cell(1, 7), varargin(end)]; % args{:} (should have 10 entries) will be the inputs for preprima_last
else
    if ismember(invoker, internal_invokers) % Private/unexpected error
