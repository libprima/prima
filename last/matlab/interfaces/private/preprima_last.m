function [fun, x0, Aineq, bineq, Aeq, beq, lb, ub, nonlcon, options, probinfo] = preprima_last(fun, x0, Aineq, bineq, Aeq, beq, lb, ub, nonlcon, options)
%PREPRIMA preprocesses the input to prima_last and its solvers.
%
%   ***********************************************************************
...
invoker_list = [all_solvers(), 'prima_last'];
callstack = dbstack;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% If invoker is a solver called by prima_last, then preprima_last should have been called in prima_last.
if (length(callstack) >= 3) && strcmp(callstack(3).name, 'prima_last')
    if nargin ~= 10 % There should be 10 input arguments
        ...
        '%s: UNEXPECTED ERROR: %d inputs received; this should not happen as preprima_last has been called once in prima_last.', funname, nargin);
    end
    % In this case, we set probinfo to empty.
    probinfo = [];
    return % Return because preprima_last has already been called in prima_last.
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% The raw data can be useful when debugging. At the end of preprima_last, if
...
if ~any([probinfo.infeasible_lineq; probinfo.infeasible_leq; probinfo.infeasible_bound])
    ...
    rbounds(fixedx_value == fixedx_value_save) = 0;
    probinfo.constrv_x0 = max([probinfo.constrv_x0; abs(rbounds)], [], 'includenan');
...
if ~prob_solv_match(probinfo.refined_type, invoker)
    if strcmp(invoker, 'prima_last') || (nargin ~= 1)
        % Private/unexpected error
...
% and probinfo.refined_data.ub will be used for defining rhobeg if bobyqa_last is selected.
probinfo.refined_data = struct('objective', fun, 'x0', x0, 'Aineq', Aineq, 'bineq', bineq, ...
    'Aeq', Aeq, 'beq', beq, 'lb', lb, 'ub', ub, 'nonlcon', nonlcon);

% Select a solver if invoker = 'prima_last'; record the solver in options.solver.
...
if ~options.debug % Do not carry the raw data with us unless in debug mode.
    ...
    % Set this field to empty instead of remove it, because postprima_last
...
% The decoded problem will be sent to the preprima_last function for validation.
...
% 1. When invoker is in {uobyqa_last, ..., cobyla_last}, we will not complain that
%    a solver is specified unless invoker ~= solver. See function pre_options.
% 2. When invoker is in {uobyqa_last, ..., cobyla_last}, if the problem turns out
%    unsolvable for the invoker, then we will raise an error in preprima_last.
...
% Possible invokers
invoker_list = [solver_list, 'prima_last'];
...
maxfun = 500*lenx0;
rhobeg = 1; % The default rhobeg and rhoend will be revised if solver = 'bobyqa_last'
rhoend = 1e-6;
ftarget = -inf;
...
% We do the following:
% 1. If invoker = 'prima_last':
% 1.1 If no solver is specified or solver = 'prima_last', we do not complain
...
% solver_list or ''. The second case is possible only if invoker = 'prima_last',
...
if strcmp(invoker, 'prima_last')
    ...
    if isfield(options, 'solver')
        ...
        elseif ~strcmpi(options.solver, 'prima_last')
        % We should not complain about 'unknown solver' if invoker = options.solver = 'prima_last'
...
else % invoker is in {'uobyqa_last', ..., 'cobyla_last'}
...
options.solver = char(solver); % Record solver in options.solver; will be used in postprima_last
% When the invoker is prima_last, options.solver = solver = '' unless the user defines
...
if isempty(solver) || any(strcmpi(solver, {'newuoa_last', 'bobyqa_last', 'lincoa_last'}))
    known_fields = [known_fields, 'npt'];
end
if isempty(solver) || any(strcmpi(solver, {'bobyqa_last', 'lincoa_last', 'cobyla_last'}))
    known_fields = [known_fields, 'scale'];
end
if isempty(solver) || strcmpi(solver, 'bobyqa_last')
    known_fields = [known_fields, 'honour_x0'];
end
if isempty(solver) || any(strcmpi(solver, {'lincoa_last', 'cobyla_last'}))
    known_fields = [known_fields, {'ctol', 'cweight', 'maxfilt'}];
end
if isempty(solver) || strcmpi(solver, 'cobyla_last')
...
% If solver = '' (empty char array), then invoker must be prima_last, and a solver
...
if isempty(solver)
    ...
    switch lower(solver)
    case {'newuoa_last', 'bobyqa_last', 'lincoa_last'}
        npt = 2*lenx0 + 1;
    case {'uobyqa_last'}
        npt = (lenx0+1)*(lenx0+2)/2;
    case {'cobyla_last'}
        npt = lenx0+1;
		% uobyqa_last and cobyla_last do not need npt an option, but we need npt to validate/set maxfun
    end
end
...
if strcmpi(solver, 'bobyqa_last') && ~options.scale
...
if isfield(options, 'npt')
    if isempty(solver) && (~isintegerscalar(options.npt) || options.npt < 1 || isnan(options.npt))
        ...
        warning(wid, '%s', wmsg);
        warnings = [warnings, wmsg];
    elseif any(strcmpi(solver, {'newuoa_last', 'bobyqa_last', 'lincoa_last'})) && (~isintegerscalar(options.npt) || isnan(options.npt) || options.npt < lenx0+2 || options.npt > (lenx0+1)*(lenx0+2)/2)
        % newuoa_last, bobyqa_last and lincoa_last requires n+2 <= npt <= (n+1)*)(n+2)/2;
        % uobyqa_last and cobyla_last do not use npt.
        wid = sprintf('%s:InvalidNpt', invoker);
...
if ~validated  % options.npt has not got a valid value yet
    ...
    % For uobyqa_last and cobyla_last, we also adopt the 'default npt' defined above,
...
if isfield(options, 'maxfun')
    ...
    elseif ~isempty(solver) && options.maxfun <= options.npt  % Here, options.maxfun or options.npt cannot be NaN. No worry about the comparison.
        ...
        case {'newuoa_last', 'lincoa_last', 'bobyqa_last'}
...
if isfield(options, 'rhobeg')
    if ~isrealscalar(options.rhobeg) || options.rhobeg <= 0 || isnan(options.rhobeg) || options.rhobeg == inf
        ...
        warning(wid, '%s', wmsg);
        warnings = [warnings, wmsg];
    elseif strcmpi(solver, 'bobyqa_last')  % Validate options.rhobeg for bobyqa_last
        if options.scale && options.rhobeg > 1  % This case cannot be combined with the next case, as ub and lb are NOT scaled yet in preprima_last
            wid = sprintf('%s:InvalidRhobeg', invoker);
...
if ~validated % options.rhobeg has not got a valid value yet
    % Take into account `rhoend` if it has got a valid value. We do not do this for `bobyqa_last`, which
    % requires that rhobeg <= min(xu-xl)/2.
    if isfield(options, 'rhoend') && isrealscalar(options.rhoend) && options.rhoend >= 0 && options.rhoend < inf && ~strcmpi(solver, 'bobyqa_last')
...
invoker_list = {'prima_last'};
% Only prima_last needs select_solver. We may have other invokers in the future!
solver_list = all_solvers();
% We may add other solvers in the future!
% Note that prima_last is not a possible solver here!
callstack = dbstack;
funname = callstack(1).name; % Name of the current function
...
if ~solver_correct
    ...
    case 'unconstrained'
        if (n >= 2 && n <= 8 && options.maxfun >= (n+1)*(n+2)/2 + 1)
            solver = 'uobyqa_last';
        elseif (options.maxfun <= n+2) % After preprima_last, options.maxfun >= n+2 is ensured. Thus options.maxfun <= n+2 indeed means options.maxfun = n+2
            solver = 'cobyla_last';
        else
            solver = 'newuoa_last';  % options.npt will be set later
            % Interestingly, we note in our test that lincoa_last outperformed
            % newuoa_last on unconstrained CUTEst problems when the dimension
            % was not large (i.e., <= 50) or the precision requirement
            % was not high (i.e., >= 1e-5). Therefore, it is worthwhile to
            % try lincoa_last when an unconstrained problem is given.
            % Nevertheless, for the moment, we set the default solver
            % for unconstrained problems to be newuoa_last.
        end
    case 'bound-constrained'
        if (options.maxfun <= n+2)
            solver = 'cobyla_last';
        else
            solver = 'bobyqa_last';  % options.npt will be set later
        end
    case 'linearly-constrained'
        if (options.maxfun <= n+2)
            solver = 'cobyla_last';
        else
            solver = 'lincoa_last';  % options.npt will be set later
        end
...
if ismember(solver, {'newuoa_last', 'bobyqa_last', 'lincoa_last'}) && (isnan(options.npt) || options.npt < n+2 || options.npt > min((n+1)*(n+2)/2, options.maxfun-1))
...
if strcmp(solver, 'bobyqa_last') && options.rhobeg > min(probinfo.refined_data.ub-probinfo.refined_data.lb)/2
    ...
    if ismember('rhobeg', probinfo.user_options_fields) || ismember('rhoend', probinfo.user_options_fields)
        wid = sprintf('%s:InvalidRhobeg', invoker);
        wmsg = sprintf('%s: rhobeg is set to %g and rhoend to %g according to the selected solver bobyqa_last, which requires rhoend <= rhobeg <= min(ub-lb)/2.', invoker, options.rhobeg, options.rhoend);
        warning(wid, '%s', wmsg);
        warnings = [warnings, wmsg];
    end
end
...
solver_list = [all_solvers(), 'prima_last'];
% Note: prima_last is also a possible solver when prob_solv_match is called in
% preprima_last to check whether the invoker can handle the problem.
...
match = match || strcmp(solver, 'prima_last');  % prima_last matches all types of problems.
