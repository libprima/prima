function [x, fx, exitflag, output] = lincoa_last(varargin)
...
%
%   x = lincoa_last(fun, x0, Aineq, bineq, Aeq, beq, lb, ub)
%
...
%
%   [x, fx, exitflag, output] = lincoa_last(INPUTS)
%
%   See "3. Outputs" below for explanations on these outputs.
%
%   2. Flexible syntax
%
%   x = lincoa_last(fun, x0) solves
%       minimize fun(x)
%   x = lincoa_last(fun, x0, Aineq, bineq) solves
%       minimize fun(x) s.t. Aineq * x <= bineq
%   x = lincoa_last(fun, x0, Aineq, bineq, Aeq, beq) solves
%       minimize fun(x) s.t. Aineq * x <= bineq, Aeq * x = beq
%   x = lincoa_last(fun, x0, Aineq, bineq, Aeq, beq, lb) solves
...
%       solver: backend solver that does the computation, i.e., 'lincoa_last'
...
%   options = struct();
%   options.maxfun = 50;
%   x = lincoa_last(@cos, -1, 2, 3, options);
%
%   solves
%       min cos(x) s.t. 2 * x <= 3
...
elseif (nvararg >= 2 && nvararg <= maxarg)
    ...
    args = [varargin(1:8), {[]}, varargin(end)]; % args{:} (should have 10 entries) will be the inputs for preprima_last
else
    if ismember(invoker, internal_invokers) % Private/unexpected error
...
try % preprima_last is a private function that may generate public errors; error-handling needed
    [fun, x0, Aineq, bineq, Aeq, beq, lb, ub, ~, options, probinfo] = preprima_last(args{:});
catch exception
...
elseif ~strcmp(invoker, 'prima_last') &&  probinfo.feasibility_problem
    ...
    % Note that preprima_last has removed the 'trivial' linear constraints in
    ...
    % preprima_last has tried to make find a feasible x0. Raise a warning is
