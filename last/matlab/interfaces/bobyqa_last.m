function [x, fx, exitflag, output] = bobyqa_last(varargin)
...
%
%   [x, fx, exitflag, output] = bobyqa_last(INPUTS)
%
%   See "3. Outputs" below for explanations on these outputs.
%
%   2. Flexible syntax
%
%   x = bobyqa_last(fun, x0) solves
%       minimize fun(x)
%   x = bobyqa_last(fun, x0, lb) solves
...
%       solver: backend solver that does the computation, i.e., 'bobyqa_last'
...
%   options = struct();
%   options.maxfun = 50;
%   x = bobyqa_last(@cos, -1, 2, 3, options);
%
%   solves
%       min cos(x) s.t. 2 <= x <= 3
...
elseif (nvararg >= 2 && nvararg <= maxarg)
    ...
    args = [varargin(1:2), cell(1, 4), varargin(3:4), {[]}, varargin(end)]; % args{:} (should have 10 entries) will be the inputs for preprima_last
else
    if ismember(invoker, internal_invokers) % Private/unexpected error
