function testpdfon(release, precision, nrun)
%TESTPDFO tests pdfon on a few VERY simple problems.
%
%   Note: Do NOT follow the syntax here when you use pdfon. This file is
%   written for testing purpose, and it uses quite atypical syntax. See
%   rosenbrock_example.m for an illustration about how to use pdfon.
%
%   ***********************************************************************
            ...
            for isolver = 1 : length(solver_list{itype})
                ...
                for ifun = 1 : length(fun_list)
                    for iclflag = 1 : length(clflag_list)
                        ...
                        fun = fun_list{ifun};
                        x0 = x0_list{ifun};
                        if ~strcmp(solver, 'cobylan') || ~strcmp(func2str(fun), 'chebquad')
                            % The result of cobylan on chebquad is sensitive to x0, so we do not perturb x0
                        ...
                        switch type
                        case 'unconstrained'
                            [x, fx] = pdfon(fun, x0, options);
                        case 'bound-constrained' % 0 <= x <= 0.5
                            lb = -0.5*ones(n,1);
                            ub = 0.5*ones(n,1);
                            [x, fx] = pdfon(fun, x0, [], [], [], [], lb, ub, options);
                            problem.lb = lb;
                            problem.ub = ub;
                        case 'linearly-constrained' % simplex
                            ...
                            bineq = 1;
                            lb = zeros(n,1);
                            [x, fx] = pdfon(fun, x0, Aineq, bineq, [], [], lb, [], options);
                            problem.Aineq = Aineq;
                            problem.bineq = bineq;
                            problem.lb = lb;
                        case 'nonlinearly-constrained' % intersection of the unit ball and the positive orthant
                            ...
                            lb = zeros(n,1);
                            [x, fx] = pdfon(fun, x0, [], [], [], [], lb, [], nonlcon, options);
                            problem.lb = lb;
                        ...
                        if strcmp(solver, 'cobylan') % The precision of cobylan is lower
                        ...
                        if (norm(x-xs) > 0) || ((fx-fopt)/max(1, abs(fopt)) > prec) || (~release && abs(fx-fopt)/max(1, abs(fopt)) > prec)
                            success = false;
                            if clflag
                                fprintf('\n**** pdfon (classical mode) FAILED a test on %s problem: solver = ''%s'', objective function = ''%s''.\n', type, solver, func2str(fun));
                            else
                                fprintf('\n**** pdfon FAILED a test on %s problem: solver = ''%s'', objective function = ''%s''.\n', type, solver, func2str(fun));
                            end
