function [mrec, seed] = testcu(solver, problem, options)

% Specify the paths to the directories containing the mex files and 
% probinfo.mat, which were generated by getcu.m.
testcu_dir = '/home/zaikunzhang/local/cute_sif/testcu/';
mexdir = [testcu_dir, 'mex/']; 
probinfodir = [testcu_dir, 'probinfo/'];
probinfomat = 'probinfo.mat';
probinfomat = [probinfodir, probinfomat];

testedprob = './problem.test'; % This file records the list of tested problems.

datadir = './test_data/'; % This directory contains seed.testalg.mat. 
system(['mkdir -p ', datadir]);

if (nargin < 2)
    fprintf('solver and problem must be specified.\n');
elseif (nargin == 2)
    options = [];
end
...
if (nargin==3 && ~isempty(options))
    ...
    elseif (isfield(options, 'reproduce') && options.reproduce == true) % Set options.reproduce = true to reproduce the results of the last experiment. 
        load(strcat(datadir,'seed.testalg.mat'), 'seed');
    else
        rng('shuffle'); 
        seed = ceil(1e8*rand); 
        save(strcat(datadir,'seed.testalg.mat'), 'seed');
    end
else
    seed = 0;
end
...
ctol = 1e-10;
cpenalty = 10;
type = 'u'; % The types of problems to test 
mindim = 2; % The minimal dimension of problems to test
maxdim = 6; % The maximal dimension of problems to test
mincon = 0; % The minimal number of constraints of problems to test 
maxcon = 20*maxdim; % The maximal number of constraints of problems to test
thorough_test = 1;
...
% Define the lists of solvers and test problems
...
for iprob = 1:length(probinfo)
    ...
    if (any(strcmpi(probs, name)) && (np_max==1 || select_prob(probinfo{iprob}, options))) % If only one problem is specified (i.e., np_max=1), then we do not call select_prob
        ...
        cd([mexdir, name]); % Note that everything below is conducted in mexdir/name
        problem = build_problem(name, fun_options, con_options);
        ...
        cutest_terminate();
        cd(olddir); % Note that everything until here is conducted in mexdir/name
    end
end
fclose(fprob);
toc; 
...
try
    ...
    fprintf('%s encouters an error when solving %s with the following messaege:\n %s\n', solv, problem.name, exception.message);
...
if strcmpi(solv, 'oldcobyla')
    fval_history = fval_history(2:end);
    cv_history = cv_history(2:end);
end
nf = length(fval_history);
...
if (~contains(requirements.type, probinfo.type, 'IgnoreCase', true))
    selected = false;
elseif (probinfo.n < requirements.mindim || probinfo.n > requirements.maxdim) 
    selected = false;
elseif (probinfo.lnlcon < requirements.mincon || probinfo.lnlcon > requirements.maxcon) 
    selected = false;
