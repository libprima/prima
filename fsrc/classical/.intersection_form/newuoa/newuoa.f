...
! IS_FINITE(RHOEND) even if PRESENT(RHOEND) is false!
      ...
      elseif (maxfun_loc >= 1) then
          npt_loc = int(max(n + 2, min(maxfun_loc - 1, 2 * n + 1)), kind&
     &(npt_loc))
      else
...
!-------------------- Call NEWUOB, which performs the real calculations. --------------------------!
      call newuob(calfun, iprint_loc, maxfun_loc, npt_loc, ftarget_loc, &
     &rhobeg_loc, rhoend_loc, x, f, info_loc, nf_loc, xhist_loc, fhist_l&
     &oc)
!--------------------------------------------------------------------------------------------------!
!--------------------------------------------------------------------------------------------------!
