...
!*++
     ...
     &        biglsq, bsum, cauchy, crvmin, curv, delsq, delta,  &
     &        den, denom, densav, diff, diffa, diffb, diffc,     &
     &        dist, distsq, dnorm, dsq, dx, errbig, fopt,        &
     &        fracsq, frhosq, gisq, gqsq, half, hdiag, one,      &
     &        pqold, ratio, rho, scaden, sum, suma, sumb, sumpq,&
     &        sumw, sumz, temp, ten, tenth, two, vquad, xoptsq, &
     &        zero
      integer :: i, ih, ip, itest, j, jj, jp, k, kbase, knew, &
     &           kopt, ksav, nf, nfsav, nh, np, nptm, nresc,    &
     &           ntrits
!*++
...
!     1  FVAL,GOPT,HQ,PQ,BMAT,ZMAT,NDIM,SL,SU,NF,KOPT)
      call PRELIM(N, Npt, X, Xl, Xu, Rhobeg, Iprint, Maxfun, Xbase, Xpt, Fval,    &
     &            Gopt, Hq, Pq, Bmat, Zmat, Ndim, Sl, Su, nf, kopt, F, Ftarget)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
          ...
          if (crvmin > zero .and. errbig > frhosq * crvmin) goto 800
          bdtol = errbig / rho
          do j = 1, N
              bdtest = bdtol
...
!     2  VLAG,W,W(N+NP),W(NDIM+NP))
     ...
     &            Pq, Bmat, Zmat, Ndim, Sl, Su, nf, delta, kopt, Vlag, W, W(N + np), &
     &            W(Ndim + np), F, Ftarget)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
500   do j = 1, N
      ...
      call ALTMOV(N, Npt, Xpt, Xopt, Bmat, Zmat, Ndim, Sl, Su, kopt, knew, adelt,  &
...
99002         format(/4X, 'Return from BOBYQA because a trust',           &
...
99003         format(5X)
              print 99004, rho, nf
99004         format(/4X, 'New RHO =', 1PD11.4, 5X, 'Number of',             &
       &              ' function values =', I6)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          print 99005, nf
99005     format(/4X, 'At the return from BOBYQA', 5X,                    &
      &           'Number of function values =', I6)
          print 99008, F, (X(i), i=1, N)
      end if
99006 format(/5X, 'Return from BOBYQA because of much',                 &
