...
!*==cobylb.f90  processed by SPAG 7.50RE at 00:16 on 26 May 2021
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
!      SUBROUTINE COBYLB (N,M,MPP,X,RHOBEG,RHOEND,IPRINT,MAXFUN,
!     1  CON,SIM,SIMI,DATMAT,A,VSIG,VETA,SIGBAR,DX,W,IACT)
            subroutine COBYLB(N, M, Mpp, X, Rhobeg, Rhoend, Iprint, Maxf&
     &un, Con, Sim, Simi, Datmat, A, Vsig, Veta, Sigbar, Dx, W, Iact, F,&
     & Info, Ftarget, Resmax)
            implicit none
!*--COBYLB14
!*++
...
!*++
            real*8 :: almost_infinity, alpha, barmu, beta, cmax, cmin, c&
     &sum, cvmaxm, cvmaxp, delta, denom, dxsign, edgmax, error, gamma, h&
     &ugenum, pareta, parmu, parsig, phi, phimin, prerec, prerem, ratio,&
     & resnew, resref, rho, sum, temp, tempa, trured, vmnew, vmold, weta&
     &, wsig
            logical :: better
            real*8, dimension(Mpp) :: consav, datdrop
            real*8, dimension(Mpp, NSMAX) :: datsav
            integer :: i, ibrnch, idxnew, iflag, ifull, iptem, iptemp, i&
     &sdirn, ivmc, ivmd, iz, izdota, j, jdrop, k, l, mp, nbest, nfvals, &
     &np, nsav
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      99001 format(/3X, 'The initial value of RHO is', 1PE13.6, 2X, 'and&
...
!     at X). Similar thing can be said about RESMAX.
      ...
      99002 format(/3X, 'Return from subroutine COBYLA because the ', 'M&
     &AXFUN limit has been reached.')
                    Info = 3
                    goto 500
                end if
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! JDROP < NP is guaranteed
                    ...
                    call SAVEX(xdrop(1:N), datdrop(1:Mpp), xsav(1:N, 1:N&
     &SMAX), datsav(1:Mpp, 1:NSMAX), N, M, nsav, NSMAX, CTOL)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    if (jdrop == 0) goto 400
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            ...
                            do i = 1, N
                                Simi(j, i) = Simi(j, i) - temp * Simi(jd&
     &rop, i)
                            end do
                        end if
                    end do
...
!      IF (TRURED .GT. 0.0 .AND. TRURED .GE. 0.1*PREREM) GOTO 140
                    if (trured <= 0.0D0 .or. trured < 0.1D0 * prerem) go&
     &to 400
                    exit
                else
!
...
!              X(JDROP)=SIM(JDROP,NP)
                                if (nfvals <= N) X(jdrop) = Sim(jdrop, n&
     &p)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            else
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        if (Datmat(Mpp, j) < Datmat(Mpp, nbest)) nbest =&
     & j
                    end if
                end do
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      99003 format(/3X, 'Return from subroutine COBYLA because ', 'round&
     &ing errors are becoming damaging.')
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
                    Info = 7
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    goto 500
...
!      VETA(J)=SQRT(WETA)
                    ...
                    Veta(j) = DSQRT(weta)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    if (Vsig(j) < parsig .or. Veta(j) > pareta) iflag = &
     &0
                end do
!
...
! the code, including uninitialized indices.
      ...
      99004 format(/3X, 'Return from subroutine COBYLA because ', 'round&
     &ing errors are becoming damaging.')
                                Info = 7
                                goto 500
                            end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                                if (Datmat(Mpp, j) < Datmat(Mpp, np)) go&
     &to 300
                            end if
                        end do
                    end if
...
! will happen because we will read SIM(:, JDROP) and VSIG(JDROP).
      ...
      99006 format(/3X, 'Return from subroutine COBYLA because ', 'round&
     &ing errors are becoming damaging.')
                        Info = 7
                        goto 500
                    end if
...
!      IF (PARMU*(CVMAXP-CVMAXM) .GT. SUM+SUM) DXSIGN=-1.0
                    dxsign = 1.0D0
                    if (parmu * (cvmaxp - cvmaxm) > sum + sum) dxsign = &
     &-1.0D0
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
! See the comments above for why to check these J
                    if (Datmat(Mpp, j) <= resref) then
                        call ISBETTER(F, Resmax, Datmat(mp, j), Datmat(M&
     &pp, j), parmu, CTOL, better)
                        if (better) then
                            do i = 1, N
...
!          DO J = 1, NSAV
                do j = nsav, 1, -1 ! We start with the most recent point
                    if (datsav(Mpp, j) <= resref) then
                        call ISBETTER(F, Resmax, datsav(mp, j), datsav(M&
     &pp, j), parmu, CTOL, better)
                        if (better) then
                            do i = 1, N
                                X(i) = xsav(i, j)
...
!*==savex.f90  processed by SPAG 7.50RE at 00:16 on 26 May 2021

!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
! Zaikun 20190820: See the comments below line number 480
            subroutine SAVEX(Xdrop, Datdrop, Xsav, Datsav, N, M, Nsav, N&
     &smax, Ctol)
...
!*++ Dummy argument declarations rewritten by SPAG
!*++
            real*8, intent(IN), dimension(N) :: Xdrop
            real*8, intent(IN), dimension(M + 2) :: Datdrop
            real*8, intent(INOUT), dimension(N, Nsmax) :: Xsav
            real*8, intent(INOUT), dimension(M + 2, Nsmax) :: Datsav
            integer, intent(IN) :: N
            integer, intent(IN) :: M
            integer, intent(INOUT) :: Nsav
            integer, intent(IN) :: Nsmax
            real*8, intent(IN) :: Ctol
!*++
!*++ Local variable declarations rewritten by SPAG
!*++
            logical :: better
...
!*==isbetter.f90  processed by SPAG 7.50RE at 00:16 on 26 May 2021
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!*++
            ...
            real*8, intent(IN) :: F
            real*8, intent(IN) :: R
            real*8, intent(IN) :: Parmu
            real*8, intent(IN) :: Ctol
            logical, intent(OUT) :: Better
!*++
!*++ Local variable declarations rewritten by SPAG
!*++
            logical :: f0infnan, finfnan, fle, flt, r0infnan, rinfnan, r&
     &le, rlt
            real*8 :: hugenum = huge(0.0D0)
!*++
!*++ End of declarations rewritten by SPAG
!*++
...
! hold, because one or both of the two sides may be NaN.
            if (Parmu >= 0.0D0 .and. F + Parmu * R < F0 + Parmu * R0 .an&
     &d. R < Ctol) Better = .true.
...
! insfeasible ones.
            if (R < Ctol .and. .not. (R0 < Ctol) .and. .not. finfnan) Be&
     &tter = .true.

! If F0 or R0 is Inf/NaN while neither F nor R is Inf/NaN, then (F, R)
! is better than (F0, R0).
            if ((f0infnan .or. r0infnan) .and. .not. (finfnan .or. rinfn&
     &an)) Better = .true.
...
! F0 nor R0 is -Inf, then the (F, R) is better than (F0, R0).
            if ((F < -hugenum) .and. (.not. rinfnan) .and. (F0 >= -hugen&
     &um) .and. (R0 >= -hugenum)) Better = .true.
            if ((R < -hugenum) .and. (.not. finfnan) .and. (F0 >= -hugen&
     &um) .and. (R0 >= -hugenum)) Better = .true.
