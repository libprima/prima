...
! Zaikun 20190820: See the comments below line number 480
      ...
      write (10, *) '530 bs'
      call SAVEX(XDROP(1:N), DATDROP(1:MPP), XSAV(1:N, 1:NSMAX), DATSAV(&
     &1:MPP, 1:NSMAX), N, M, NSAV, NSMAX, CTOL)
      write (10, *) '530 as'
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
! See the comments above for why to check these J
              if (DATMAT(MPP, J) <= RESREF) then
                  call ISBETTER(F, RESMAX, DATMAT(MP, J), DATMAT(MPP, J)&
     &, PARMU, CTOL, BETTER)
                  if (BETTER) then
                      do I = 1, N
...
! because XDROP should not be inluded into XSAV.
          call ISBETTER(DATDROP(MP), DATDROP(MPP), DATSAV(MP, I), DATSAV&
     &(MPP, I), PARMU, CTOL, BETTER)
          if (BETTER) return
! If XDROP dominates XSAV(:, I), then increment NREMOVE by 1 and save
! I as IREMOVE(NREMOVE).
          call ISBETTER(DATSAV(MP, I), DATSAV(MPP, I), DATDROP(MP), DATD&
     &ROP(MPP), PARMU, CTOL, BETTER)
          if (BETTER) then
              NREMOVE = NREMOVE + 1
              IREMOVE(NREMOVE) = I
          end if
...
! hold, because one or both of the two sides may be NaN.
      if (PARMU >= ZERO .and. F + PARMU * R < F0 + PARMU * R0 .and. R < &
     &CTOL) then
          BETTER = .true.
      end if
...
! is better than (F0, R0).
      if ((F0INFNAN .or. R0INFNAN) .and. .not. (FINFNAN .or. RINFNAN)) t&
     &hen
          BETTER = .true.
      end if
...
! F0 nor R0 is -Inf, then the (F, R) is better than (F0, R0).
      if ((F < -HUGENUM) .and. (.not. RINFNAN) .and. (F0 >= -HUGENUM) .a&
     &nd. (R0 >= -HUGENUM)) BETTER = .true.
      if ((R < -HUGENUM) .and. (.not. FINFNAN) .and. (F0 >= -HUGENUM) .a&
     &nd. (R0 >= -HUGENUM)) BETTER = .true.
      end subroutine
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
