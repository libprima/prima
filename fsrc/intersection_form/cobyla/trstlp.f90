!*==trstlp.f90  processed by SPAG 7.50RE at 00:16 on 26 May 2021
      SUBROUTINE TRSTLP(N,M,A,B,Rho,Dx,Ifull,Iact,Z,Zdota,Vmultc,Sdirn, &
     &                  Dxnew,Vmultd)
...
!*++
     ...
     &        ratio , resmax , resold , sd , sp , spabs , ss , step ,   &
     &        stpful , sum , sumabs , sumd , temp , tempa , tot ,       &
     &        vsave , zdotv , zdotw , zdvabs , zdwabs
      REAL*8 , DIMENSION(N) :: dsav
      INTEGER :: i , icon , icount , isave , iterc , j , k , kk , kl ,  &
     &           kp , kw , mcon , nact , nactx
!*++
...
!                  IOUT=K
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                  IF ( ratio<0.0D0 .OR. tempa<ratio ) ratio = tempa
               ENDIF
               IF ( k>=2 ) THEN
                  kw = Iact(k)
                  DO i = 1 , N
