...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine getact(n, m, amat, nact, iact, qfac, rfac, snorm, resne&
     &w, resact, g, dw, vlam, w)
...
!      DIMENSION AMAT(N,*),B(*),IACT(*),QFAC(N,*),RFAC(*),
     ...
     &resnew(m), resact(m), g(n), dw(n), vlam(n), w(n)
!
...
810   if (jc < nact) then
          ...
          if (jcp < nact) then
              do j = jcp + 1, nact
                  ...
                  rfac(jw + jcp) = cval * rfac(jw + jc) - sval * rfac(jw&
     & + jcp)
                  rfac(jw + jc) = temp
                  jw = jw + j
              end do
