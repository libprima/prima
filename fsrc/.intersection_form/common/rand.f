!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This is the intersection-form version of rand.f90.
...
! Local variable
      ...
      integer :: n
      ! Should be a default INTEGER according to F2018.
...
! Note that 1.0_DP cannot be written as ONE, because KIND(ONE) = RP, which may not be DP.
      cos_seed = min(max(cos(real([(i, i=seed - n + 1, seed)], DP)), -1.&
     &0_DP), 1.0_DP)
...
!--------------------------------------------------------------------------------------------------!
      ...
      if (present(seed)) then
          ...
          if (allocated(seed_to_put)) deallocate (seed_to_put)
          allocate (seed_to_put(1:n), stat=alloc_status)
          if (.not. (alloc_status == 0 .and. allocated(seed_to_put))) th&
     &en
              call errstop(srname, 'Memory allocation fails.')
          end if
