!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This is the intersection-form version of checkexit.f90.
...
! Generic modules
      ...
      use, non_intrinsic :: info_mod, only : INFO_DFT, NAN_INF_X, NAN_IN&
     &F_F, FTARGET_ACHIEVED, MAXFUN_REACHED
...
! Preconditions
      if (DEBUGGING) then
          call assert(.not. any([NAN_INF_X, NAN_INF_F, FTARGET_ACHIEVED,&
     & MAXFUN_REACHED] == INFO_DFT), 'NAN_INF_X, NAN_INF_F, FTARGET_ACHI&
     &EVED, and MAXFUN_REACHED differ from INFO_DFT', srname)
...
!====================!
      ...
      info = INFO_DFT
      ! Default info, indicating that the solver should not exit.
...
! Postconditions
      if (DEBUGGING) then
          call assert(any([INFO_DFT, NAN_INF_X, FTARGET_ACHIEVED, MAXFUN&
     &_REACHED] == info), 'INFO is NAN_INF_X, FTARGET_ACHIEVED, MAXFUN_R&
     &EACHED, or INFO_DFT', srname)
      end if

      end function checkexit_unc


      function checkexit_con(maxfun, nf, cstrv, ctol, f, ftarget, x) res&
     &ult(info)
!--------------------------------------------------------------------------------------------------!
...
! With the moderated extreme barrier, F or CSTRV cannot be NaN/+Inf.
          call assert(.not. (is_nan(f) .or. is_posinf(f) .or. is_nan(cst&
     &rv) .or. is_posinf(cstrv)), 'F or CSTRV is not NaN/+Inf', srname)
      end if
...
! Although NAN_INF_F should not happen unless there is a bug, we include the following for security.
      if (is_nan(f) .or. is_posinf(f) .or. is_nan(cstrv) .or. is_posinf(&
     &cstrv)) then
          info = NAN_INF_F
      end if
...
! Postconditions
      if (DEBUGGING) then
          call assert(any([INFO_DFT, NAN_INF_F, FTARGET_ACHIEVED, MAXFUN&
     &_REACHED] == info), 'INFO is NAN_INF_X, FTARGET_ACHIEVED, MAXFUN_R&
     &EACHED, or INFO_DFT', srname)
      end if
