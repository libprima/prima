!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This is the intersection-form version of shiftbase.f90.
...
! Started: July 2020
!
! Last Modified: Wednesday, January 26, 2022 AM10:28:41
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
     ...
     &QUART, DEBUGGING
      ...
      use, non_intrinsic :: linalg_mod, only : r1update, r2update, inpro&
     &d, matprod, issymmetric, hess_mul
...
! Preconditions
     ...
     &E(XBASE) == N, XBASE is finite', srname)
          call assert(size(xopt) == n .and. all(is_finite(xopt)), 'SIZE(&
     &XOPT) == N, XOPT is finite', srname)
...
! W1 equals MATPROD(XPT, XOPT) after XPT is updated TEMPORARILY as follows.
      ...
      ! Implement R2UPDATE properly so that HQ is symmetric.
