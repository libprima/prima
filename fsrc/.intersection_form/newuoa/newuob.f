...
! Generic modules
      ...
      use, non_intrinsic :: consts_mod, only : RP, IK, ONE, TWO, HALF, T&
     &ENTH, HUGENUM, DEBUGGING
...
! Solver-specific modules
      ...
      use, non_intrinsic :: update_mod, only : updateh, updateq, updatex&
     &f, tryqalt
...
! Local variables
      ...
      real(RP), parameter :: tr_tol = 1.0E-2_RP
...
! Sizes
     ...
     &ND > 0', srname)
          call assert(eta1 >= 0 .and. eta1 <= eta2 .and. eta2 < 1, '0 <=&
     & ETA1 <= ETA2 < 1', srname)
          call assert(gamma1 > 0 .and. gamma1 < 1 .and. gamma2 > 1, '0 <&
     & GAMMA1 < 1 < GAMMA2', srname)
...
! Initialize FVAL, XBASE, and XPT.
      call initxf(calfun, iprint, maxfun, ftarget, rhobeg, x, ij, kopt, &
     &nf, fhist, fval, xbase, xhist, xpt, subinfo)
      xopt = xpt(:, kopt)
      fopt = fval(kopt)
      x = xbase + xopt
      ! Set X.
...
! The last calculated X can be Inf (finite + finite can be Inf numerically).
     ...
     &n NaN/+Inf', srname)
              call assert(.not. any(fhist(1:min(nf, maxfhist)) < f), 'F &
     &is the smallest in FHIST', srname)
          end if
          return
      end if
...
! Solve the trust-region subproblem.
          call trsapp(delta, gq, hq, pq, tr_tol, xopt, xpt, crvmin, d, s&
     &ubinfo)
...
! REDUCE_RHO_1 corresponds to Box 14 of the NEWUOA paper.
          reduce_rho_1 = shortd .and. (maxval(abs(moderrsav)) <= 0.125_R&
     &P * crvmin * rho**2) .and. (maxval(dnormsav) <= rho)
...
! KNEW_GEO may equal KOPT, with which GEOSTEP will not work.
              tr_success = (f < fopt)
              knew_tr = setdrop_tr(idz, kopt, tr_success, bmat, d, delta&
     &, rho, xpt, zmat)
...
! Update the quadratic model using the updated BMAT, ZMAT, IDZ.
              call updateq(idz, knew_tr, kopt, bmat, d, f, fval, xpt, zm&
     &at, gq, hq, pq)
...
! reset ITEST to 0 when RHO is reduced?
              ...
              ! DELTA = RHO.
                  call tryqalt(idz, fval - fopt, ratio, bmat, zmat, ites&
     &t, gq, hq, pq)
              end if
          end if
...
! of BAD_TRSTEP. Nevertheless, we keep it for robustness.
          ...
          improve_geo = (.not. reduce_rho_1) .and. (maxval(xdist) > TWO &
     &* delta) .and. bad_trstep
...
! 3. DELTA < DNORM may hold due to the update of DELTA.
          bad_trstep = (shortd .or. ratio <= 0 .or. knew_tr == 0)
          ! BAD_TRSTEP for REDUCE_RHO_2
          reduce_rho_2 = (maxval(xdist) <= TWO * delta) .and. (max(delta&
     &, dnorm) <= rho) .and. bad_trstep
...
! Update the quadratic model using the updated BMAT, ZMAT, IDZ.
              call updateq(idz, knew_geo, kopt, bmat, d, f, fval, xpt, z&
     &mat, gq, hq, pq)
...
! The calculations with the current RHO are complete. Pick the next values of RHO and DELTA.
              if (rho <= rhoend) then
                  ...
                  delta = max(delta, rho)
                  call rhomsg(solver, iprint, nf, fopt, rho, xbase + xop&
     &t)
...
! The last calculated X can be Inf (finite + finite can be Inf numerically).
     ...
     &N/+Inf', srname)
          call assert(.not. any(fhist(1:min(nf, maxfhist)) < f), 'F is t&
     &he smallest in FHIST', srname)
      end if
