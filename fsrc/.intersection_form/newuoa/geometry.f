...
!--------------------------------------------------------------------------------------------------!
      ...
      function setdrop_tr(idz, kopt, tr_success, bmat, d, delta, rho, xp&
     &t, zmat) result(knew)
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
      use, non_intrinsic :: consts_mod, only : RP, IK, ONE, TENTH, DEBUG&
     &GING
...
! Preconditions
     ...
     &rname)
          call assert(delta >= rho .and. rho > 0, 'DELTA >= RHO > 0', sr&
     &name)
...
! Calculate the distance between the interpolation points and the optimal point up to now.
      if (tr_success) then
          xdist = sqrt(sum((xpt - spread(xpt(:, kopt) + d, dim=2, ncopie&
     &s=npt))**2, dim=1))
      else
          xdist = sqrt(sum((xpt - spread(xpt(:, kopt), dim=2, ncopies=np&
     &t))**2, dim=1))
      end if

      hdiag = -sum(zmat(:, 1:idz - 1)**2, dim=2) + sum(zmat(:, idz:size(&
     &zmat, 2))**2, dim=2)
...
! See (7.5) of the NEWUOA paper for the following definition of KNEW.
      if (any(sigma > ONE) .or. (tr_success .and. any(.not. is_nan(sigma&
     &)))) then
          knew = int(maxloc(sigma, mask=(.not. is_nan(sigma)), dim=1), k&
     &ind(knew))
      end if
...
! of only NaN, then KNEW can be 0 even when RATIO > 0.
      if (tr_success .and. knew <= 0) then
          knew = int(maxloc(xdist, mask=(.not. is_nan(xdist)), dim=1), k&
     &ind(knew))
      end if
...
! Postconditions
      if (DEBUGGING) then
          call assert(knew >= 0 .and. knew <= npt, '0 <= KNEW <= NPT', s&
     &rname)
          call assert(knew >= 1 .or. .not. tr_success, 'KNEW >= 1 unless&
     & TR_SUCCESS = FALSE', srname)
          call assert(knew /= kopt .or. tr_success, 'KNEW /= KOPT unless&
     & TR_SUCCESS = TRUE', srname)
...
! Generic modules
      use, non_intrinsic :: consts_mod, only : RP, IK, ZERO, ONE, TWO, D&
     &EBUGGING
      use, non_intrinsic :: debug_mod, only : assert
      use, non_intrinsic :: infnan_mod, only : is_finite
      use, non_intrinsic :: linalg_mod, only : issymmetric, norm, omega_&
     &col
...
! Preconditions
     ...
     & srname)
          call assert(idz >= 1 .and. idz <= size(zmat, 2) + 1, '1 <= IDZ&
     & <= SIZE(ZMAT, 2) + 1', srname)
          call assert(knew >= 1 .and. knew <= npt, '1 <= KNEW <= NPT', s&
     &rname)
          call assert(kopt >= 1 .and. kopt <= npt, '1 <= KOPT <= NPT', s&
     &rname)
...
! Generic modules
     ...
     &ALF, QUART, TENTH, EPS, DEBUGGING
      use, non_intrinsic :: circle_mod, only : circle_maxabs
      use, non_intrinsic :: debug_mod, only : assert
      use, non_intrinsic :: infnan_mod, only : is_finite
      use, non_intrinsic :: linalg_mod, only : Ax_plus_y, inprod, matpro&
     &d, issymmetric, norm, project, omega_col
...
! Preconditions
     ...
     &T+1:NPT+N) is symmetric', srname)
          call assert(delbar > 0, 'DELBAR > 0', srname)
          call assert(size(x) == n .and. all(is_finite(x)), 'SIZE(X) == &
     &N, X is finite', srname)
          call assert(all(is_finite(xpt)), 'XPT is finite', srname)
          call assert(size(zmat, 1) == npt .and. size(zmat, 2) == npt - &
     &n - 1, 'SIZE(ZMAT) == [NPT, NPT - N - 1]', srname)
      end if
...
! Scale D and GD, with a sign change if needed. Set S to another vector in the initial 2-D subspace.
      ...
      if (is_finite(sum(abs(scaling * d)))) then
          ...
          maxiter = 0_IK
          ! Return immediately to avoid producing a D containing NaN/Inf.
      end if
...
! In precise arithmetic, INPROD(S, D) = 0 and |S| = |D| = DELBAR.
          if (abs(inprod(d, s)) >= TENTH * norm(d) * norm(s) .or. norm(s&
     &) >= TWO * delbar) then
              exit
          end if
...
! Test for convergence.
          if (abs(circle_fun_biglag(angle, cf)) <= 1.1_RP * abs(circle_f&
     &un_biglag(ZERO, cf))) then
              exit
          end if
...
! Generic modules
      use, non_intrinsic :: consts_mod, only : RP, IK, ZERO, ONE, TWO, H&
     &ALF, TENTH, QUART, EPS, DEBUGGING
      use, non_intrinsic :: circle_mod, only : circle_maxabs
      use, non_intrinsic :: debug_mod, only : assert
      use, non_intrinsic :: infnan_mod, only : is_finite
      use, non_intrinsic :: linalg_mod, only : inprod, matprod, issymmet&
     &ric, norm, project, omega_col, omega_mul
...
! Preconditions
     ...
     & 'SIZE(BMAT)==[N, NPT+N]', srname)
          call assert(issymmetric(bmat(:, npt + 1:npt + n)), 'BMAT(:, NP&
     &T+1:NPT+N) is symmetric', srname)
          call assert(size(d0) == n .and. all(is_finite(d0)), 'SIZE(D0) &
     &== N, D0 is finite', srname)
          call assert(all(is_finite(xpt)), 'XPT is finite', srname)
          call assert(size(zmat, 1) == npt .and. size(zmat, 2) == npt - &
     &n - 1, 'SIZE(ZMAT) == [NPT, NPT - N - 1]', srname)
      end if
...
!====================!
      ...
      x = xpt(:, kopt)
      ! For simplicity, we use X to denote XOPT.
...
! K can be 0 due to NaN. In that case, set K = KNEW. Otherwise, memory errors will occur.
          ...
          if ((.not. (dstemp(k)**2 / sstemp(k) >= dtest)) .and. k /= kop&
     &t) then
! `.NOT. (A >= B)` differs from `A < B`.  The former holds iff A < B or {A, B} contains NaN.
...
! Put the coefficients of THETA*WCHECK in PROD.
          do j = 1, 5
              ...
              nw = npt
              if (j == 2 .or. j == 3) then
                  prod(1:npt, j) = prod(1:npt, j) + matprod(w(npt + 1:np&
     &t + n, j), bmat(:, 1:npt))
                  nw = npt + n
              end if
              prod(npt + 1:npt + n, j) = matprod(bmat(:, 1:nw), w(1:nw, &
     &j))
          end do
...
! Include in DEN the part of BETA that depends on THETA.
          ...
          denex(5) = alpha * den(5) + tempa + prod(knew, 2) * prod(knew,&
     & 3)
...
! Set S to HALF the gradient of the denominator with respect to D. First, calculate the new VLAG.
          par = [ONE, cos(angle), sin(angle), cos(2.0_RP * angle), sin(2&
     &.0_RP * angle)]
...
! Postconditions
      if (DEBUGGING) then
          call assert(size(d) == n .and. all(is_finite(d)), 'SIZE(D) == &
     &N, D is finite', srname)
          call assert(norm(d) <= TWO * norm(d0), '|D| <= 2*DELBAR', srna&
     &me)
! Due to rounding, it may happen that |D| > DELBAR, but |D| > 2*DELBAR is highly improbable.
      end if
...
!====================!
      ...
      cth = cos(theta)
      sth = sin(theta)
      f = args(1) + (args(2) + args(4) * cth) * cth + (args(3) + args(5)&
     & * cth) * sth
