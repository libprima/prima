...
! Started: July 2020
!
! Last Modified: Monday, November 22, 2021 AM10:49:29
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
      use, non_intrinsic :: consts_mod, only : RP, IK, ONE, ZERO, DEBUGG&
     &ING
      use, non_intrinsic :: debug_mod, only : assert
      use, non_intrinsic :: infnan_mod, only : is_finite
      use, non_intrinsic :: linalg_mod, only : matprod, planerot, r2upda&
     &te, symmetrize, issymmetric
...
! In the loop, if 2 <= J < IDZ, then JL = 1; if IDZ < J <= NPT - N - 1, then JL = IDZ.
      ...
      do j = 2, int(npt - n - 1, kind(j))
          ...
          if (abs(zmat(knew, j)) > ZERO) then
              grot = planerot(zmat(knew, [jl, j]))
              ! MATLAB code: GROT = PLANEROT(ZMAT(KNEW, [JL, J])')
              zmat(:, [jl, j]) = matprod(zmat(:, [jl, j]), transpose(gro&
     &t))
              zmat(knew, j) = ZERO
          end if
      end do
...
!    & 'H = W^{-1} in (3.12) of the NEWUOA paper', srname)
      ...
      subroutine updateq(idz, knew, kopt, bmat, d, f, fval, xpt, zmat, g&
     &q, hq, pq)
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
      ...
      use, non_intrinsic :: debug_mod, only : assert
      use, non_intrinsic :: infnan_mod, only : is_finite, is_posinf, is_&
     &nan
      use, non_intrinsic :: linalg_mod, only : r1update, issymmetric, ca&
     &lquad, omega_col
...
! Preconditions
     ...
     &rname)
          call assert(kopt >= 1 .and. kopt <= npt, '1 <= KOPT <= NPT', s&
     &rname)
          call assert(knew >= 1 .or. f >= fval(kopt), 'KNEW >= 1 unless &
     &F >= FVAL(KOPT)', srname)
          call assert(knew /= kopt .or. f < fval(kopt), 'KNEW /= KOPT un&
     &less F < FVAL(KOPT)', srname)
          call assert(size(bmat, 1) == n .and. size(bmat, 2) == npt + n,&
     & 'SIZE(BMAT)==[N, NPT+N]', srname)
          call assert(issymmetric(bmat(:, npt + 1:npt + n)), 'BMAT(:, NP&
     &T+1:NPT+N) is symmetric', srname)
...
! Postconditions
      if (DEBUGGING) then
          call assert(kopt >= 1 .and. kopt <= npt, '1 <= KOPT <= NPT', s&
     &rname)
          call assert(abs(f - fval(knew)) <= ZERO, 'F == FVAL(KNEW)', sr&
     &name)
          call assert(abs(fopt - fval(kopt)) <= ZERO, 'FOPT == FVAL(KOPT&
     &)', srname)
          call assert(size(xopt) == n, 'SIZE(XOPT) == N', srname)
          call assert(norm(xopt - xpt(:, kopt)) <= ZERO, 'XOPT == XPT(:,&
     & KOPT)', srname)
          call assert(.not. any(fval < fopt), '.NOT. ANY(FVAL < FOPT)', &
     &srname)
      end if
