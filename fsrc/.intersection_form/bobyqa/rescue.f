...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine rescue(calfun, n, npt, xl, xu, iprint, maxfun, xbase, x&
     &pt, fval, xopt, gopt, hq, pq, bmat, zmat, ndim, sl, su, nf, delta,&
     & kopt, vlag, ptsaux, ptsid, w, f, ftarget, xhist, maxxhist, fhist,&
     & maxfhist)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      dimension xbase(*), xpt(npt, *), fval(*), xopt(*), gopt(*), hq(*),&
     & pq(*), bmat(ndim, *), zmat(npt, *), sl(*), su(*), vlag(*), ptsaux&
     &(2, *), ptsid(*), w(*)
!
...
!
      ...
      do j = 1, n
          ...
          if (jpn <= npt) then
              ...
              zmat(1, j) = dsqrt(2.0D0) / dabs(ptsaux(1, j) * ptsaux(2, &
     &j))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                  ...
                  if (ip == 0) then
                      ...
                      if (iq > 0) then
                          ...
                          hq(iw) = hq(iw) + temp * ptsaux(1, ip) * ptsau&
     &x(1, iq)
                      end if
                  end if
              end if
