...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine bobyqb(calfun, n, npt, x, xl, xu, rhobeg, rhoend, iprin&
     &t, maxfun, xbase, xpt, fval, xopt, gopt, hq, pq, bmat, zmat, ndim,&
     & sl, su, xnew, xalt, d, vlag, w, f, info, ftarget, nf, xhist, maxx&
     &hist, fhist, maxfhist)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      dimension xbase(n), xpt(npt, n), fval(npt), xopt(n), gopt(n), hq(n&
     & * (n + 1) / 2), pq(npt), bmat(ndim, *), zmat(npt, *), sl(*), su(*&
     &), xnew(n), xalt(n), d(n), vlag(*), w(*)
!
...
!
      call prelim(calfun, n, npt, x, xl, xu, rhobeg, iprint, maxfun, xba&
     &se, xpt, fval, gopt, hq, pq, bmat, zmat, ndim, sl, su, nf, kopt, f&
     &, ftarget, xhist, maxxhist, fhist, maxfhist)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
60    do i = 1, n
      ...
      call trsbox(n, npt, xpt, xopt, gopt, hq, pq, sl, su, delta, xnew, &
     &d, w, w(np), w(np + n), w(np + 2 * n), w(np + 3 * n), dsq, crvmin)
...
90    if (dsq <= 1.0D-3 * xoptsq) then
          ...
          do k = 1, npt
              ...
              do i = 1, n
                  ...
                  ip = npt + i
                  do j = 1, i
                      bmat(ip, j) = bmat(ip, j) + w(i) * vlag(j) + vlag(&
     &i) * w(j)
                  end do
              end do
          end do
...
!------------------------------------------------------------------------------------------------!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      call rescue(calfun, n, npt, xl, xu, iprint, maxfun, xbase, xpt, fv&
     &al, xopt, gopt, hq, pq, bmat, zmat, ndim, sl, su, nf, delta, kopt,&
     & vlag, w, w(n + np), w(ndim + np), f, ftarget, xhist, maxxhist, fh&
     &ist, maxfhist)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!------------------------------------------------------------------------------------------------!
...
210   do j = 1, n
      ...
      call altmov(n, npt, xpt, xopt, bmat, zmat, ndim, sl, su, kopt, kne&
     &w, adelt, xnew, xalt, alpha, cauchy, w, w(np), w(ndim + 1))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      do i = 1, n
          d(i) = xnew(i) - xopt(i)
      end do
...
230   do k = 1, npt
      ...
      beta = dx * dx + dsq * (xoptsq + dx + dx + HALF * dsq) + beta - bs&
     &um
