...
!--------------------------------------------------------------------------------------------------!
      ...
      function goodgeo(delta, factor_alpha, factor_beta, sim, simi) resu&
     &lt(good_geo)
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
      use, non_intrinsic :: consts_mod, only : IK, RP, ONE, TENTH, DEBUG&
     &GING
...
! Preconditions
     ...
     &(SIMI) == [N, N]', srname)
          ...
          call assert(isinv(sim(:, 1:n), simi, itol), 'SIMI = SIM(:, 1:N&
     &)^{-1}', srname)
          call assert(delta > 0, 'DELTA > 0', srname)
          call assert(factor_alpha > 0 .and. factor_alpha < 1, '0 < FACT&
     &OR_ALPHA < 1', srname)
...
!====================!
      ...
      function setdrop_tr(tr_success, d, delta, factor_alpha, factor_del&
     &ta, sim, simi) result(jdrop)
!--------------------------------------------------------------------------------------------------!
...
! when TR_SUCCESS == FALSE, MAXVAL(ABS(SIMID)) <= 1, and MAXVAL(VETA) <= EDGMAX).
      ...
      if (any(abs(simid) > ONE) .or. (tr_success .and. any(.not. is_nan(&
     &simid)))) then
          jdrop = int(maxloc(abs(simid), mask=(.not. is_nan(simid)), dim&
     &=1), kind(jdrop))
      end if

      if (tr_success) then
          veta = sqrt(sum((sim(:, 1:n) - spread(d, dim=2, ncopies=n))**2&
     &, dim=1))
      else
          veta = sqrt(sum(sim(:, 1:n)**2, dim=1))
      end if
...
! The following JDROP will overwrite the previous one if its premise holds.
      if (any(veta > edgmax .and. (sigbar >= parsig .or. sigbar >= vsig)&
     &)) then
          jdrop = int(maxloc(veta, mask=(veta > edgmax .and. (sigbar >= &
     &parsig .or. sigbar >= vsig)), dim=1), kind(jdrop))
      end if
...
! and we exit once encountering an iterate containing Inf (due to overflow).
      if (tr_success .and. jdrop <= 0) then
      ! Write JDROP <= 0 instead of JDROP == 0 for robustness.
          jdrop = int(maxloc(veta, mask=(.not. is_nan(veta)), dim=1), ki&
     &nd(jdrop))
      end if
...
! Postconditions
      if (DEBUGGING) then
          call assert(jdrop >= 0 .and. jdrop <= n, '0 <= JDROP <= N', sr&
     &name)
          call assert(jdrop >= 1 .or. .not. tr_success, 'JDROP >= 1 unle&
     &ss the trust-region step failed', srname)
      end if

      end function setdrop_tr


      function setdrop_geo(delta, factor_alpha, factor_beta, sim, simi) &
     &result(jdrop)
!--------------------------------------------------------------------------------------------------!
...
! acceptability of the simplex. See equations (15) and (16) of the COBYLA paper.
     ...
     &nd(jdrop))
      elseif (any(vsig < parsig)) then
          jdrop = int(minloc(vsig, mask=(.not. is_nan(vsig)), dim=1), ki&
     &nd(jdrop))
      else
...
!Postconditions
      if (DEBUGGING) then
          call assert(jdrop >= 1 .and. jdrop <= n, '1 <= JDROP <= N', sr&
     &name)
      end if
      end function setdrop_geo


      function geostep(jdrop, cpen, conmat, cval, delta, fval, factor_ga&
     &mma, simi) result(d)
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
      use, non_intrinsic :: consts_mod, only : IK, RP, ZERO, ONE, TWO, D&
     &EBUGGING
      use, non_intrinsic :: debug_mod, only : assert
...
! Preconditions
     ...
     &or. is_posinf(fval)), 'SIZE(FVAL) == NPT and FVAL is not NaN/+Inf'&
     &, srname)
     ...
     &or. is_posinf(cval)), 'SIZE(CVAL) == NPT and FVAL is not NaN/+Inf'&
     &, srname)
          call assert(jdrop >= 1 .and. jdrop <= n, '1 <= JDROP <= N', sr&
     &name)
      end if
...
! finite-precision arithmetic.
      A(:, 1:m) = transpose(matprod(conmat(:, 1:n) - spread(conmat(:, n &
     &+ 1), dim=2, ncopies=n), simi))
...
! Postconditions
      if (DEBUGGING) then
          call assert(size(d) == n .and. all(is_finite(d)), 'SIZE(D) == &
     &N, D is finite', srname)
          call assert(norm(d) <= TWO * factor_gamma * delta, '|D| <= 2*F&
     &ACTOR_GAMMA*DELTA', srname)
      end if
