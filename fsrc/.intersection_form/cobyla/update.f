...
!--------------------------------------------------------------------------------------------------!
      ...
      subroutine updatexfc(jdrop, constr, cpen, cstrv, d, f, conmat, cva&
     &l, fval, sim, simi, info)
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
      ...
      use, non_intrinsic :: infnan_mod, only : is_nan, is_neginf, is_pos&
     &inf, is_finite
...
! Preconditions
      if (DEBUGGING) then
          call assert(jdrop >= 0 .and. jdrop <= n, '1 <= JDROP <= N', sr&
     &name)
          call assert(.not. any(is_nan(constr) .or. is_neginf(constr)), &
     &'CONSTR does not contain NaN/-Inf', srname)
          call assert(.not. (is_nan(cstrv) .or. is_posinf(cstrv)), 'CSTR&
     &V is not NaN/+Inf', srname)
...
!====================!
     ...
     &(SIMI) == [N, N]', srname)
          call assert(all(is_finite(simi)), 'SIMI is finite', srname)
          call assert(isinv(sim(:, 1:n), simi, itol) .or. info == DAMAGI&
     &NG_ROUNDING, 'SIMI = SIM(:, 1:N)^{-1} unless the rounding is damag&
     &ing', srname)
      end if
...
! Generic modules
      use, non_intrinsic :: consts_mod, only : IK, RP, ZERO, TENTH, DEBU&
     &GGING
...
! When JOPT == N + 1, there is nothing to switch; in addition, SIMI(JOPT, :) will be illegal.
          ...
          ! Exchange CONMAT(:, JOPT) AND CONMAT(:, N+1)
...
! Postconditions
      if (DEBUGGING) then
          call assert(findpole(cpen, cval, fval) == n + 1 .or. info == D&
     &AMAGING_ROUNDING, 'The best point is SIM(:, N+1) unless the roundi&
     &ng is damaging', srname)
          call assert(size(conmat, 1) == m .and. size(conmat, 2) == n + &
     &1, 'SIZE(CONMAT) = [M, N+1]', srname)
          call assert(.not. any(is_nan(conmat) .or. is_neginf(conmat)), &
     &'CONMAT does not contain NaN/-Inf', srname)
...
! Identify the optimal vertex of the current simplex.
      jopt = int(size(fval), kind(jopt))
      ! We use N + 1 as the default value of JOPT.
      phi = fval + cpen * cval
      phimin = minval(phi)
      if (phimin < phi(jopt)) then
      ! We keep JOPT = N + 1 unless there is a strictly better choice.
          jopt = int(minloc(phi, dim=1), kind(jopt))
      end if
      if (cpen <= ZERO .and. any(cval < cval(jopt) .and. phi <= phimin))&
     & then
...
! We code in this way to avoid equality comparison of real numbers.
          jopt = int(minloc(cval, mask=(phi <= phimin), dim=1), kind(jop&
     &t))
      end if
...
! Postconditions
      if (DEBUGGING) then
          call assert(jopt >= 1 .and. jopt <= n + 1, '1 <= JOPT <= N+1',&
     & srname)
      end if
