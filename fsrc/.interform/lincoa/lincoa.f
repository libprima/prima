...
!--------------------------------------------------------------------------------------------------!
      ...
      subroutine lincoa(calfun, x, f, cstrv, A, b, nf, rhobeg, rhoend, f&
     &target, ctol, cweight, maxfun, npt, iprint, eta1, eta2, gamma1, ga&
     &mma2, xhist, fhist, chist, maxhist, maxfilt, info)
!--------------------------------------------------------------------------------------------------!
!
...
! Working variables (to be removed)
      ...
      integer(IK) :: iw, iamat, ib, ndim, ixb, ixp, ifv, ixs, ixo, igo, &
     &ihq, ipq, ibmat, izmat, istp, isp, ixn, iac, irc, iqf, irf, ipqw, &
     &j, m
...
! Preconditions
      if (DEBUGGING) then
          ...
          call assert(present(A) .eqv. present(b), 'A and B are both pre&
     &sent or both absent', srname)
          if (present(A)) then
              call assert((size(A, 1) == n .and. size(A, 2) == m) .or. (&
     &size(A, 1) == 0 .and. size(A, 2) == 0 .and. m == 0), 'SIZE(A) == [&
     &N, M] unless A and B are both empty', srname)
          end if
      end if
...
! Preprocess the inputs in case some of them are invalid. It does nothing if all inputs are valid.
     ...
     &et_loc, rhobeg_loc, rhoend_loc, npt=npt_loc, ctol=ctol_loc, cweigh&
     &t=cweight_loc, eta1=eta1_loc, eta2=eta2_loc, gamma1=gamma1_loc, ga&
     &mma2=gamma2_loc, maxfilt=maxfilt_loc)
...
! if they are requested; replace MAXFUN with 0 for the history that is not requested.
     ...
     &ist), fhist_loc, present(chist), chist_loc)


!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Working space (to be removed)
      call safealloc(w, m * (2_IK + n) + npt_loc * (4_IK + n + npt_loc) &
     &+ n * (9_IK + 3_IK * n) + max(m + 3_IK * n, 2_IK * m + n, 2_IK * n&
     &pt_loc))
...
! is feasible.
      ...
      if (m > 0) then
          ...
          do j = 1, m
              ...
              temp = dsqrt(temp)
              constr_modified = constr_modified .or. (sum - b_loc(j) > s&
     &mallx * temp)
      ...
      call lincob(calfun, n, npt_loc, m, w(iamat), w(ib), x, rhobeg_loc,&
     & rhoend_loc, iprint_loc, maxfun_loc, w(ixb), w(ixp), w(ifv), w(ixs&
     &), w(ixo), w(igo), w(ihq), w(ipq), w(ibmat), w(izmat), ndim, w(ist&
     &p), w(isp), w(ixn), iact, w(irc), w(iqf), w(irf), w(ipqw), w, f, i&
     &nfo_loc, ftarget_loc, A_loc, b_loc, cstrv_loc, nf_loc, xhist_loc, &
     &size(xhist_loc, 2, kind=IK), fhist_loc, size(fhist_loc, kind=IK), &
     &chist_loc, size(chist_loc, kind=IK))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! If NF_LOC > MAXHIST_LOC, warn that not all history is recorded.
      if ((present(xhist) .or. present(fhist) .or. present(chist)) .and.&
     & maxhist_loc < nf_loc) then
