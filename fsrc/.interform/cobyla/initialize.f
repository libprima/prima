...
!--------------------------------------------------------------------------------------------------!
      ...
      subroutine initxfc(calcfc, iprint, maxfun, constr0, ctol, f0, ftar&
     &get, rhobeg, x0, nf, chist, conhist, conmat, cval, fhist, fval, si&
     &m, simi, xhist, evaluated, info)
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
      ...
      use, non_intrinsic :: consts_mod, only : RP, IK, ZERO, TENTH, HUGE&
     &NUM, DEBUGGING
      use, non_intrinsic :: debug_mod, only : assert
      use, non_intrinsic :: evaluate_mod, only : evaluate, moderatef, mo&
     &deratec
...
! Sizes
      ...
      maxhist = int(max(maxchist, maxconhist, maxfhist, maxxhist), kind(&
     &maxhist))
...
! Preconditions
     ...
     &r -3', srname)
          call assert(size(conmat, 1) == m .and. size(conmat, 2) == n + &
     &1, 'SIZE(CONMAT) = [M, N+1]', srname)
          call assert(size(cval) == n + 1, 'SIZE(CVAL) == N+1', srname)
          call assert(size(fval) == n + 1, 'SIZE(FVAL) == N+1', srname)
          call assert(size(sim, 1) == n .and. size(sim, 2) == n + 1, 'SI&
     &ZE(SIM) == [N, N+1]', srname)
          call assert(size(simi, 1) == n .and. size(simi, 2) == n, 'SIZE&
     &(SIMI) == [N, N]', srname)
          call assert(size(evaluated) == n + 1, 'SIZE(EVALUATED) == N + &
     &1', srname)
...
! Save X, F, CONSTR, CSTRV into the history.
          call savehist(k, x, xhist, f, fhist, cstrv, chist, constr, con&
     &hist)
...
! Postconditions
     ...
     &, srname)
          call assert(.not. any(evaluated(1:min(nf, maxchist)) .and. (is&
     &_nan(chist(1:min(nf, maxchist))) .or. is_posinf(chist(1:min(nf, ma&
     &xchist))))), 'CHIST does not contain NaN/Inf', srname)
          call assert(size(conhist, 1) == m .and. size(conhist, 2) == ma&
     &xconhist, 'SIZE(CONHIST) == [M, MAXCONHIST]', srname)
          call assert(size(conmat, 1) == m .and. size(conmat, 2) == n + &
     &1, 'SIZE(CONMAT) = [M, N+1]', srname)
          call assert(.not. any(is_nan(conmat) .or. is_neginf(conmat)), &
     &'CONMAT does not contain NaN/-Inf', srname)
          call assert(size(cval) == n + 1 .and. .not. any(is_nan(cval) .&
     &or. is_posinf(cval)), 'SIZE(CVAL) == N+1 and CVAL is not NaN/+Inf'&
     &, srname)
          call assert(size(fhist) == maxfhist, 'SIZE(FHIST) == MAXFHIST'&
     &, srname)
          call assert(maxfhist * (maxfhist - maxhist) == 0, 'SIZE(FHIST)&
     & == 0 or MAXHIST', srname)
          call assert(size(fval) == n + 1 .and. .not. any(is_nan(fval) .&
     &or. is_posinf(fval)), 'SIZE(FVAL) == N+1 and FVAL is not NaN/+Inf'&
     &, srname)
     ...
     &ZE(SIM) == [N, N+1]', srname)
          ...
          call assert(size(simi, 1) == n .and. size(simi, 2) == n, 'SIZE&
     &(SIMI) == [N, N]', srname)
          call assert(all(is_finite(simi)) .or. subinfo == DAMAGING_ROUN&
     &DING, 'SIMI is finite', srname)
          call assert(isinv(sim(:, 1:n), simi, itol) .or. any(.not. eval&
     &uated) .or. info == DAMAGING_ROUNDING, 'SIMI = SIM(:, 1:N)^{-1} un&
     &less the rounding is damaging', srname)
      end if

      end subroutine initxfc


      subroutine initfilt(conmat, ctol, cweight, cval, fval, sim, evalua&
     &ted, nfilt, cfilt, confilt, ffilt, xfilt)
!--------------------------------------------------------------------------------------------------!
...
! Preconditions
     ...
     &t, 'SIZE(XFILT) == [N, MAXFILT]', srname)
          call assert(size(ffilt) == maxfilt, 'SIZE(FFILT) == MAXFILT', &
     &srname)
          call assert(size(conmat, 1) == m .and. size(conmat, 2) == n + &
     &1, 'SIZE(CONMAT) = [M, N+1]', srname)
          call assert(.not. any(is_nan(conmat) .or. is_neginf(conmat)), &
...
!====================!
      ...
      do i = 1, n + 1_IK
          if (evaluated(i)) then
              ...
              call savefilt(conmat(:, i), cval(i), ctol, cweight, fval(i&
     &), x, nfilt, cfilt, confilt, ffilt, xfilt)
          end if
      end do
...
! Postconditions
     ...
     &ame)
          call assert(size(cfilt) == maxfilt, 'SIZE(CFILT) == MAXFILT', &
     &srname)
          call assert(.not. any(is_nan(cfilt(1:nfilt)) .or. is_posinf(cf&
     &ilt(1:nfilt))), 'CFILT does not contain NaN/Inf', srname)
          call assert(size(xfilt, 1) == n .and. size(xfilt, 2) == maxfil&
     &t, 'SIZE(XFILT) == [N, MAXFILT]', srname)
