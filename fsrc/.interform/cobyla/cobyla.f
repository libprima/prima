...
!--------------------------------------------------------------------------------------------------!
      ...
      subroutine cobyla(calcfc, m, x, f, cstrv, constr, f0, constr0, nf,&
     & rhobeg, rhoend, ftarget, ctol, cweight, maxfun, iprint, eta1, eta&
     &2, gamma1, gamma2, xhist, fhist, chist, conhist, maxhist, maxfilt,&
     & info)
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
      ...
      use, non_intrinsic :: consts_mod, only : MAXFUN_DIM_DFT, MAXFILT_D&
     &FT, IPRINT_DFT
      use, non_intrinsic :: consts_mod, only : RHOBEG_DFT, RHOEND_DFT, C&
     &TOL_DFT, CWEIGHT_DFT, FTARGET_DFT
     ...
     &ALF, TEN, TENTH, EPS, MSGLEN
      ...
      use, non_intrinsic :: infnan_mod, only : is_nan, is_finite, is_neg&
     &inf, is_posinf
...
! Preconditions
      if (DEBUGGING) then
          call assert(present(f0) .eqv. present(constr0), 'F0 and CONSTR&
     &0 are both present or both absent', srname)
      end if
...
! Otherwise, set [F, CONSTR_LOC] = [F(X0), CONSTR(X0)], so that COBYLB only needs a single interface.
      if (present(f0) .and. present(constr0) .and. all(is_finite(x))) th&
     &en
          f = f0
          constr_loc = constr0
      else
! Replace any NaN in X by ZERO and Inf/-Inf in X by HUGENUM/-HUGENUM.
          ...
          ! Indeed, CSTRV_LOC needs not to be evaluated.
      end if
...
! Preprocess the inputs in case some of them are invalid. It does nothing if all inputs are valid.
     ...
     &et_loc, rhobeg_loc, rhoend_loc, m=m, ctol=ctol_loc, cweight=cweigh&
     &t_loc, eta1=eta1_loc, eta2=eta2_loc, gamma1=gamma1_loc, gamma2=gam&
     &ma2_loc, maxfilt=maxfilt_loc)
...
! if they are requested; replace MAXFUN with 0 for the history that is not requested.
     ...
     &ist), fhist_loc, present(chist), chist_loc, m, present(conhist), c&
     &onhist_loc)

!-------------------- Call COBYLB, which performs the real calculations. --------------------------!
      call cobylb(calcfc, iprint_loc, maxfilt_loc, maxfun_loc, ctol_loc,&
     & cweight_loc, eta1_loc, eta2_loc, ftarget_loc, gamma1_loc, gamma2_&
     &loc, rhobeg_loc, rhoend_loc, constr_loc, f, x, nf_loc, chist_loc, &
     &conhist_loc, cstrv_loc, fhist_loc, xhist_loc, info_loc)
!--------------------------------------------------------------------------------------------------!
...
!--------------------------------------------------!
          call safealloc(chist, nhist)
          ! Removable in F2003.
!--------------------------------------------------!
          chist = chist_loc(1:nhist)
          ! The same as XHIST, we must cap CHIST at NF_LOC.
      end if
...
!----------------------------------------------------------!
          call safealloc(conhist, m, nhist)
          ! Removable in F2003.
!----------------------------------------------------------!
          conhist = conhist_loc(:, 1:nhist)
          ! The same as XHIST, we must cap CONHIST at NF_LOC.
      end if
      deallocate (conhist_loc)

! If NF_LOC > MAXHIST_LOC, warn that not all history is recorded.
      if ((present(xhist) .or. present(fhist) .or. present(chist) .or. p&
     &resent(conhist)) .and. maxhist_loc < nf_loc) then
...
! Postconditions
     ...
     &, 'FHIST does not contain NaN/+Inf', srname)
          ...
          if (present(chist)) then
              call assert(size(chist) == nhist, 'SIZE(CHIST) == NHIST', &
     &srname)
              call assert(.not. any(is_nan(chist) .or. is_posinf(chist))&
     &, 'CHIST does not contain NaN/+Inf', srname)
          end if
          if (present(conhist)) then
              call assert(size(conhist, 1) == m .and. size(conhist, 2) =&
     &= nhist, 'SIZE(CONHIST) == [M, NHIST]', srname)
              call assert(.not. any(is_nan(conhist) .or. is_neginf(conhi&
     &st)), 'CONHIST does not contain NaN/-Inf', srname)
          end if
          if (present(fhist) .and. present(chist)) then
              call assert(.not. any([(isbetter([fhist(i), chist(i)], [f,&
     & cstrv_loc], ctol_loc), i=1, nhist)]), 'No point in the history is&
     & better than X', srname)
          end if
      end if
