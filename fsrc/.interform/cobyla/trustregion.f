...
! Preconditions
      if (DEBUGGING) then
          call assert(size(A, 1) >= 1 .and. size(A, 2) >= 1, 'SIZE(A) >=&
     & [1, 1]', srname)
          call assert(size(b) == size(A, 2), 'SIZE(B) == size(A, 2)', sr&
     &name)
      end if
...
!====================!
      ...
      call trstlp_sub(iact(1:m), nact, 1_IK, A(:, 1:m), b(1:m), delta, d&
     &, vmultc(1:m), z)
...
! Postconditions
      if (DEBUGGING) then
          call assert(size(d) == size(A, 1), 'SIZE(D) == SIZE(A, 1)', sr&
     &name)
...
! Generic modules
     ...
     &UGENUM, DEBUGGING
      ...
      use, non_intrinsic :: linalg_mod, only : inprod, matprod, eye, ism&
     &inor, lsqr, qradd, qrexc, norm
      implicit none
...
! Preconditions
      if (DEBUGGING) then
          call assert(n >= 1 .or. mcon == 0, 'N >= 1 when MCON > 0', srn&
     &ame)
          call assert(size(b) == mcon, 'SIZE(B) == MCON', srname)
          call assert(size(iact) == mcon, 'SIZE(IACT) == MCON', srname)
          call assert(size(vmultc) == mcon, 'SIZE(VMULTC) == MCON', srna&
     &me)
          call assert(size(d) == n, 'SIZE(D) == N', srname)
          call assert(size(z, 1) == n .and. size(z, 2) == n, 'SIZE(Z) ==&
     & [N, N]', srname)
          call assert(delta > 0, 'DELTA > 0', srname)
          if (stage == 2) then
              call assert(all(is_finite(d)) .and. norm(d) <= TWO * delta&
     &, 'D is finite and |D| <= 2*DELTA at the beginning of stage 2', sr&
     &name)
              call assert((nact >= 0 .and. nact <= min(mcon, n)), '0 <= &
     &NACT <= MIN(MCON, N) at the beginning of stage 2', srname)
              call assert(all(vmultc(1:mcon - 1) >= 0), 'VMULTC >= 0 at &
     &the beginning of stage 2', srname)
...
! Check whether a quick return is possible. Make sure the In-outputs have been initialized.
          ...
          if (all(is_nan(b))) then
              ...
              icon = int(maxloc(b, mask=(.not. is_nan(b)), dim=1), kind(&
     &icon))
          end if
          m = mcon
          sdirn = ZERO
...
! In MATLAB or Python: MAXITER = MIN(10000, 100*MAX(M, N))
      maxiter = int(min(int(10_IK**min(4, range(0_IK)), IK), 100_IK * ma&
     &x(m, n)), kind(maxiter))
      do iter = 1, maxiter
          if (DEBUGGING) then
...
! computer rounding errors, which is tested by ISMINOR.
          if (icon > nact) then
              ...
              ! QRADD may update NACT tp NACT + 1.
...
! Powell did, we should use the UNUPDATED version, namely ZDASAV.
                  vmultd(1:nact) = lsqr(A(:, iact(1:nact)), A(:, iact(ic&
     &on)), z(:, 1:nact), zdasav(1:nact))
                  if (.not. any(vmultd(1:nact) > ZERO .and. iact(1:nact)&
     & <= m)) then
...
! Only the places where VMULTD > 0 and IACT <= M is relevant blow, if any.
                  ...
                  ! FRACMULT(NACT+1:MCON) may contain garbage.
...
! N.B.: We cannot arrive here with NACT == 0, which should have triggered an exit above.
                  if (abs(zdota(nact)) <= 0 .or. is_nan(zdota(nact))) th&
     &en
                      exit
                  end if
...
! Postconditions
     ...
     &me)
          ...
          call assert(size(z, 1) == n .and. size(z, 2) == n, 'SIZE(Z) ==&
     & [N, N]', srname)
          call assert(nact >= 0 .and. nact <= min(mcon, n), '0 <= NACT <&
     &= MIN(MCON, N)', srname)
      end if
      ...
      function trrad(delta_in, dnorm, eta1, eta2, gamma1, gamma2, ratio)&
     & result(delta)
!--------------------------------------------------------------------------------------------------!
...
! Preconditions
      if (DEBUGGING) then
          call assert(delta_in >= dnorm .and. dnorm > 0, 'DELTA_IN >= DN&
     &ORM > 0', srname)
...
!====================!
      ...
      elseif (ratio <= eta2) then
          ...
          ! Powell' version (taken from NEWUOA)
