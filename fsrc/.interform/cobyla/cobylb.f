...
!--------------------------------------------------------------------------------------------------!
      ...
      subroutine cobylb(calcfc, iprint, maxfilt, maxfun, ctol, cweight, &
     &eta1, eta2, ftarget, gamma1, gamma2, rhobeg, rhoend, constr, f, x,&
     & nf, chist, conhist, cstrv, fhist, xhist, info)

! Generic modules
      use, non_intrinsic :: checkexit_mod, only : checkexit
      use, non_intrinsic :: consts_mod, only : RP, IK, ZERO, TWO, HALF, &
     &QUART, TENTH, HUGENUM, DEBUGGING
      use, non_intrinsic :: consts_mod, only : MIN_MAXFILT
      use, non_intrinsic :: debug_mod, only : assert
      use, non_intrinsic :: evaluate_mod, only : evaluate
      use, non_intrinsic :: history_mod, only : savehist, rangehist
      use, non_intrinsic :: infnan_mod, only : is_nan, is_posinf, is_neg&
     &inf
      use, non_intrinsic :: info_mod, only : INFO_DFT, MAXTR_REACHED, SM&
     &ALL_TR_RADIUS, DAMAGING_ROUNDING
      use, non_intrinsic :: linalg_mod, only : inprod, matprod, norm
      use, non_intrinsic :: output_mod, only : retmsg, rhomsg, fmsg, cpe&
     &nmsg
      use, non_intrinsic :: pintrf_mod, only : OBJCON
      use, non_intrinsic :: ratio_mod, only : redrat
      use, non_intrinsic :: redrho_mod, only : redrho
      use, non_intrinsic :: selectx_mod, only : savefilt, selectx, isbet&
     &ter

! Solver-specific modules
      use, non_intrinsic :: geometry_mod, only : goodgeo, setdrop_geo, s&
     &etdrop_tr, geostep
      use, non_intrinsic :: initialize_mod, only : initxfc, initfilt
      use, non_intrinsic :: trustregion_mod, only : trstlp, trrad
      use, non_intrinsic :: update_mod, only : updatexfc, updatepole, fi&
     &ndpole
...
!    factors for updating the trust-region radius DELTA.
      ...
      ! The factor alpha in the COBYLA paper
      real(RP), parameter :: factor_beta = 2.1_RP
      ! The factor better in the COBYLA paper
      real(RP), parameter :: factor_delta = 1.1_RP
      ! The factor delta in the COBYLA paper
      real(RP), parameter :: factor_gamma = HALF
      ! The factor gamma in the COBYLA paper
      real(RP) :: ratio
...
! Sizes
      ...
      maxhist = int(max(maxxhist, maxfhist, maxconhist, maxchist), kind(&
     &maxhist))

! Preconditions
      if (DEBUGGING) then
          call assert(abs(iprint) <= 3, 'IPRINT is 0, 1, -1, 2, -2, 3, o&
     &r -3', srname)
          call assert(n >= 1, 'N >= 1', srname)
          call assert(maxfun >= n + 2, 'MAXFUN >= N + 2', srname)
          call assert(rhobeg >= rhoend .and. rhoend > 0, 'RHOBEG >= RHOE&
     &ND > 0', srname)
!call assert(eta1 >= 0 .and. eta1 <= eta2 .and. eta2 < 1, '0 <= ETA1 <= ETA2 < 1', srname)
!call assert(gamma1 > 0 .and. gamma1 < 1 .and. gamma2 > 1, '0 < GAMMA1 < 1 < GAMMA2', srname)
          call assert(maxhist >= 0 .and. maxhist <= maxfun, '0 <= MAXHIS&
     &T <= MAXFUN', srname)
          call assert(maxfilt >= min(MIN_MAXFILT, maxfun) .and. maxfilt &
     &<= maxfun, 'MIN(MIN_MAXFILT, MAXFUN) <= MAXFILT <= MAXFUN', srname&
     &)
          call assert(size(xhist, 1) == n .and. maxxhist * (maxxhist - m&
     &axhist) == 0, 'SIZE(XHIST, 1) == N, SIZE(XHIST, 2) == 0 or MAXHIST&
     &', srname)
          call assert(maxfhist * (maxfhist - maxhist) == 0, 'SIZE(FHIST)&
     & == 0 or MAXHIST', srname)
          call assert(size(conhist, 1) == m .and. maxconhist * (maxconhi&
     &st - maxhist) == 0, 'SIZE(CONHIST, 1) == M, SIZE(CONHIST, 2) == 0 &
     &or MAXHIST', srname)
          call assert(maxchist * (maxchist - maxhist) == 0, 'SIZE(CHIST)&
     & == 0 or MAXHIST', srname)
      end if
...
! constraint values, and constraint violations on the vertices in the order corresponding to SIM.
      call initxfc(calcfc, iprint, maxfun, constr, ctol, f, ftarget, rho&
     &beg, x, nf, chist, conhist, conmat, cval, fhist, fval, sim, simi, &
     &xhist, evaluated, subinfo)

! Initialize the filter, including XFILT, FFILT, CONFILT, CFILT, and NFILT.
      call initfilt(conmat, ctol, cweight, cval, fval, sim, evaluated, n&
     &filt, cfilt, confilt, ffilt, xfilt)
...
! Postconditions
          if (DEBUGGING) then
              ...
              call assert(size(x) == n .and. .not. any(is_nan(x)), 'SIZE&
     &(X) == N, X does not contain NaN', srname)
              call assert(.not. (is_nan(f) .or. is_posinf(f)), 'F is not&
     & NaN/+Inf', srname)
              call assert(size(xhist, 1) == n .and. size(xhist, 2) == ma&
     &xxhist, 'SIZE(XHIST) == [N, MAXXHIST]', srname)
              call assert(.not. any(is_nan(xhist(:, 1:min(nf, maxxhist))&
     &)), 'XHIST does not contain NaN', srname)
! The last calculated X can be Inf (finite + finite can be Inf numerically).
              call assert(size(fhist) == maxfhist, 'SIZE(FHIST) == MAXFH&
     &IST', srname)
              call assert(.not. any(is_nan(fhist(1:min(nf, maxfhist))) .&
     &or. is_posinf(fhist(1:min(nf, maxfhist)))), 'FHIST does not contai&
     &n NaN/+Inf', srname)
              call assert(size(conhist, 1) == m .and. size(conhist, 2) =&
     &= maxconhist, 'SIZE(CONHIST) == [M, MAXCONHIST]', srname)
              call assert(.not. any(is_nan(conhist(:, 1:min(nf, maxconhi&
     &st))) .or. is_neginf(conhist(:, 1:min(nf, maxconhist)))), 'CONHIST&
     & does not contain NaN/-Inf', srname)
              call assert(size(chist) == maxchist, 'SIZE(CHIST) == MAXCH&
     &IST', srname)
              call assert(.not. any(is_nan(chist(1:min(nf, maxchist))) .&
     &or. is_posinf(chist(1:min(nf, maxchist)))), 'CHIST does not contai&
     &n NaN/+Inf', srname)
              call assert(.not. any([(isbetter([fhist(k), chist(k)], [f,&
     & cstrv], ctol), k=1, minval([nf, maxfhist, maxchist]))]), 'No poin&
     &t in the history is better than X', srname)
          end if
          return
      end if
...
! unlikely to be reached.
      maxtr = max(maxfun, 2_IK * maxfun)
      ! MAX: precaution against overflow, which will make 2*MAXFUN < 0.
      info = MAXTR_REACHED
...
! 2. TRSTLP accesses A mostly by columns, so it is not more reasonable to save A^T instead of A.
          A(:, 1:m) = transpose(matprod(conmat(:, 1:n) - spread(conmat(:&
...
! iff PREREF and CPEN are both 0.
              ...
              ! Can be negative.
              prerec = cval(n + 1) - maxval([-matprod(d, A(:, 1:m)) - co&
     &nmat(:, n + 1), ZERO])
...
! Save X, F, CONSTR, CSTRV into the history.
              call savehist(nf, x, xhist, f, fhist, cstrv, chist, constr&
     &, conhist)
! Save X, F, CONSTR, CSTRV into the filter.
              call savefilt(constr, cstrv, ctol, cweight, f, x, nfilt, c&
     &filt, confilt, ffilt, xfilt)
...
! the predicted and actual reductions in the merit function respectively.
              ...
              ! Theoretically nonnegative; equals 0 if CPEN = 0 = PREREF.
              actrem = (fval(n + 1) + cpen * cval(n + 1)) - (f + cpen * &
     &cstrv)
              if (cpen <= 0 .and. f <= fval(n + 1) .and. f >= fval(n + 1&
     &)) then
...
! We code in this way to avoid compilers complaining about equality comparison of reals.
              ...
              if (is_nan(actrem)) then
                  actrem = -HUGENUM
                  ! Signify a bad trust-region step.
              end if
...
! N.B.: COBYLA never sets JDROP_TR = N + 1.
              tr_success = (actrem > 0)
              jdrop_tr = setdrop_tr(tr_success, d, delta, factor_alpha, &
     &factor_delta, sim, simi)
...
! N.B.: UPDATEXFC does nothing when JDROP_TR == 0.
              call updatexfc(jdrop_tr, constr, cpen, cstrv, d, f, conmat&
     &, cval, fval, sim, simi, subinfo)
...
! Should we enhance the resolution by reducing RHO?
          reduce_rho = bad_trstep .and. good_geo .and. (max(delta, dnorm&
     &) <= rho)
...
! and 5000 constraints, while the algorithm framework is simplified.
              if (.not. goodgeo(delta, factor_alpha, factor_beta, sim, s&
     &imi)) then
...
! N.B.: COBYLA never sets JDROP_GEO = N + 1.
                  jdrop_geo = setdrop_geo(delta, factor_alpha, factor_be&
     &ta, sim, simi)
...
! Calculate the geometry step D.
                  d = geostep(jdrop_geo, cpen, conmat, cval, delta, fval&
     &, factor_gamma, simi)
...
! Evaluate the objective and constraints at X, taking care of possible Inf/NaN values.
                  ...
                  call fmsg(solver, iprint, nf, f, x, cstrv, constr)
! Save X, F, CONSTR, CSTRV into the history.
                  call savehist(nf, x, xhist, f, fhist, cstrv, chist, co&
     &nstr, conhist)
! Save X, F, CONSTR, CSTRV into the filter.
                  call savefilt(constr, cstrv, ctol, cweight, f, x, nfil&
     &t, cfilt, confilt, ffilt, xfilt)
! Update SIM, SIMI, FVAL, CONMAT, and CVAL so that SIM(:, JDROP_GEO) is replaced by D.
                  call updatexfc(jdrop_geo, constr, cpen, cstrv, d, f, c&
     &onmat, cval, fval, sim, simi, subinfo)
! Check whether to exit due to damaging rounding detected in UPDATEPOLE (called by UPDATEXFC).
                  if (subinfo == DAMAGING_ROUNDING) then
                      info = subinfo
                      exit
...
! Check whether to exit.
                  subinfo = checkexit(maxfun, nf, cstrv, ctol, f, ftarge&
     &t, x)
                  if (subinfo /= INFO_DFT) then
                      info = subinfo
                      exit
...
! Enhance the resolution of the algorithm by reducing RHO; update DELTA and CPEN at the same time.
          if (reduce_rho) then
              ...
              ! It may set CPEN to 0.
              call rhomsg(solver, iprint, nf, fval(n + 1), rho, sim(:, n&
     & + 1), cval(n + 1), conmat(:, n + 1), cpen)
              call updatepole(cpen, conmat, cval, fval, sim, simi, subin&
     &fo)
! Check whether to exit due to damaging rounding detected in UPDATEPOLE.
              if (subinfo == DAMAGING_ROUNDING) then
                  info = subinfo
...
! Return the best calculated values of the variables.
! N.B. SELECTX and FINDPOLE choose X by different standards. One cannot replace the other.
      kopt = selectx(ffilt(1:nfilt), cfilt(1:nfilt), max(cpen, cweight),&
     & ctol)
      x = xfilt(:, kopt)
      f = ffilt(kopt)
      constr = confilt(:, kopt)
...
! Postconditions
     ...
     &== N, X does not contain NaN', srname)
          call assert(.not. (is_nan(f) .or. is_posinf(f)), 'F is not NaN&
     &/+Inf', srname)
          call assert(size(xhist, 1) == n .and. size(xhist, 2) == maxxhi&
     &st, 'SIZE(XHIST) == [N, MAXXHIST]', srname)
          call assert(.not. any(is_nan(xhist(:, 1:min(nf, maxxhist)))), &
     &'XHIST does not contain NaN', srname)
! The last calculated X can be Inf (finite + finite can be Inf numerically).
          call assert(size(fhist) == maxfhist, 'SIZE(FHIST) == MAXFHIST'&
     &, srname)
          call assert(.not. any(is_nan(fhist(1:min(nf, maxfhist))) .or. &
     &is_posinf(fhist(1:min(nf, maxfhist)))), 'FHIST does not contain Na&
     &N/+Inf', srname)
          call assert(size(conhist, 1) == m .and. size(conhist, 2) == ma&
     &xconhist, 'SIZE(CONHIST) == [M, MAXCONHIST]', srname)
          call assert(.not. any(is_nan(conhist(:, 1:min(nf, maxconhist))&
     &) .or. is_neginf(conhist(:, 1:min(nf, maxconhist)))), 'CONHIST doe&
     &s not contain NaN/-Inf', srname)
          call assert(size(chist) == maxchist, 'SIZE(CHIST) == MAXCHIST'&
     &, srname)
          call assert(.not. any(is_nan(chist(1:min(nf, maxchist))) .or. &
     &is_posinf(chist(1:min(nf, maxchist)))), 'CHIST does not contain Na&
     &N/+Inf', srname)
          call assert(.not. any([(isbetter([fhist(k), chist(k)], [f, cst&
     &rv], ctol), k=1, minval([nf, maxfhist, maxchist]))]), 'No point in&
     & the history is better than X', srname)
      end if
...
! Preconditions
      if (DEBUGGING) then
          ...
          call assert(size(conmat, 2) == size(fval), 'SIZE(CONMAT, 2) ==&
     & SIZE(FVAL)', srname)
      end if
...
!====================!
      ...
      if (any(cmin < HALF * cmax)) then
          denom = minval(max(cmax, ZERO) - cmin, mask=(cmin < HALF * cma&
     &x))
