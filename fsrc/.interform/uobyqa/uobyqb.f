...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine uobyqb(calfun, n, x, rhobeg, rhoend, iprint, maxfun, np&
     &t, xbase, xopt, xnew, xpt, pq, pl, h, g, d, vlag, w, f, info, ftar&
     &get, nf, xhist, maxxhist, fhist, maxfhist)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      dimension xbase(n), xopt(n), xnew(n), xpt(npt, n), pq(npt - 1), pl&
     &(npt, npt - 1), h(n, n * n), g(n), d(n), vlag(npt), w(max(6 * n, (&
     &n**2 + 3 * n + 2) / 2))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
          ...
          if (jswitch > 0) then
              if (j >= 1) then
                  ...
                  if (w(j) < ZERO) then
                      ...
                      pq(j) = (4.0D0 * fsave - 3.0D0 * fbase - f) / (TWO&
     & * rho)
...
!     Exit if F .LE. FTARGET.
      if (f <= ftarget) then
          info = 1
          goto 430
          ! Should not goto 420. fopt may not be defined yet
      end if
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
