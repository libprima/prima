...
!--------------------------------------------------------------------------------------------------!
      ...
      subroutine initxf(calfun, iprint, maxfun, ftarget, rhobeg, x0, ij,&
     & kopt, nf, fhist, fval, xbase, xhist, xpt, info)
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
     ...
     &BUGGING
      ...
      use, non_intrinsic :: infnan_mod, only : is_finite, is_nan, is_pos&
     &inf
...
! Preconditions
     ...
     &', srname)
          call assert(size(ij, 1) == max(0_IK, npt - 2_IK * n - 1_IK) .a&
     &nd. size(ij, 2) == 2, 'SIZE(IJ) == [NPT - 2*N - 1, 2]', srname)
          ...
          call assert(size(xpt, 1) == n .and. size(xpt, 2) == npt, 'SIZE&
     &(XPT) == [N, NPT]', srname)
      end if
...
! Set XPT.
      ...
      ! XPT(:, 2*N+2 : NPT) = ZERO if it is nonempty.
...
! NPT - (2*N + 1) elements of such a permutation. Powell took the following permutation.
      ij(:, 1) = int(([(k, k=2_IK * n + 2_IK, npt)] - n - 2) / n, IK)
      ! [(K, K=1, NPT)] = [1, 2, ..., NPT]
      ij(:, 2) = int([(k, k=2_IK * n + 2_IK, npt)] - (ij(:, 1) + 1) * n &
     &- 1, IK)
      ij(:, 1) = modulo(ij(:, 1) + ij(:, 2) - 1_IK, n) + 1_IK
      ! MODULO(K-1,N) + 1 = K-N for K in [N+1,2N]
...
! Postconditions
      if (DEBUGGING) then
          call assert(size(ij, 1) == max(0_IK, npt - 2_IK * n - 1_IK) .a&
     &nd. size(ij, 2) == 2, 'SIZE(IJ) == [NPT - 2*N - 1, 2]', srname)
          call assert(all(ij >= 2 .and. ij <= 2 * n + 1), '1 <= IJ <= N'&
     &, srname)
          call assert(all(modulo(ij(:, 1) - 2_IK, n) > modulo(ij(:, 2) -&
     & 2_IK, n)), 'MODULO(IJ(:, 1) - 2, N) > MODULO(IJ(:, 2) - 2, N)', s&
     &rname)
          call assert(nf <= npt, 'NF <= NPT', srname)
          call assert(kopt >= 1 .and. kopt <= nf, '1 <= KOPT <= NF', srn&
     &ame)
          call assert(size(xbase) == n .and. all(is_finite(xbase)), 'SIZ&
     &E(XBASE) == N, XBASE is finite', srname)
          call assert(size(xpt, 1) == n .and. size(xpt, 2) == npt, 'SIZE&
     &(XPT) == [N, NPT]', srname)
          call assert(all(is_finite(xpt)), 'XPT is finite', srname)
          call assert(size(fval) == npt .and. .not. any(evaluated .and. &
     &(is_nan(fval) .or. is_posinf(fval))), 'SIZE(FVAL) == NPT and FVAL &
     &is not NaN or +Inf', srname)
          call assert(.not. any(fval < fval(kopt)), 'FVAL(KOPT) = MINVAL&
     &(FVAL)', srname)
...
! Generic modules
      use, non_intrinsic :: consts_mod, only : RP, IK, ZERO, HALF, DEBUG&
     &GING
      use, non_intrinsic :: debug_mod, only : assert
...
! Preconditions
      if (DEBUGGING) then
          call assert(n >= 1 .and. npt >= n + 2, 'N >= 1, NPT >= N + 2',&
     & srname)
          call assert(size(fval) == npt .and. .not. any(is_nan(fval) .or&
     &. is_posinf(fval)), 'SIZE(FVAL) == NPT and FVAL is not NaN or +Inf&
     &', srname)
          call assert(size(ij, 1) == max(0_IK, npt - 2_IK * n - 1_IK) .a&
     &nd. size(ij, 2) == 2, 'SIZE(IJ) == [NPT - 2*N - 1, 2]', srname)
          call assert(all(ij >= 2 .and. ij <= 2 * n + 1), '1 <= IJ <= N'&
     ...
     &rname)
          ...
          call assert(size(hq, 1) == n .and. size(hq, 2) == n, 'SIZE(HQ)&
     & = [N, N]', srname)
...
! If possible, revise GQ to central difference.
      ...
      gq(1:k) = HALF * (gq(1:k) + (fbeg - fval(n + 2:n + 1 + k)) / rhobe&
     &g)
...
! Thus the HQ(I,J) defined below approximates frac{partial^2}{partial X_I partial X_J} F(XBASE).
          ...
          xi = xpt(i, k + 2 * n + 1)
          xj = xpt(j, k + 2 * n + 1)
          hq(i, j) = (fbeg - fval(ij(k, 1)) - fval(ij(k, 2)) + fval(k + &
     &2 * n + 1)) / (xi * xj)
          hq(j, i) = hq(i, j)
      end do
      ...
      if (any(is_nan(gq)) .or. any(is_nan(hq)) .or. any(is_nan(pq))) the&
     &n
          info = NAN_MODEL
      else
          info = INFO_DFT
...
! Postconditions
      if (DEBUGGING) then
          call assert(size(gq) == n, 'SIZE(GQ) = N', srname)
          call assert(size(hq, 1) == n .and. issymmetric(hq), 'HQ is an &
     &NxN symmetric matrix', srname)
          call assert(size(pq) == npt, 'SIZE(PQ) = NPT', srname)
      end if
...
! Set BMAT.
      ...
      if (npt <= 2 * n + 1) then
! Set BMAT(1 : NPT-N-1, :)
          bmat(1:npt - n - 1, 2:npt - n) = (HALF / rhobeg) * eye(npt - n&
     & - 1_IK)
          bmat(1:npt - n - 1, n + 2:npt) = -(HALF / rhobeg) * eye(npt - &
     &n - 1_IK)
! Set BMAT(NPT-N : N, :)
          bmat(npt - n:n, 1) = -ONE / rhobeg
          bmat(npt - n:n, npt - n + 1:n + 1) = (ONE / rhobeg) * eye(2_IK&
     & * n - npt + 1_IK)
          bmat(npt - n:n, 2 * npt - n:npt + n) = -(HALF * rhosq) * eye(2&
     &_IK * n - npt + 1_IK)
      else
...
! Set ZMAT(:, N+1 : NPT-N-1).
          ...
          zmat(2 * n + 2:npt, n + 1:npt - n - 1) = (ONE / rhosq) * eye(n&
     &pt - 2_IK * n - 1_IK)
          do k = 1, npt - 2_IK * n - 1_IK
