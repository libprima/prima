...
! Generic modules
      use, non_intrinsic :: consts_mod, only : RP, IK, TWO, HALF, ZERO, &
     &TENTH, DEBUGGING
...
!twod_search = (n >= 2)  ! TWOD_SEARCH should be FALSE if N = 1.
              twod_search = (n >= 2 .and. gg > (tol**2) * gg0)
              ! TWOD_SEARCH should be FALSE if N = 1.
              exit
          end if
...
! In precise arithmetic, INPROD(D, S) = 0 and |D| = |S| = DELTA.
          if (abs(inprod(d, s)) >= TENTH * norm(d) * norm(s) .or. norm(d&
     &) >= TWO * delta) then
              info = -1_IK
              exit
          end if
...
! Test for convergence.
          reduc = circle_fun_trsapp(ZERO, args) - circle_fun_trsapp(angl&
     &e, args)
          qred = qred + reduc
...
! Postconditions
      if (DEBUGGING) then
          call assert(size(s) == n .and. all(is_finite(s)), 'SIZE(S) == &
     &N, S is finite', srname)
...
!====================!
      ...
      cth = cos(theta)
      sth = sin(theta)
      f = (args(1) + args(2) * cth) * cth + (args(3) + args(4) * cth) * &
     &sth
