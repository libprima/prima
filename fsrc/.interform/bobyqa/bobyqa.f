...
!--------------------------------------------------------------------------------------------------!
      ...
      subroutine bobyqa(calfun, x, f, xl, xu, nf, rhobeg, rhoend, ftarge&
     &t, maxfun, npt, iprint, eta1, eta2, gamma1, gamma2, xhist, fhist, &
     &maxhist, honour_x0, info)
!--------------------------------------------------------------------------------------------------!
...
! Generic modules
     ...
     &ALF, TEN, TENTH, EPS, HUGENUM, MSGLEN
...
! Working variables (to be removed)
      ...
      integer(IK) :: ndim, ixb, ixp, ifv, ixo, igo, ihq, ipq, ibmat, izm&
     &at, isl, isu, ixn, ixa, id, ivl, iw, j, jsl, jsu
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! Preconditions
      if (DEBUGGING) then
          ...
          if (present(xl)) then
              call assert(size(xl) == n .or. size(xl) == 0, 'SIZE(XL) ==&
     & N unless XL is empty', srname)
          end if
          if (present(xu)) then
              call assert(size(xu) == n .or. size(xu) == 0, 'SIZE(XU) ==&
     & N unless XU is empty', srname)
          end if
      end if
...
! Preprocess the inputs in case some of them are invalid. It does nothing if all inputs are valid.
     ...
     &et_loc, rhobeg_loc, rhoend_loc, npt=npt_loc, eta1=eta1_loc, eta2=e&
     &ta2_loc, gamma1=gamma1_loc, gamma2=gamma2_loc)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! this, INFO=6 should never occur.
      rhobeg_loc = min(0.5D0 * (1.0D0 - 1.0D-5) * minval(xu_loc(1:n) - x&
     &l_loc(1:n)), rhobeg_loc)
...
! Working space (to be removed)
      call safealloc(w, int((npt_loc + 5) * (npt_loc + n) + 3 * n * (n +&
     & 5) / 2, IK))
...
!
      call bobyqb(calfun, n, npt_loc, x, xl_loc, xu_loc, rhobeg_loc, rho&
     &end_loc, iprint_loc, maxfun_loc, w(ixb), w(ixp), w(ifv), w(ixo), w&
     &(igo), w(ihq), w(ipq), w(ibmat), w(izmat), ndim, w(isl), w(isu), w&
     &(ixn), w(ixa), w(id), w(ivl), w(iw), f, info_loc, ftarget_loc, nf_&
     &loc, xhist_loc, size(xhist_loc, 2, kind=IK), fhist_loc, size(fhist&
     &_loc, kind=IK))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
