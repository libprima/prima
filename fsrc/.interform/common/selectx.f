...
!--------------------------------------------------------------------------------------------------!
      ...
      subroutine savefilt(constr, cstrv, ctol, cweight, f, x, nfilt, cfi&
     &lt, confilt, ffilt, xfilt)
!--------------------------------------------------------------------------------------------------!
...
!--------------------------------------------------------------------------------------------------!
      ...
      use, non_intrinsic :: consts_mod, only : RP, IK, ZERO, HUGENUM, DE&
     &BUGGING
...
! Check NFILT
          call assert(nfilt >= 0 .and. nfilt <= maxfilt, '0 <= NFILT <= &
     &MAXFILT', srname)
! Check the sizes of XFILT, FFILT, CONFILT, CFILT.
          call assert(maxfilt >= 1, 'MAXFILT >= 1', srname)
          call assert(size(xfilt, 1) == n .and. size(xfilt, 2) == maxfil&
     &t, 'SIZE(XFILT) == [N, MAXFILT]', srname)
          call assert(size(confilt, 1) == m .and. size(confilt, 2) == ma&
     &xfilt, 'SIZE(CONFILT) == [M, MAXFILT]', srname)
          call assert(size(cfilt) == maxfilt, 'SIZE(CFILT) == MAXFILT', &
     &srname)
! Check the values of XFILT, FFILT, CONFILT, CFILT.
          call assert(.not. any(is_nan(xfilt(:, 1:nfilt))), 'XFILT does &
     &not contain NaN', srname)
          call assert(.not. any(is_nan(ffilt(1:nfilt)) .or. is_posinf(ff&
     &ilt(1:nfilt))), 'FFILT does not contain NaN/+Inf', srname)
          call assert(.not. any(is_nan(confilt(:, 1:nfilt)) .or. is_negi&
     &nf(confilt(:, 1:nfilt))), 'CONFILT does not contain NaN/-Inf', srn&
     &ame)
          call assert(.not. any(cfilt(1:nfilt) < 0 .or. is_nan(cfilt(1:n&
     &filt)) .or. is_posinf(cfilt(1:nfilt))), 'CFILT does not contain no&
     &nnegative values of NaN/+Inf', srname)
...
! CSTRV cannot be NaN/+Inf due to the moderated extreme barrier.
          call assert(.not. (cstrv < 0 .or. is_nan(cstrv) .or. is_posinf&
     &(cstrv)), 'CSTRV is nonnegative and not NaN/+Inf', srname)
      end if
...
! BETTER is defined by an array constructor with an implied do loop.
      better = [(isbetter([ffilt(i), cfilt(i)], [f, cstrv], ctol), i=1, &
     &nfilt)]
      if (any(better)) then
          return
      end if
...
! Decide which columns of XFILT to keep. We use again an array constructor with an implied do loop.
      keep = [(.not. isbetter([f, cstrv], [ffilt(i), cfilt(i)], ctol), i&
     &=1, nfilt)]
...
! not ensure that F_1 == F_2!!!
          ...
          kworst = int(maxloc(cfilt, mask=(ffilt >= fref), dim=1), kind(&
     &kworst))
          if (kworst < 1 .or. kworst > size(keep)) then
...
! Check NFILT and the sizes of XFILT, FFILT, CONFILT, CFILT.
          call assert(nfilt >= 1 .and. nfilt <= maxfilt, '1 <= NFILT <= &
     &MAXFILT', srname)
          call assert(size(xfilt, 1) == n .and. size(xfilt, 2) == maxfil&
     &t, 'SIZE(XFILT) == [N, MAXFILT]', srname)
          call assert(size(ffilt) == maxfilt, 'SIZE(FFILT) = MAXFILT', s&
     &rname)
          call assert(size(confilt, 1) == m .and. size(confilt, 2) == ma&
     &xfilt, 'SIZE(CONFILT) == [M, MAXFILT]', srname)
          call assert(size(cfilt) == maxfilt, 'SIZE(CFILT) = MAXFILT', s&
     &rname)
! Check the values of XFILT, FFILT, CONFILT, CFILT.
          call assert(.not. any(is_nan(xfilt(:, 1:nfilt))), 'XFILT does &
     &not contain NaN', srname)
...
! Check that no point in the filter is better than X, and X is better than no point.
          call assert(.not. any([(isbetter([ffilt(i), cfilt(i)], [f, cst&
     &rv], ctol), i=1, nfilt)]), 'No point in the filter is better than &
     &X', srname)
          call assert(.not. any([(isbetter([f, cstrv], [ffilt(i), cfilt(&
     &i)], ctol), i=1, nfilt)]), 'X is better than no point in the filte&
     &r', srname)
      end if
...
!--------------------------------------------------------------------------------------------------!
      ...
      use, non_intrinsic :: consts_mod, only : IK, RP, HUGENUM, HUGEFUN,&
     & HUGECON, ZERO, TWO, DEBUGGING
...
! Preconditions
      if (DEBUGGING) then
          ...
          call assert(size(chist) == nhist, 'SIZE(FHIST) == SIZE(CHIST)'&
     &, srname)
          call assert(.not. any(is_nan(fhist) .or. is_posinf(fhist)), 'F&
     &HIST does not contain NaN/+Inf', srname)
          call assert(.not. any(chist < 0 .or. is_nan(chist) .or. is_pos&
     &inf(chist)), 'CHIST does not contain nonnegative values or NaN/+In&
     &f', srname)
...
! We select X among the points whose shifted constraint violations are at most CREF.
          cref = TWO * cmin
          ! CREF = 0 if CMIN = 0; thus asking for CSTRV_SHIFTED < CREF is WRONG!
...
! 2. In finite-precision arithmetic, PHI_1 == PHI_2 and CSTRV_SHIFTED_1 == CSTRV_SHIFTED_2 do
! not ensure that F_1 == F_2!!!
          phimin = minval(phi, mask=(fhist < fref .and. chist_shifted <=&
     & cref))
          cref = minval(chist_shifted, mask=(fhist < fref .and. phi <= p&
     &himin))
          fref = minval(fhist, mask=(chist_shifted <= cref))
          kopt = int(minloc(chist, mask=(fhist <= fref), dim=1), kind(ko&
     &pt))
      end if
...
! Postconditions
      if (DEBUGGING) then
          call assert(kopt >= 1 .and. kopt <= nhist, '1 <= KOPT <= SIZE(&
     &FHIST)', srname)
          call assert(.not. any([(isbetter([fhist(i), chist(i)], [fhist(&
     &kopt), chist(kopt)], ctol), i=1, nhist)]), 'No point in the histor&
     &y is better than X', srname)
      end if
...
!--------------------------------------------------------------------------------------------------!
      ...
      use, non_intrinsic :: consts_mod, only : RP, TEN, EPS, HUGECON, HU&
     &GENUM, DEBUGGING
      use, non_intrinsic :: infnan_mod, only : is_nan, is_posinf
      use, non_intrinsic :: debug_mod, only : assert
      implicit none
...
! Preconditions
      if (DEBUGGING) then
          call assert(size(fc1) == 2 .and. size(fc2) == 2, 'SIZE(FC1) ==&
     & 2 == SIZE(FC2)', srname)
          call assert(.not. any(is_nan(fc1) .or. is_posinf(fc1)), 'FC1 d&
     &oes not contain NaN/+Inf', srname)
          call assert(.not. any(is_nan(fc2) .or. is_posinf(fc2)), 'FC2 d&
     &oes not contain NaN/+Inf', srname)
          call assert(fc1(2) >= 0 .and. fc2(2) >= 0, 'FC1(2) >= 0, FC(2)&
     & >= 0', srname)
          call assert(ctol >= 0, 'CTOL >= 0', srname)
      end if
...
! security and robustness, the code below does not make this assumption.
      is_better = is_better .or. (any(is_nan(fc2)) .and. .not. any(is_na&
     &n(fc2)))
...
! is indeed CREF. However, this may not be true if CTOL > 1E-1*HUGECON.
      cref = TEN * max(EPS, min(ctol, 1.0E-2_RP * HUGECON))
      ! The MIN avoids overflow.
      is_better = is_better .or. (fc1(1) < HUGENUM .and. fc1(2) <= ctol &
     &.and. (fc2(2) > max(ctol, cref) .or. is_nan(fc2(2))))
...
! Postconditions
      if (DEBUGGING) then
          call assert(.not. (is_better .and. fc1(1) >= fc2(1) .and. fc1(&
     &2) >= fc2(2)), 'FC1 >= FC2 and IS_BETTER cannot be both true', srn&
     &ame)
          call assert(is_better .or. .not. (fc1(1) <= fc2(1) .and. fc1(2&
     &) < fc2(2)), 'if FC1 <= FC2 but not equal, then IS_BETTER must be &
     &true', srname)
          call assert(is_better .or. .not. (fc1(1) < fc2(1) .and. fc1(2)&
     & <= fc2(2)), 'if FC1 <= FC2 but not equal, then IS_BETTER must be &
     &true', srname)
