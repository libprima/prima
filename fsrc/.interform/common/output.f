...
! Format for NF at return.
      character(len=*), parameter :: retnf_fmt = '(1A, '//spaces//', 1A,&
     & '//ifmt//')'
...
! Format for RHO and NF when RHO is updated, with or without CPEN.
      character(len=*), parameter :: rnf_fmt = '(/1A, '//rfmt//', '//spa&
     &ces//', /1A, '//ifmt//')'
      character(len=*), parameter :: rpnf_fmt = '(/1A, '//rfmt//', '//sp&
     &aces//', 1A, '//rfmt//', '//spaces//', /1A, '//ifmt//')'
...
!--------------------------------------------------------------------------------------------------!
      use, non_intrinsic :: consts_mod, only : RP, IK, ZERO, MSGLEN, FNA&
     &MELEN, OUTUNIT, STDOUT, DEBUGGING
      use, non_intrinsic :: debug_mod, only : assert, warning
      use, non_intrinsic :: info_mod, only : FTARGET_ACHIEVED, MAXFUN_RE&
     &ACHED, MAXTR_REACHED, SMALL_TR_RADIUS
      use, non_intrinsic :: info_mod, only : TRSUBP_FAILED, NAN_INF_X, N&
     &AN_INF_F, NAN_MODEL, DAMAGING_ROUNDING
      implicit none
...
! Local variables
      ...
      integer :: iostat
      ! IO status of the writing. Should be an integer of default kind.
      integer :: wunit
      ! Logical unit for the writing. Should be an integer of default kind.
      integer(IK), parameter :: valid_exit_flags(9) = [FTARGET_ACHIEVED,&
     & MAXFUN_REACHED, MAXTR_REACHED, SMALL_TR_RADIUS, TRSUBP_FAILED, NA&
     &N_INF_F, NAN_INF_X, NAN_MODEL, DAMAGING_ROUNDING]
      logical :: fexist
...
! Preconditions
      if (DEBUGGING) then
          call assert(any(info == valid_exit_flags), 'The exit flag is v&
     &alid', srname)
      end if
...
!====================!
      ...
      ! Print the message to a file named FOUT with the writing unit being OUTUNIT.
          wunit = OUTUNIT
          fout = trim(solver)//'_output.txt'
          inquire (file=fout, exist=fexist)
          fstat = merge(tsource='old', fsource='new', mask=fexist)
          open (unit=wunit, file=fout, status=fstat, position='append', &
...
! Decide the exit message.
     ...
     &'
      case (MAXTR_REACHED)
          msg = 'the maximal number of trust region iterations has been &
     &reached.'
      case (SMALL_TR_RADIUS)
          msg = 'the trust region radius reaches its lower bound.'
      case (TRSUBP_FAILED)
          msg = 'a trust region step has failed to reduce the quadratic &
     &model.'
      case (NAN_INF_X)
...
! Print the message.
      ...
      write (wunit, '(/1A)') 'Return from '//solver//' because '//trim(m&
     &sg)
      write (wunit, retnf_fmt) 'At the return from '//solver, 'Number of&
...
!====================!
      ...
      subroutine rhomsg(solver, iprint, nf, f, rho, x, cstrv, constr, cp&
     &en)
!--------------------------------------------------------------------------------------------------!
! This subroutine prints messages when RHO is updated.
!--------------------------------------------------------------------------------------------------!
      use, non_intrinsic :: consts_mod, only : RP, IK, ZERO, OUTUNIT, ST&
     &DOUT, FNAMELEN
...
! Print the message.
      ...
      if (present(cpen)) then
          write (wunit, rpnf_fmt) 'New RHO = ', rho, ' CPEN = ', cpen, '&
     &Number of function evaluations = ', nf
      else
          write (wunit, rnf_fmt) 'New RHO = ', rho, 'Number of function &
     &evaluations = ', nf
      end if
      write (wunit, f_fmt) 'Least function value = ', f
      if (is_constrained) then
...
!--------------------------------------------------------------------------------------------------!
      use, non_intrinsic :: consts_mod, only : RP, IK, FNAMELEN, OUTUNIT&
     &, STDOUT
      use, non_intrinsic :: debug_mod, only : warning
      implicit none
