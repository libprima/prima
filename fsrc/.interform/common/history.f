!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This is the intersection-form version of history.f90.
...
! Started: July 2020
!
! Last Modified: Wednesday, February 09, 2022 AM12:32:57
!--------------------------------------------------------------------------------------------------!
      ...
      subroutine prehist(maxhist, n, output_xhist, xhist, output_fhist, &
     &fhist, output_chist, chist, m, output_conhist, conhist)
!--------------------------------------------------------------------------------------------------!
...
!--------------------------------------------------------------------------------------------------!
      use, non_intrinsic :: consts_mod, only : RP, IK, MAXMEMORY, DEBUGG&
     &ING
...
! Preconditions
      if (DEBUGGING) then
          ...
          call assert(present(output_chist) .eqv. present(chist), 'OUTPU&
     &T_CHIST and CHIST are both present or both absent', srname)
          call assert((present(m) .eqv. present(conhist)) .and. (present&
     &(output_conhist) .eqv. present(conhist)), 'M, OUTPUT_CONHIST, and &
     &CONHIST are all present or all absent', srname)
      end if
...
! Revise MAXHIST according to MAXMEMORY, i.e., the maximal memory allowed for the history.
      ...
      if (present(m) .and. present(output_conhist) .and. present(conhist&
     &)) then
...
! Postconditions
      if (DEBUGGING) then
          call assert(maxhist >= 0 .and. maxhist <= maxhist_in, '0 <= MA&
     &XHIST <= MAXHIST_IN', srname)
          call assert(int(maxhist, kind(MAXMEMORY)) * int(unit_memo, kin&
     &d(MAXMEMORY)) <= MAXMEMORY, 'The history will not take more memory&
     & than MAXMEMORY', srname)
          call assert(allocated(xhist), 'XHIST is allocated', srname)
          call assert(size(xhist, 1) == n .and. size(xhist, 2) == maxhis&
     &t * int(output_xhist), 'if XHIST is requested, then SIZE(XHIST) ==&
     & [N, MAXHIST]; otherwise, SIZE(XHIST) == [N, 0]', srname)
          call assert(allocated(fhist), 'FHIST is allocated', srname)
          call assert(size(fhist) == maxhist * int(output_fhist), 'if FH&
     &IST is requested, then SIZE(FHIST) == MAXHIST; otherwise, SIZE(FHI&
     &ST) == 0', srname)
          if (present(output_chist) .and. present(chist)) then
              call assert(allocated(chist), 'CHIST is allocated', srname&
     &)
              call assert(size(chist) == maxhist * int(output_chist), 'i&
     &f CHIST is requested, then SIZE(CHIST) == MAXHIST; otherwise, SIZE&
     &(CHIST) == 0', srname)
          end if
          if (present(m) .and. present(output_conhist) .and. present(con&
     &hist)) then
              call assert(allocated(conhist), 'CONHIST is allocated', sr&
     &name)
              call assert(size(conhist, 1) == m .and. size(conhist, 2) =&
     &= maxhist * int(output_conhist), 'if CONHIST is requested, then SI&
     &ZE(CONHIST) == [M, MAXHIST]; otherwise, SIZE(CONHIST) == [M, 0]', &
     &srname)
          end if
      end if
      ...
      subroutine savehist(nf, x, xhist, f, fhist, cstrv, chist, constr, &
     &conhist)
!--------------------------------------------------------------------------------------------------!
...
!--------------------------------------------------------------------------------------------------!
      ...
      use, non_intrinsic :: debug_mod, only : assert
      use, non_intrinsic :: infnan_mod, only : is_nan, is_finite, is_pos&
     &inf, is_neginf
      implicit none
...
! Check the presence of CSTRV, CHIST, CONSTR, CONHIST.
          call assert(present(cstrv) .eqv. present(chist), 'CSTRV and CH&
     &IST are both present or both absent', srname)
          call assert(present(constr) .eqv. present(conhist), 'CONSTR an&
     &d CONHIST are both present or both absent', srname)
! Check the size of X.
          call assert(size(x) >= 1, 'SIZE(X) >= 1', srname)
! Check the sizes of XHIST, FHIST, CONHIST, CHIST.
          call assert(size(xhist, 1) == size(x) .and. maxxhist * (maxxhi&
     &st - maxhist) == 0, 'SIZE(XHIST, 1) == SIZE(X), SIZE(XHIST, 2) == &
     &0 or MAXHIST', srname)
     ...
     & == 0 or MAXHIST', srname)
          if (present(constr) .and. present(conhist)) then
              call assert(size(conhist, 1) == size(constr) .and. maxconh&
     &ist * (maxconhist - maxhist) == 0, 'SIZE(CONHIST, 1) == SIZE(CONST&
     &R), SIZE(CONHIST, 2) == 0 or MAXNHIST', srname)
          end if
...
! As long as this subroutine is called, XHIST contains only finite values.
          call assert(all(is_finite(xhist(:, 1:min(nf - 1_IK, maxxhist))&
     &)), 'XHIST is finite', srname)
          call assert(.not. any(is_nan(fhist(1:min(nf - 1_IK, maxfhist))&
     &) .or. is_posinf(fhist(1:min(nf - 1_IK, maxfhist)))), 'FHIST does &
     &not contain NaN/+Inf', srname)
!----------------------------------------------------------------------------------------------!
...
!----------------------------------------------------------------------------------------------!
          if (present(conhist)) then
              call assert(.not. any(is_nan(conhist(:, 1:min(nf - 1_IK, m&
     &axconhist))) .or. is_neginf(conhist(:, 1:min(nf - 1_IK, maxconhist&
     &)))), 'CONHIST does not contain NaN/-Inf', srname)
          end if
! Check the values of X, F, CSTRV, CONSTR.
! X does not contain NaN if X0 does not and the trust-region/geometry steps are proper.
          call assert(.not. any(is_nan(x)), 'X does not contain NaN', sr&
     &name)
...
! Postconditions
      if (DEBUGGING) then
      ! Called after each function evaluation when debugging; can be expensive.
          call assert(size(xhist, 1) == size(x) .and. size(xhist, 2) == &
     &maxxhist, 'SIZE(XHIST) == [SIZE(X), MAXXHIST]', srname)
...
!----------------------------------------------------------------------------------------------!
          if (present(conhist) .and. present(constr)) then
              call assert(size(conhist, 1) == size(constr) .and. size(co&
     &nhist, 2) == maxconhist, 'SIZE(CONHIST) == [SIZE(CONSTR), MAXCONHI&
     &ST]', srname)
...
! Check the sizes of XHIST, FHIST, CHIST, CONHIST.
          ...
          call assert(maxxhist * (maxxhist - maxhist) == 0, 'SIZE(XHIST,&
     & 2) == 0 or MAXHIST', srname)
          call assert(maxfhist * (maxfhist - maxhist) == 0, 'SIZE(FHIST)&
     & == 0 or MAXHIST', srname)
          call assert(maxchist * (maxchist - maxhist) == 0, 'SIZE(CHIST)&
     & == 0 or MAXHIST', srname)
          call assert(maxconhist * (maxconhist - maxhist) == 0, 'SIZE(CO&
     &NHIST, 2) == 0 or MAXHIST', srname)
...
! The ranging should be done only if 0 < MAXCONHIST < NF. Otherwise, it leads to errors/wrong results.
      if (maxconhist > 0 .and. maxconhist < nf) then
          ...
          conhist = reshape([conhist(:, khist + 1:maxconhist), conhist(:&
     &, 1:khist)], shape(conhist))
      end if
