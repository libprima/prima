!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This is the intersection-form version of evaluate.f90.
...
! Although this should not happen unless there is a bug, we include this case for security.
          ...
          ! Evaluate F; We moderate X before doing so.
...
! Set F, CONSTR, and CSTRV to NaN.
          ...
          call calcfc(moderatex(x), f, constr)
          ! Evaluate F and CONSTR; We moderate X before doing so.
...
! CONSTR cannot be NaN/-Inf.
     ...
     &'CONSTR does not containt NaN/-Inf', srname)
          call assert(.not. (cstrv < ZERO .or. is_nan(cstrv) .or. is_pos&
     &inf(cstrv)), 'CSTRV is nonnegative and not NaN/+Inf', srname)
      end if
