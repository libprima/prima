!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! This is the intersection-form version of circle.f90.
...
!====================!
      ...
      angle = unit_angle * (real(iopt, RP) + step)
      ! It may not be in [0, 2*PI].
...
!====================!
      ...
      iopt = int(maxloc(abs(fval), mask=(.not. is_nan(fval)), dim=1) - 1&
     &, IK)
      fopt = fval(iopt + 1)
      fa = fval(modulo(iopt - 1_IK, grid_size) + 1)
      fb = fval(modulo(iopt + 1_IK, grid_size) + 1)
