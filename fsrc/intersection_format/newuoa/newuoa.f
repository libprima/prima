! This is the intersection-format version of newuoa.f.
...
! and the NEWUOA paper.
            ...
            subroutine newuoa(x, f, rhobeg, rhoend, eta1, eta2, gamma1, &
     &gamma2, ftarget, npt, maxfun, iprint, info)
...
! the variables.
! The value of IPRINT should be set to 0, 1, 2, 3, or 4, which
...
! variables are output if IPRINT=3. When IPRINT=4, all the output
! of IPRINT=3 will be recorded in a file named NEWUOA.output,
! which can be costly in terms of time and space (the file will be
...
!
! FTARGET is the target function value. The minimization will
! terminate when a point with function value <= FTARGET is found.
!
! ETA1, ETA2, GAMMA1, and GAMMA2 are parameters not included in
! Powell's original interface. Roughly speaking, the trust region
! radius will be contracted by a factor of GAMMA1 when the
! reduction ratio is below ETA1, and it will be elarged by
! a factor of GAMMA2 when the reduction ratio is above ETA2.
! Powell set ETA1 = 0.1, ETA2 = 0.7, GAMMA1 = 0.5, GAMMA2 = 2.
! See the TRRAD function in trustregion.f for details.
!
...
! TRSUBP_FAILED: a trust region step has failed to reduce the
! uadratic model.
! MAXFUN_REACHED: the objective function has been evaluated
! MAXFUN times.
! NAN_X: NaN occurs in x.
! NAN_INF_F: the objective function returns NaN or nearly infinite
! value.
...
! X(1 : N).
            ...
            use consts_mod, only : RP, IK,ZERO, ONE, TWO, HALF, TENTH, E&
     &PS
...
! Inputs
            ...
            real(RP), intent(in) :: eta2 ! Threshold for increasing DELT&
     &A
...
! Intermediate variables
            integer(IK) :: n, nf
            integer(IK) :: maxfun_v, npt_v, iprint_v
            real(RP) :: rhobeg_v, rhoend_v
            real(RP) :: eta1_v, eta2_v
            real(RP) :: gamma1_v, gamma2_v
            real(RP) :: ftarget_v
...
! Verify and possibly revise the inputs. RHOBEG_V is the value of
! RHOBEG after verification. The others are similar.
            rhobeg_v = rhobeg
            rhoend_v = rhoend
            eta1_v = eta1
            eta2_v = eta2
...
! and RHOEND to equal when their difference is tiny.
            if ((rhobeg_v-rhoend_v) < 1.0e2_RP*EPS*max(abs(rhobeg_v),ONE&
     &))then
                rhoend_v = rhobeg_v
            end if

            if (rhobeg_v <= 0 .or. is_nan(rhobeg_v) .or. is_inf(rhobeg_v&
     &))then
                rhobeg_v = RHOBEG_DEF
            end if
     ...
     &) .or. is_inf(rhoend_v)) then
            ...
            if (eta1_v < 0.0_RP .or. eta1_v > HALF .or. is_nan(eta1_v)) &
     &then
                eta1_v = TENTH
            end if

            if (eta2_v < eta1_v .or. eta2_v > 1.0_RP .or. is_nan(eta2_v)&
     &) then
               eta2_v = min(1.0_RP, max(eta1_v, 0.7_RP))
            end if

            if (gamma1_v <= 0.0_RP .or. gamma1_v >= 1.0_RP .or. is_nan(g&
     &amma1_v)) then
                gamma1_v = HALF
            end if

            if (gamma2_v <= 1.0_RP .or. is_nan(gamma2_v) .or. is_inf(gam&
     &ma2_v)) then
                gamma2_v = TWO
            end if
            ...
            if (npt_v < n + 2 .or. npt > min(maxfun_v - 1, ((n + 2)*(n +&
     &1))/2)) then
                npt_v = int(min(maxfun_v - 1, 2*n + 1), kind(npt_v))
            end if

            if (iprint_v /= 0 .and. iprint_v /= 1 .and. iprint_v /= 2 .a&
     &nd. iprint_v /=3 .and. iprint_v /= 4) then
                iprint_v = IPRINT_DEF
            end if

            call newuob (iprint_v, maxfun_v, npt_v, eta1_v, eta2_v, ftar&
     &get_v, gamma1_v, gamma2_v, rhobeg_v, rhoend_v, x, nf, f, info)
