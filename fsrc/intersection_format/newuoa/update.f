...
! In-outputs
            ...
            real(RP), intent(inout) :: zmat(:, :) ! ZMAT(NPT, NPT - N - &
     &1)
...
! ZMAT(KNEW, J) becomes 0.
            ...
            if (idz <= npt - n - 1) then
                jl = idz ! For J = IDZ + 1, ..., NPT - N - 1, set JL = I&
     &DZ.
            end if
...
! W, and calculate the parameters of the updating formula.
            ...
            vlag(knew) = vlag(knew) - ONE ! VLAG = Hw-e_t in the NEWUOA &
     &paper.
...
! There are two nonzeros in ZMAT(KNEW, :) after the rotation.
     ...
     &))
                zmat(:, jb) = scalb*(zmat(:, jb) - tempa*w(1 : npt) - te&
     &mpb*vlag(1 : npt))
...
! Finally,  update the matrix BMAT.
            ...
            v1 = (alpha*vlag(npt+1 : npt+n) - tau*w(npt+1 : npt+n))/deno&
     &m
            v2 = (-beta*w(npt+1 : npt+n) - tau*vlag(npt+1 : npt+n))/deno&
     &m
...
! BMAT(:, NPT+1 : NPT+N) to be symmetric. Symmetrization needed.
            ...
            subroutine updateq(idz, knew, bmatknew, fqdiff, zmat, xptkne&
     &w, gq, hq, pq)
...
! Get and verify the sizes.
            ...
            if (DEBUGGING) then
                if (n == 0 .or. npt < n + 2) then
                    call errstop(srname, 'SIZE(GQ) or SIZE(PQ) is invali&
     &d')
                end if
                call verisize(zmat, npt, int(npt - n - 1, kind(n)))
...
! Update the gradient.
            ...
            subroutine tryqalt(idz, fval, ratio, smat, zmat, itest, gq, &
     &hq,pq)
