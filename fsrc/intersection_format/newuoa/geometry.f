! This is the intersection-format version of geometry.f.
...
! on 03-Aug-2020.
            ...
            subroutine setremove(idz, kopt, beta, delta, ratio, rho, vla&
     &g, xopt, xpt, zmat, knew)
...
! Get and verify the sizes
            ...
            hdiag = -sum(zmat(:, 1 : idz - 1)**2, dim = 2) + sum(zmat(:,&
     &idz : npt - n - 1)**2, dim = 2)
...
! FOPT), then KNEW > 0, ensuring XNEW to be included into XPT.
            ...
            subroutine ameliorgeo(idz, knew, kopt, bmat, delbar, xopt, x&
     &pt, zmat, d, beta, vlag)
...
! VLAG and BETA for this D are calculated within BIGDEN.
            if (abs(ONE + alpha*beta/vlag(knew)**2) <= 0.8_RP) then
                call bigden(idz, knew, kopt, bmat, xopt, xpt,zmat,d,beta&
     &,vlag)
            end if
...
! Calculate the new D and GD. Then test for convergence.
     ...
     &)*sth
            ...
            subroutine bigden(idz, knew, kopt, bmat, x, xpt, zmat,d,beta&
     &,vlag)
...
! General modules
            use consts_mod, only : RP, IK, ONE, TWO, HALF, QUART, PI, ZE&
     &RO
            use consts_mod, only : DEBUGGING, SRNLEN
            use debug_mod, only : errstop, verisize
            use lina_mod, only : Ax_plus_y
...
! Intermediate variable
            ...
            real(RP) :: alpha, angle, unitang, dd, denmax, denold, densa&
     &v
            real(RP) :: ds, dtest, ss, ssden, denom, denomold, dena, den&
     &b
...
!----------------------------------------------------------------------!
                ...
                if ((.not. (dstemp(k)*dstemp(k)/sstemp(k) >= dtest)) .an&
     &d. k /= kopt) then
...
! Put the coefficents of THETA*WCHECK in PROD.
                do jc = 1, 5
                    ...
                    if (jc == 2 .or. jc == 3) then
                        prod(1 : npt, jc) = prod(1 : npt, jc) + matprod(&
     &w(npt + 1 : npt + n, jc), bmat(:, 1 : npt))
                        nw = npt + n
                    end if
...
! Include in DEN the part of BETA that depends on THETA.
                ...
                denex(5) = alpha*den(5) + tempa + prod(knew, 2)*prod(kne&
     &w, 3)
...
! Seek the value of the angle that maximizes the |DENOM|.
                denom = denex(1) + denex(2) + denex(4) + denex(6) + dene&
     &x(8)
                denold = denom
                denmax = denom
                isave = 0
                ...
                do i = 1, iu
                    ...
                    do j = 4, 8, 2
                        ...
                        par(j + 1) = par(2)*par(j - 1) + par(3)*par(j - &
     &2)
                    end do
                    denomold = denom
...
! to D.
                s = tau*bmat(:,knew) + alpha*(dxn*x+xnsq*d-vlag(npt+1:np&
     &t+n))
                v = matprod(xnew, xpt)
