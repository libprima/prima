! This is the intersection-format version of prelim.f.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &*),
      ...
      C
      C The arguments N, NPT, X, XL, XU, RHOBEG, IPRINT and MAXFUN are t&
     &he
      C same as the corresponding arguments in SUBROUTINE BOBYQA.
      C The arguments XBASE, XPT, FVAL, HQ, PQ, BMAT, ZMAT, NDIM, SL and&
     &SU
      C are the same as the corresponding arguments in BOBYQB, the eleme&
     &nts
      C of SL and SU being set in BOBYQA.
      C GOPT is usually the gradient of the quadratic model at XOPT+XBAS&
     &E, but
      C it is set by PRELIM to the gradient of the quadratic model at XB&
     &ASE.
      C If XOPT is nonzero, BOBYQB will change it to its usual value lat&
     &er.
      C NF is maintaned as the number of calls of CALFUN so far.
      C KOPT will be such that the least calculated value of F so far is&
     &at
      C the point XPT(KOPT,.)+XBASE in the space of the variables.
      C
      C SUBROUTINE PRELIM sets the elements of XBASE, XPT, FVAL, GOPT, H&
     &Q, PQ,
      C BMAT and ZMAT for the first iteration, and it maintains the valu&
     &es of
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C Set XBASE to the initial vector of variables, and set the initia&
     &l
      ...
      C
            ...
            END DO
      C
      C Begin the initialization procedure. NF becomes one more than the&
     &number
      C of function values so far. The coordinates of the displacement o&
     &f the
     ...
     &)
            ...
            END IF
      C
      C Calculate the next value of F. The least function value so far a&
     &nd
      ...
      C
         ...
         70 FORMAT (/4X,'Function number',I6,' F =',1PD18.10,
           1 ' The corresponding X is:'/(2X,5D15.6))
            ...
            END IF
      C
      C Set the nonzero initial elements of BMAT and the quadratic model&
     &in the
      C cases when NF is at most 2*N+1. If NF exceeds N+1, then the posi&
     &tions
      C of the NF-th and (NF-N)-th interpolation points may be switched,&
     &in
      C order that the function value at the first of them contributes t&
     &o the
      C off-diagonal second derivative terms of the initial quadratic mo&
     &del.
      C
            IF (NF <= 2*N+1) THEN
                ...
                END IF
      C
      C Set the off-diagonal second derivatives of the Lagrange function&
     &s and
