! This is the intersection-format version of bobyqa.f.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &ables,
      ...
      C interpolation. There is usually some freedom in the interpolatio&
     &n
      C conditions, which is taken up by minimizing the Frobenius norm o&
     &f
      C the change to the second derivative of the model, beginning with&
     &the
      C zero matrix. The values of the variables are constrained by uppe&
     &r and
     ...
     &o.
      C NPT is the number of interpolation conditions. Its value must be&
     &in
      C the interval [N+2,(N+1)(N+2)/2]. Choices that exceed 2*N+1 are n&
     &ot
      C recommended.
     ...
     &). They
      ...
      C For I=1,2,...,N, XL(I) and XU(I) must provide the lower and uppe&
     &r
      C bounds, respectively, on X(I). The construction of quadratic mod&
     &els
      C requires XL(I) to be strictly less than XU(I) for each I. Furthe&
     &r,
     ...
     &a trust
      C region radius, so both must be positive with RHOEND no greater t&
     &han
      C RHOBEG. Typically, RHOBEG should be about one tenth of the great&
     &est
     ...
     &An
      C error return occurs if any of the differences XU(I)-XL(I), I=1,.&
     &..,N,
     ...
     &CCCCC
      ...
      C -1: NaN occurs in x.
      C -2: the objective function returns a NaN or nearly infinite valu&
     &e.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &t set
      C F to the value of the objective function for the current values &
     &of the
      C variables X(1),X(2),...,X(N), which are generated automatically &
     &in a
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &can
      C be treated separately during the calculation of BOBYQB. The part&
     &ition
      C requires the first (NPT+2)*(NPT+N)+3*N*(N+5)/2 elements of W plu&
     &s the
      C space that is taken by the last array in the argument list of BO&
     &BYQB.
      C
            NDIM=NPT+N
            IXB=1
            ...
            IW=IVL+NDIM
      C
      C Return if there is insufficient space between the bounds. Modify&
     &the
      C initial X if necessary in order to avoid conflicts between the b&
     &ounds
      C and the construction of the first quadratic model. The lower and&
     &upper
      C bounds on moves from the updated X are set now, in the ISL and I&
     &SU
      C partitions of W, in order to provide useful and exact informatio&
     &n about
      C components of X that become within distance RHOBEG from their bo&
     &unds.
      C
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
     &CCCCC
     ...
     &RHOBEG,
      C XU and XL may change a bit (due to rounding ???). It was oberved&
     &in
      C a MATLAB test that MEX passed 1 to Fortran as 0.9999999999999997&
     &8.
      C If we set RHOBEG = MIN(XU-XL)/2 in the interfaces, then it may h&
     &appen
      C that RHOBEG > MIN(XU-XL)/2. That is why we do the following. Aft&
     &er
      C this, INFO=6 should never occur.
            RHOBEG = MIN(0.5D0*(1.0D0-1.0D-5)*MINVAL(XU(1:N)-XL(1:N)),RH&
     &OBEG)
