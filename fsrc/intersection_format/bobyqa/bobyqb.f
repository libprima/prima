! This is the intersection-format version of bobyqb.f.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
      C The arguments N, NPT, X, XL, XU, RHOBEG, RHOEND, IPRINT and MAXF&
     &UN
      C are identical to the corresponding arguments in SUBROUTINE BOBYQ&
     &A.
      C XBASE holds a shift of origin that should reduce the contributio&
     &ns
      C from rounding errors to values of the model and Lagrange functio&
     &ns.
      ...
      C XOPT is set to the displacement from XBASE of the trust region c&
     &entre.
      ...
      C PQ contains the parameters of the implicit second derivatives of&
     &the
      C quadratic model.
      C BMAT holds the last N columns of H.
      C ZMAT holds the factorization of the leading NPT by NPT submatrix&
     &of H,
      C this factorization being ZMAT times ZMAT^T, which provides both &
     &the
      C correct rank and positive semi-definiteness.
      C NDIM is the first dimension of BMAT and has the value NPT+N.
      C SL and SU hold the differences XL-XBASE and XU-XBASE, respective&
     &ly.
      C All the components of every XOPT are going to satisfy the bounds
      C SL(I) .LEQ. XOPT(I) .LEQ. SU(I), with appropriate equalities whe&
     &n
      C XOPT is on a constraint boundary.
      C XNEW is chosen by SUBROUTINE TRSBOX or ALTMOV. Usually XBASE+XNE&
     &W is the
      C vector of variables for the next call of CALFUN. XNEW also satis&
     &fies
      C the SL and SU constraints in the way that has just been mentione&
     &d.
      C XALT is an alternative to XNEW, chosen by ALTMOV, that may repla&
     &ce XNEW
      C in order to increase the denominator in the updating of UPDATE.
      C D is reserved for a trial step from XOPT, which is usually XNEW-&
     &XOPT.
      C VLAG contains the values of the Lagrange functions at a new poin&
     &t X.
      C They are part of a product that requires VLAG to be of length ND&
     &IM.
      C W is a one-dimensional array that is used for working space. Its&
     &length
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C The call of PRELIM sets the elements of XBASE, XPT, FVAL, GOPT, &
     &HQ, PQ,
      C BMAT and ZMAT for the first iteration, with the corresponding va&
     &lues of
      C of NF and KOPT, which are the number of calls of CALFUN so far a&
     &nd the
      C index of the interpolation point at the trust region centre. The&
     &n the
      C initial XOPT is set too. The branch to label 720 occurs if MAXFU&
     &N is
      C less than NPT. GOPT will be updated if KOPT is different from KB&
     &ASE.
      C
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
     &CCCCC
      C Zaikun 2019-08-29: FSAVE is not needed any more. See line number&
     &720.
      C FSAVE=FVAL(1)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            KBASE=1
      C
      C Complete the settings that are required for the iterative proced&
     &ure.
      C
            RHO=RHOBEG
            DELTA=RHO
            ...
            NFSAV=NF
      C
      C Update GOPT if necessary before the first iteration and after ea&
     &ch
      ...
      C
         20 IF (KOPT /= KBASE) THEN
            ...
            END IF
      C
      C Generate the next point in the trust region that provides a smal&
     &l value
      C of the quadratic model subject to the constraints on the variabl&
     &es.
      C The integer NTRITS is set to the number "trust region" iteration&
     &s that
      C have occurred since the last "alternative" iteration. If the len&
     &gth
      C of XNEW-XOPT is less than HALF*RHO, however, then there is a bra&
     &nch to
      C label 650 or 680 with NTRITS=-1, instead of calculating F at XNE&
     &W.
      C
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
     &CCCCC
     ...
     &havior
      C of TRBOX, ALTMOV, or RESCUE is not predictable, and Segmentation&
     &Fault or
      C infinite cycling may happen. This is because any equality/inequa&
     &lity
      C comparison involving NaN returns FALSE, which can lead to uninte&
     &nded
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
      C The following choice between labels 650 and 680 depends on wheth&
     &er or
      C not our work with the current RHO seems to be complete. Either R&
     &HO is
      C decreased or termination occurs if the errors in the quadratic m&
     &odel at
      C the last three interpolation points compare favourably with pred&
     &ictions
      C of likely improvements to the model within distance HALF*RHO of &
     &XOPT.
      C
           ...
           1 GOTO 650
            ...
            NTRITS=NTRITS+1
      C
      C Severe cancellation is likely to occur if XOPT is too far from X&
     &BASE.
      C If the following test holds, then XBASE is shifted so that XOPT &
     &becomes
      C zero. The appropriate changes are made to BMAT and to the second
      C derivatives of the current model, beginning with the changes to &
     &BMAT
      C that do not depend on ZMAT. VLAG is used temporarily for working&
     &space.
      C
         90 IF (DSQ <= 1.0D-3*XOPTSQ) THEN
                ...
                DO K=1,NPT
                    ...
                    DO I=1,N
                        ...
                        IP=NPT+I
                        DO J=1,I
                            BMAT(IP,J)=BMAT(IP,J)+W(I)*VLAG(J)+VLAG(I)*W&
     &(J)
                        END DO
                    END DO
                END DO
      ...
      C
                ...
                END DO
      C
      C The following instructions complete the shift, including the cha&
     &nges
      ...
      C
            ...
            GOTO 230
      C
      C XBASE is also moved to XOPT by a call of RESCUE. This calculatio&
     &n is
      C more expensive than the previous shift, because new matrices BMA&
     &T and
      C ZMAT are generated from scratch, which may include the replaceme&
     &nt of
      C interpolation points whose positions seem to be causing near lin&
     &ear
      C dependence in the interpolation conditions. Therefore RESCUE is &
     &called
      C only if rounding errors have reduced by at least a factor of two&
     &the
      C denominator of the formula for updating the H matrix. It provide&
     &s a
      C useful safeguard, but is not invoked in most applications of BOB&
     &YQA.
      C
        190 NFSAV=NF
            KBASE=KOPT
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C XOPT is updated now in case the branch below to label 720 is tak&
     &en.
      C Any updating of GOPT occurs after the branch below to label 20, &
     &which
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            IF (NTRITS > 0) GOTO 60
      C
      C Pick two alternative vectors of variables, relative to XBASE, th&
     &at
      C are suitable as new positions of the KNEW-th interpolation point&
     &.
      C Firstly, XNEW is set to the point on a line through XOPT and ano&
     &ther
      C interpolation point that minimizes the predicted value of the ne&
     &xt
      C denominator, subject to ||XNEW - XOPT|| .LEQ. ADELT and to the S&
     &L
      C and SU bounds. Secondly, XALT is set to the best feasible point &
     &on
      ...
      C being returned in CAUCHY. The choice between these alternatives &
     &is
     ...
     &CCCCC
      ...
      C
      C Although very rare, NaN can sometimes occur in BMAT or ZMAT. If &
     &it
      C happens, we terminate the code. See the comments above line numb&
     &er 60.
      C Indeed, if ALTMOV is called with such matrices, then altmov.f wi&
     &ll
      C encounter a memory error at lines 173--174. This is because the &
     &first
      C value of PREDSQ in ALTOMOV (see line 159 of altmov.f) will be Na&
     &N, line
      C 164 will not be reached, and hence no value will be assigned to &
     &IBDSAV.
      C
      C Such an error was observed when BOBYQA was (mistakenly) tested o&
     &n CUTEst
      ...
      C neglecting all the constraints other than bounds. With only the &
     &bound
      C constraints, the objective function turned to be unbounded from
      C below, which led to abnormal values in BMAT (indeed, BETA define&
     &d in
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C the step D of ALTMOV by a Cauchy step. Then RESCUE may be called&
     &if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                END IF
      C
      C Alternatively, if NTRITS is positive, then set KNEW to the index&
     &of
      C the next interpolation point to be deleted to make room for a tr&
     &ust
      C region step. Again RESCUE may be called if rounding errors have &
     &damaged
      C the chosen denominator, which is the reason for attempting to se&
     &lect
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END IF
      C
      C Put the variables for the next calculation of the objective func&
     &tion
      C in XNEW, with any adjustments for the bounds.
      C
      C
      C Calculate the value of the objective function at XBASE+XNEW, unl&
     &ess
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END IF
      C
      C Use the quadratic model to predict the change in F due to the st&
     &ep D,
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
            ...
            END IF
      C
      C Update BMAT and ZMAT, so that the KNEW-th interpolation point ca&
     &n be
      ...
      C
            ...
            END DO
      C
      C Include the new interpolation point, and make the changes to GOP&
     &T at
      C the old XOPT that are caused by the updating of the quadratic mo&
     &del.
      C
            FVAL(KNEW)=F
            DO I=1,N
            ...
            END DO
      C
      C Update XOPT, GOPT and KOPT if the new calculated F is less than &
     &FOPT.
      C
            IF (F < FOPT) THEN
                KOPT=KNEW
            ...
            END IF
      C
      C Calculate the parameters of the least Frobenius norm interpolant&
     &to
      C the current data, the gradient of this interpolant at XOPT being&
     &put
      ...
      C
            IF (NTRITS > 0) THEN
                ...
                END DO
      C
      C Test whether to replace the new quadratic model by the least Fro&
     &benius
      C norm interpolant, making the replacement if the test is satisfie&
     &d.
      C
                ITEST=ITEST+1
            ...
            END IF
      C
      C If a trust region step has provided a sufficient decrease in F, &
     &then
      C branch for another trust region calculation. The case NTRITS=0 o&
     &ccurs
      C when the new interpolation point was reached by an alternative s&
     &tep.
      C
            IF (NTRITS == 0) GOTO 60
            IF (F <= FOPT+TENTH*VQUAD) GOTO 60
      C
      C Alternatively, find out if the interpolation points are close en&
     &ough
      ...
      C
        ...
        650 KNEW=0
            ...
            END DO
      C
      C If KNEW is positive, then ALTMOV finds alternative new positions&
     &for
      C the KNEW-th interpolation point within distance ADELT of XOPT. I&
     &t is
      C reached via label 90. Otherwise, there is a branch to label 60 f&
     &or
      ...
      C
      C The calculations with the current value of RHO are complete. Pic&
     &k the
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            END IF
      C
      C Return from the calculation, after another Newton-Raphson step, &
     &if
     ...
     &CCCCC
      ...
      C Why update X only when FVAL(KOPT) .LE. FSAVE? This seems INCORRE&
     &CT,
      C because it may lead to a return with F and X that are not the be&
     &st
      C available.
