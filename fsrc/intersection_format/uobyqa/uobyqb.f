! This is the intersection-format version of uobyqb.f.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C The arguments N, X, RHOBEG, RHOEND, IPRINT and MAXFUN are identi&
     &cal to
      C the corresponding arguments in SUBROUTINE UOBYQA.
      C NPT is set by UOBYQA to (N*N+3*N+2)/2 for the above dimension st&
     &atement.
      C XBASE will contain a shift of origin that reduces the contributi&
     &ons from
      ...
      C XPT will contain the interpolation point coordinates relative to&
     &XBASE.
      C PQ will contain the parameters of the quadratic model.
      C PL will contain the parameters of the Lagrange functions.
      C H will provide the second derivatives that TRSTEP and LAGMAX req&
     &uire.
      C G will provide the first derivatives that TRSTEP and LAGMAX requ&
     &ire.
      C D is reserved for trial steps from XOPT, except that it will con&
     &tain
      C diagonal second derivatives during the initialization procedure.
      C VLAG will contain the values of the Lagrange functions at a new &
     &point X.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &.
      C
            ...
            END DO
      C
      C The branch to label 120 obtains a new value of the objective fun&
     &ction
      C and then there is a branch back to label 50, because the new fun&
     &ction
      C value is needed to form the initial quadratic model. The least f&
     &unction
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
            IF (NF <= NNP) THEN
                ...
                IF (JSWITCH > 0) THEN
                    ...
                    END IF
      C
      C Pick the shift from XBASE to the next initial interpolation poin&
     &t
      ...
      C
                ...
                IF (NF < NNP) GOTO 30
      C
      C Form the off-diagonal second derivatives of the initial quadrati&
     &c model.
      C
                IH=N
                IP=1
     ...
     &t
      ...
      C
         ...
         60 TWORSQ=(TWO*RHO)**2
            RHOSQ=RHO*RHO
      C
      C Form the gradient of the quadratic model at the trust region cen&
     &tre.
      C
         70 KNEW=0
            IH=N
     ...
     &havior
      C of TRSTEM or LAGMAX is not predictable, and Segmentation Fault o&
     &r
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C Generate the next trust region step and test its length. Set KNE&
     &W
      C to -1 if the purpose of the next F will be to improve conditioni&
     &ng,
      C and also calculate a lower bound on the Hessian term of the mode&
     &l Q.
      C
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &ep D,
      ...
      C
      C Update SIXTHM, which is a lower bound on one sixth of the greate&
     &st
      ...
      C
      C Update FOPT and XOPT if the new F is the least value of the obje&
     &ctive
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
      C Set KNEW to the index of the next interpolation point to be dele&
     &ted.
      C
            KTEMP=0
            DETRAT=ZERO
            ...
            IF (KNEW == 0) GOTO 290
      C
      C Replace the interpolation point that has index KNEW by the point&
     &XNEW,
      ...
      C
        ...
        310 KNEW=-1
            ...
            END DO
      C
      C If a point is sufficiently far away, then set the gradient and H&
     &essian
      C of its Lagrange function at the centre of the trust region, and &
     &find
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C If ERRTOL is positive, test whether to replace the interpolation&
     &point
      C with index KNEW, using a bound on the maximum modulus of its Lag&
     &range
      ...
      C
                ...
                END IF
      C
      C If the KNEW-th point may be replaced, then pick a D that gives a&
     &large
      C value of the modulus of its Lagrange function within the trust r&
     &egion.
     ...
     &,
      C and make the corresponding changes to the gradients of the Lagra&
     &nge
