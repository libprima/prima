...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C N is the number of variables of a quadratic objective function, &
     &Q say.
      C G is the gradient of Q at the origin.
      C H is the Hessian matrix of Q. Only the upper triangular and diag&
     &onal
      C parts need be set. The lower triangular part is used to store th&
     &e
      ...
      C The arrays GG, TD, TN, W, PIV and Z will be used for working spa&
     &ce.
      C EVALUE will be set to the least eigenvalue of H if and only if D&
     &is a
      C Newton-Raphson step. Then EVALUE will be positive, but otherwise&
     &it
      C will be set to zero.
      C
      C Let MAXRED be the maximum of Q(0)-Q(D) subject to ||D|| .LEQ. DE&
     &LTA,
      C and let ACTRED be the value of Q(0)-Q(D) that is actually calcul&
     &ated.
      C We take the view that any D is acceptable if it has the properti&
     &es
      C
      C ||D|| .LEQ. DELTA and ACTRED .LEQ. (1-TOL)*MAXRED.
      C
      C The calculation of D is done by the method of Section 2 of the p&
     &aper
      C by MJDP in the 1997 Dundee Numerical Analysis Conference Proceed&
     &ings,
      ...
      C
            ...
            END DO
      C
      C Apply Householder transformations to obtain a tridiagonal matrix&
     &that
      C is similar to H, and put the elements of the Householder vectors&
     &in
      C the lower triangular part of H. Further, TD and TN will contain &
     &the
                ...
                ELSE
                    ...
                    DO J=KP,N
                        ...
                        IF (J < N) THEN
                            JP=J+1
                            DO I=JP,N
                                H(I,J)=H(I,J)-W(I)*Z(J)-W(J)*(Z(I)-WZ*W(&
     &I))
                            END DO
                        END IF
                    END DO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &CCCC
      C Zaikun 26-06-2019
      C The original code can encounter infinite cycling, which did happ&
     &en
      C when testing the CUTEst problems GAUSS1LS, GAUSS2LS, and GAUSS3L&
     &S.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
      C Branch if all the pivots are positive, allowing for the case whe&
     &n
      C G is zero.
      C
            ...
            K=KSAV
      C
      C Set D to a direction of nonpositive curvature of the given tridi&
     &agonal
      ...
      C
        ...
        170 IF (K > 1) THEN
            ...
            PARLEST=PAR-DHD/DSQ
      C
      C Terminate with D set to a multiple of the current D if the follo&
     &wing
      ...
      C
      C Make the usual test for acceptability of a full trust region ste&
     &p.
      C
            DNORM=DSQRT(DSQ)
      ...
      C
            ...
            END IF
      C
      C If required, calculate Z for the alternative test for convergenc&
     &e.
      C
            IF (POSDEF == ZERO) THEN
                W(1)=ONE/PIV(1)
      ...
      C
            ...
            GOTO 220
      C
      C Set EVALUE to the least eigenvalue of the second derivative matr&
     &ix if
      C D is a Newton-Raphson step. SHFMAX will be an upper bound on EVA&
     &LUE.
      C
        320 SHFMIN=ZERO
            PIVOT=TD(1)
            ...
            END DO
      C
      C Find EVALUE by a bisection method, but occasionally SHFMAX may b&
     &e
