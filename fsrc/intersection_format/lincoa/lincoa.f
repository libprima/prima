! This is the intersection-format version of lincoa.f.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &ables,
      C subject to general linear inequality constraints, by a trust reg&
     &ion
      C method that forms quadratic models by interpolation. Usually the&
     &re
      C is much freedom in each new model after satisfying the interpola&
     &tion
      C conditions, which is taken up by minimizing the Frobenius norm o&
     &f
      C the change to the second derivative matrix of the model. One new
      C function value is calculated on each iteration, usually at a poi&
     &nt
      C where the current model predicts a reduction in the least value &
     &so
      C far of the objective function subject to the linear constraints.
      C Alternatively, a new vector of variables may be chosen to replac&
     &e
      C an interpolation point that may be too far away for reliability,&
     &and
      C then the new point does not have to satisfy the linear constrain&
     &ts.
      C The arguments of the subroutine are as follows.
      C
      C N must be set to the number of variables and must be at least tw&
     &o.
      C NPT must be set to the number of interpolation conditions, which&
     &is
      C required to be in the interval [N+2,(N+1)(N+2)/2]. Typical choic&
     &es
      C of the author are NPT=N+6 and NPT=2*N+1. Larger values tend to b&
     &e
      C highly inefficent when the number of variables is substantial, d&
     &ue
      C to the amount of work and extra difficulty of adjusting more poi&
     &nts.
      C M must be set to the number of linear inequality constraints.
      C A is a matrix whose columns are the constraint gradients, which &
     &are
      C required to be nonzero.
      C IA is the first dimension of the array A, which must be at least&
     &N.
      C B is the vector of right hand sides of the constraints, the J-th
      C constraint being that the scalar product of A(.,J) with X(.) is &
     &at
      C most B(J). The initial vector X(.) is made feasible by increasin&
     &g
      C the value of B(J) if necessary.
      C X is the vector of variables. Initial values of X(1),X(2),...,X(&
     &N)
      C must be supplied. If they do not satisfy the constraints, then B
      C is increased as mentioned above. X contains on return the variab&
     &les
      C that have given the least calculated F subject to the constraint&
     &s.
      C RHOBEG and RHOEND must be set to the initial and final values of&
     &a
      C trust region radius, so both must be positive with RHOEND<=RHOBE&
     &G.
      C Typically, RHOBEG should be about one tenth of the greatest expe&
     &cted
      C change to a variable, and RHOEND should indicate the accuracy th&
     &at
     ...
     &and
      C there is output only at the return if IPRINT=1. Otherwise, the b&
     &est
      C feasible vector of variables so far and the corresponding value &
     &of
      C the objective function are printed whenever RHO is reduced, wher&
     &e
      C RHO is the current lower bound on the trust region radius. Furth&
     &er,
     ...
     &ALFUN,
      ...
      C W is an array used for working space. Its length must be at leas&
     &t
      C M*(2+N) + NPT*(4+N+NPT) + N*(9+3*N) + MAX [ M+3*N, 2*M+N, 2*NPT &
     &].
      C On return, W(1) is set to the final value of F, and W(2) is set &
     &to
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C SUBROUTINE CALFUN (N,X,F) has to be provided by the user. It mus&
     &t set
      C F to the value of the objective function for the variables X(1),
      C X(2),...,X(N). The value of the argument F is positive when CALF&
     &UN
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END IF
      C
      C Normalize the constraints, and copy the resultant constraint mat&
     &rix
      C and right hand sides into working space, after increasing the ri&
     &ght
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &ulation.
      C
            ...
            IPQW=IRF+(N*NP)/2
      C
      C The above settings provide a partition of W for subroutine LINCO&
     &B.
      C
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
     &CCCCC
