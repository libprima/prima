...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &),
      ...
      C
      C The arguments N, NPT, M, AMAT, B, X, RHOBEG, IPRINT, XBASE, XPT,&
     &FVAL,
      C XSAV, XOPT, GOPT, HQ, PQ, BMAT, ZMAT, NDIM, SP and RESCON are th&
     &e
      C same as the corresponding arguments in SUBROUTINE LINCOB.
      C KOPT is set to the integer such that XPT(KOPT,.) is the initial &
     &trust
      C region centre.
      C IDZ is going to be set to one, so that every element of Diag(DZ)&
     &is
      C one in the product ZMAT times Diag(DZ) times ZMAT^T, which is th&
     &e
      C factorization of the leading NPT by NPT submatrix of H.
      C STEP, PQW and W are used for working space, the arrays STEP and &
     &PQW
      C being taken from LINCOB. The length of W must be at least N+NPT.
      C
      C SUBROUTINE PRELIM provides the elements of XBASE, XPT, BMAT and &
     &ZMAT
      C for the first iteration, an important feature being that, if any&
     &of
      C of the columns of XPT is an infeasible point, then the largest o&
     &f
      C the constraint violations there is at least 0.2*RHOBEG. It also &
     &sets
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &],
      ...
      C
            ...
            END DO
      C
      C Set the remaining initial nonzero elements of XPT and ZMAT when &
     &the
      ...
      C
            ...
            END IF
      C
      C Update the constraint right hand sides to allow for the shift XB&
     &ASE.
      C
            IF (M > 0) THEN
                DO J=1,M
            ...
            END IF
      C
      C Go through the initial points, shifting every infeasible point i&
     &f
      C necessary so that its constraint violation is at least 0.2*RHOBE&
     &G.
      C
            DO NF=1,NPT
                FEAS=ONE
         ...
         80 J=J+1
           ...
           1 KBASE,NF,PQW,W)
                ...
                END IF
      C
      C Calculate the objective function at the current interpolation po&
     &int,
