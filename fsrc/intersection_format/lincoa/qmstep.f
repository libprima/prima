! This is the intersection-format version of qmstep.f.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
      C N, NPT, M, AMAT, B, XPT, XOPT, NACT, IACT, RESCON, QFAC, KOPT ar&
     &e the
     ...
     &moved.
      C DEL is the current restriction on the length of STEP, which is n&
     &ever
      C greater than the current trust region radius DELTA.
      C STEP will be set to the required step from XOPT to the new point&
     &.
      C GL must be set on entry to the gradient of LFUNC at XBASE, where&
     &LFUNC
      ...
      C RSTAT and W are used for working space. Their lengths must be at&
     &least
      C M and N, respectively. RSTAT(J) is set to -1.0, 0.0, or 1.0 if t&
     &he
      C J-th constraint is irrelevant, active, or both inactive and rele&
     &vant,
      C respectively.
      C IFEAS will be set to 0 or 1 if XOPT+STEP is infeasible or feasib&
     &le.
      C
      C STEP is chosen to provide a relatively large value of the modulu&
     &s of
      C LFUNC(XOPT+STEP), subject to ||STEP|| .LE. DEL. A projected STEP&
     &is
      C calculated too, within the trust region, that does not alter the
      C residuals of the active constraints. The projected step is prefe&
     &rred
      C if its value of | LFUNC(XOPT+STEP) | is at least one fifth of th&
     &e
      C original one, but the greatest violation of a linear constraint &
     &must
      C be at least 0.2*DEL, in order to keep the interpolation points a&
     &part.
      C The remedy when the maximum constraint violation is too small is&
     &to
      C restore the original step, which is perturbed if necessary so th&
     &at
      ...
      C
            ...
            TEST=0.2D0*DEL
      C
      C Replace GL by the gradient of LFUNC at the trust region centre, &
     &and
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
     &CCCCC
      C Zaikun 2019-08-29: With the original code, if either VLAG or VBI&
     &G is
      C NaN, KSAV will not get a value. This may cause Segmentation Faul&
     &t
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END DO
      C
      C Set STEP to the move that gives the greatest modulus calculated &
     &above.
      ...
      C
            ...
            END DO
      C
      C Set STEP to W if W gives a sufficiently large value of the modul&
     &us
      ...
      C
        ...
        170 J=J+1
            ...
            END IF
      C
      C Calculate the greatest constraint violation at XOPT+STEP with ST&
     &EP at
      C its original value. Modify STEP if this violation is unacceptabl&
     &e.
      C
        220 IFEAS=1
            BIGV=ZERO
