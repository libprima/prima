! This is the intersection-format version of update.f.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           ...
           2 VLAG(*),W(*)
      C
      C The arguments N, NPT, XPT, BMAT, ZMAT, IDZ, NDIM ,SP and STEP ar&
     &e
     ...
     &(.).
      C It is set on entry either to its final value or to 0. In the lat&
     &ter
      C case, the final value of KNEW is chosen to maximize the denomina&
     &tor
      C of the matrix updating formula times a weighting factor.
      C VLAG and W are used for working space, the first NPT+N elements &
     &of
      C both of these vectors being required.
      C
      C The arrays BMAT and ZMAT with IDZ are updated, the new matrices &
     &being
      C the ones that are suitable after the shift of the KNEW-th point &
     &to
      C the new position XPT(KOPT,.)+STEP(.). A return with KNEW set to &
     &zero
      ...
      C
            ...
            NPTM=NPT-N-1
      C
      C Calculate VLAG and BETA for the current choice of STEP. The firs&
     &t NPT
      C elements of VLAG are set to the values of the Lagrange functions&
     &at
      C XPT(KOPT,.)+STEP(.). The first NPT components of W_check are hel&
     &d
     ...
     &.
      ...
      C
      C If KNEW is zero initially, then pick the index of the interpolat&
     &ion
      ...
      C
            ...
            END IF
      C
      C Put the first NPT components of the KNEW-th column of the Z Z^T &
     &matrix
      ...
      C
      C Complete the updating of ZMAT when there is only one nonzero ele&
     &ment
      C in the KNEW-th row of the new matrix ZMAT. IFLAG is set to one w&
     &hen
     ...
     &))
            ...
            END IF
      C
      C Reduce IDZ when the diagonal part of the ZMAT times Diag(DZ) tim&
     &es
      C ZMAT^T factorization gains another positive element. Then exchan&
     &ge
