! This is the intersection-format version of trstep.f.
...
! on 03-Aug-2020.
     ...
     &CCCCC
      C Zaikun 19-03-2020: B is never used
      C SUBROUTINE TRSTEP (N,NPT,M,AMAT,B,XPT,HQ,PQ,NACT,IACT,RESCON,
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &CCCCC
      ...
      C B is never used
      C DIMENSION AMAT(N,*),B(*),XPT(NPT,*),HQ(*),PQ(*),IACT(*),
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &),
           2 D(*),DW(*),W(*)
      C
      C N, NPT, M, AMAT, B, XPT, HQ, PQ, NACT, IACT, RESCON, QFAC and RF&
     &AC
      C are the same as the terms with these names in LINCOB. If RESCON(&
     &J)
      C is negative, then |RESCON(J)| must be no less than the trust reg&
     &ion
      C radius, so that the J-th constraint can be ignored.
      C SNORM is set to the trust region radius DELTA initially. On the
      C return, however, it is the length of the calculated STEP, which &
     &is
      C set to zero if the constraints do not allow a long enough step.
      C STEP is the total calculated step so far from the trust region c&
     &entre,
      C its final value being given by the sequence of CG iterations, wh&
     &ich
      C terminate if the trust region boundary is reached.
      C G must be set on entry to the gradient of the quadratic model at&
     &the
      C trust region centre. It is used as working space, however, and i&
     &s
      C always the gradient of the model at the current STEP, except tha&
     &t
      C on return the value of G(1) is set to ONE instead of to ZERO if
      C and only if GETACT is called more than once.
      C RESNEW, RESACT, D, DW and W are used for working space. A negati&
     &ve
      ...
      C zero value of RESNEW(J) indicates that the J-th constraint is ac&
     &tive,
      C and otherwise RESNEW(J) is set to the greater of TINY and the ac&
     &tual
      C residual of the J-th constraint for the current STEP. RESACT hol&
     &ds
      C the residuals of the active constraints, which may be positive.
      C D is the search direction of each line search. DW is either anot&
     &her
      C search direction or the change in gradient along D. The length o&
     &f W
      ...
      C
            ...
            NCALL=0
      C
      C GETACT picks the active set for the current STEP. It also sets D&
     &W to
      C the vector closest to -G that is orthogonal to the normals of th&
     &e
      C active constraints. DW is scaled to have length 0.2*SNORM, as th&
     &en
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           1 RESACT,G,DW,W,W(N+1))
            ...
            END DO
      C
      C If the modulus of the residual of an active constraint is substa&
     &ntial,
      C then set D to the shortest move from STEP to the boundaries of t&
     &he
      ...
      C
            ...
            IF (RESMAX > 1.0D-4*SNORM) THEN
                ...
                END DO
      C
      C The vector D that has just been calculated is also the shortest &
     &move
      C from STEP+DW to the boundaries of the active constraints. Set GA&
     &MMA
      ...
      C
                ...
                END IF
      C
      C Reduce the steplength GAMMA if necessary so that the move along &
     &D
      ...
      C
        ...
        110 IF (GAMMA > ZERO) THEN
            ...
            END IF
      C
      C Set the next direction for seeking a reduction in the model func&
     &tion
      ...
      C
            ...
            ALPBD=ONE
      C
      C Set ALPHA to the steplength from STEP along D to the trust regio&
     &n
      ...
      C
            ...
            END DO
      C
      C Set DGD to the curvature of the model along D. Then reduce ALPHA&
     &if
      ...
      C
            ...
            END IF
      C
      C Make a further reduction in ALPHA if necessary to preserve feasi&
     &bility,
     ...
     &.
      ...
      C
      C Test for termination. Branch to label 40 if there is a new activ&
     &e
      ...
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
     &CCCCC
      C Zaikun 2019-08-29: the code can encounter infinite cycling due t&
     &o NaN
