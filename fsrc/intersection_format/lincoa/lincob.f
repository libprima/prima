! This is the intersection-format version of lincob.f.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           ...
           3 QFAC(N,*),RFAC(*),PQW(*),W(*)
      C
      C The arguments N, NPT, M, X, RHOBEG, RHOEND, IPRINT and MAXFUN ar&
     &e
      C identical to the corresponding arguments in SUBROUTINE LINCOA.
      C AMAT is a matrix whose columns are the constraint gradients, sca&
     &led
      C so that they have unit length.
      C B contains on entry the right hand sides of the constraints, sca&
     &led
      C as above, but later B is modified for variables relative to XBAS&
     &E.
     ...
     &ns.
      C XPT contains the interpolation point coordinates relative to XBA&
     &SE.
      C FVAL holds the values of F at the interpolation points.
      C XSAV holds the best feasible vector of variables so far, without&
     &any
      C shift of origin.
      C XOPT is set to XSAV-XBASE, which is the displacement from XBASE &
     &of
      C the feasible vector of variables that provides the least calcula&
     &ted
      C F so far, this vector being the current trust region centre.
      C GOPT holds the gradient of the quadratic model at XSAV = XBASE+X&
     &OPT.
     ...
     &the
      ...
      C where the elements of DZ are plus or minus one, as specified by &
     &IDZ.
      C NDIM is the first dimension of BMAT and has the value NPT+N.
      C STEP is employed for trial steps from XOPT. It is also used for &
     &working
      C space when XBASE is shifted and in PRELIM.
      C SP is reserved for the scalar products XOPT^T XPT(K,.), K=1,2,..&
     &.,NPT,
      C followed by STEP^T XPT(K,.), K=1,2,...,NPT.
      C XNEW is the displacement from XBASE of the vector of variables f&
     &or
      C the current calculation of F, except that SUBROUTINE TRSTEP uses&
     &it
      C for working space.
      C IACT is an integer array for the indices of the active constrain&
     &ts.
      C RESCON holds useful information about the constraint residuals. &
     &Every
      C nonnegative RESCON(J) is the residual of the J-th constraint at &
     &the
      C current trust region centre. Otherwise, if RESCON(J) is negative&
     &, the
      C J-th constraint holds as a strict inequality at the trust region
      C centre, its residual being at least |RESCON(J)|; further, the va&
     &lue
      C of |RESCON(J)| is at least the current trust region radius DELTA&
     &.
      C QFAC is the orthogonal part of the QR factorization of the matri&
     &x of
      C active constraint gradients, these gradients being ordered in
      C accordance with IACT. When NACT is less than N, columns are adde&
     &d
      C to QFAC to complete an N by N orthogonal matrix, which is import&
     &ant
      C for keeping calculated steps sufficiently close to the boundarie&
     &s
      C of the active constraints.
      C RFAC is the upper triangular part of this QR factorization, begi&
     &nning
      C with the first diagonal element, followed by the two elements in&
     &the
      C upper triangular part of the second column and so on.
      C PQW is used for working space, mainly for storing second derivat&
     &ive
      C coefficients of quadratic functions. Its length is NPT+N.
      C The array W is also used for working space. The required number &
     &of
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C Set the elements of XBASE, XPT, FVAL, XSAV, XOPT, GOPT, HQ, PQ, &
     &BMAT,
      C ZMAT and SP for the first iteration. An important feature is tha&
     &t,
      C if the interpolation point XPT(K,.) is not feasible, where K is &
     &any
      C integer from [1,NPT], then a change is made to XPT(K,.) if neces&
     &sary
      C so that the constraint violation is at least 0.2*RHOBEG. Also KO&
     &PT
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
         ...
         10 KNEW=0
            ...
            NVALB=0
      C
      C Shift XBASE if XOPT may be too far from XBASE. First make the ch&
     &anges
            ...
            IF (XOPTSQ >= 1.0D4*DELTA*DELTA) THEN
                ...
                DO K=1,NPT
                    ...
                    DO I=1,N
                        ...
                        IP=NPT+I
                        DO J=1,I
                            BMAT(IP,J)=BMAT(IP,J)+STEP(I)*W(J)+W(I)*STEP&
     &(J)
                        END DO
                    END DO
                END DO
      ...
      C
                ...
                END IF
      C
      C The following instructions complete the shift of XBASE, includin&
     &g the
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
      C In the case KNEW=0, generate the next trust region step by calli&
     &ng
     ...
     &havior
      C of TRSTEP or QMSTEP is not predictable, and Segmentation Fault o&
     &r
     ...
     &nded
      C behavior of the code, including uninitialized indices, which can&
     &lead
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
      C A trust region step is applied whenever its length, namely SNORM&
     &, is at
      C least HALF*DELTA. It is also applied if its length is at least 0&
     &.1999
      C times DELTA and if a line search of TRSTEP has caused a change t&
     &o the
      C active set. Otherwise there is a branch below to label 530 or 56&
     &0.
      C
                TEMP=HALF*DELTA
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C Alternatively, KNEW is positive. Then the model step is calculat&
     &ed
      C within a trust region of radius DEL, after setting the gradient &
     &at
      C XBASE and the second derivative parameters of the KNEW-th Lagran&
     &ge
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           1 QFAC,KOPT,KNEW,DEL,STEP,W,PQW,W(NP),W(NP+M),IFEAS)
            END IF
      C
      C Set VQUAD to the change to the quadratic model when the move STE&
     &P is
      C made from XOPT. If STEP is a trust region step, then VQUAD shoul&
     &d be
      C negative. If it is nonnegative due to rounding errors in this ca&
     &se,
     ...
     &CCCC
      C Zaikun 15-08-2019
      C Although very rarely, with the original code, an infinite loop c&
     &an occur
      ...
      C (i.e., DELTA is large and SNORM is not small, yet VQUAD >= 0 due&
     &to
      C rounding errors and XPT are not far from XOPT).
      C Then the program will goto 530 and then goto 20, where XBASE may&
     &be
      C shifted to the current best point, in the hope of reducing round&
     &ing
      C errors and 'improve' the model. Afterwards, another trust region&
     &step
      C is produced by the 'improved' model. Note that DELTA remains unc&
     &hanged
      C in this process. If the new trust region step turns out to satis&
     &fy
      C SNORM > 0.5*DELTA and VQUAD >= 0 again (i.e., the 'improved' mod&
     &el
      C still suffers from rounding errors), then the program will goto &
     &530
      C and then goto 20, where shifting will not happen because either &
     &XBASE
      C was already shifted to the current best point in last step, or X&
     &BASE
      C is close to the current best point. Consequently, the model will
      C remain unchanged, and produce the same trust region step again. &
     &This
      C leads to an infinite loop.
      C The infinite loop did happen when the MATLAB interface was appli&
     &ed to
      C min atan(x+100) s.t. x<=-99 (x0=-99, npt=3, rhobeg=1, rhoend=1e-&
     &6).
      C The problem does not exist in NEWUOA or BOBYQA, where the progra&
     &m will
      C exit immediately when VQUAD >= 0.
      C To prevent such a loop, here we use IMPRV to record whether the &
     &path
      C 530 --> 20 has already happened for last trust region step. IMPR&
     &V=1
      C implies that last trust region step satisfies VQUAD >= 0 and fol&
     &lowed
      C 530 --> 20. With IMPRV=1, if VQUAD is again nonnegative for the &
     &new trust
      C region step, we should not goto 530 but goto 560, where IMPRV wi&
     &ll be
      C set to 0 and DELTA will be reduced. Otherwise, an infinite loop &
     &would happen.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C Calculate the next value of the objective function. The differen&
     &ce
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            DIFF=F-FOPT-VQUAD
      C
      C If X is feasible, then set DFFALT to the difference between the &
     &new
      ...
      C
            ...
            END IF
      C
      C Update BMAT, ZMAT and IDZ, so that the KNEW-th interpolation poi&
     &nt
      C can be moved. If STEP is a trust region step, then KNEW is zero &
     &at
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
            ...
            END IF
      C
      C Update the second derivatives of the model by the symmetric Broy&
     &den
      C method, using PQW for the second derivative parameters of the ne&
     &w
      C KNEW-th Lagrange function. The contribution from the old paramet&
     &er
      C PQ(KNEW) is included in the second derivative matrix HQ. W is us&
     &ed
      ...
      C
            ...
            END IF
      C
      C Include the new interpolation point with the corresponding updat&
     &es of
      C SP. Also make the changes of the symmetric Broyden method to GOP&
     &T at
      ...
      C least calculated value so far with a feasible vector of variable&
     &s.
      C
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
            ...
            END IF
      C
      C Replace the current model by the least Frobenius norm interpolan&
     &t if
     ...
     &then
      C branch for another trust region calculation. Every iteration tha&
     &t
      C takes a model step is followed by an attempt to take a trust reg&
     &ion
      C step.
      C
            KNEW=0
     ...
     &ough
      ...
      C
        530 DISTSQ=DMAX1(DELTA*DELTA,4.0D0*RHO*RHO)
            ...
            END DO
      C
      C If KNEW is positive, then branch back for the next iteration, wh&
     &ich
      C will generate a "model step". Otherwise, if the current iteratio&
     &n
      C has reduced F, or if DELTA was above its lower bound when the la&
     &st
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        590 FORMAT (4X,'Least value of F =',1PD23.15,9X,
           1 'The corresponding X is:'/(2X,5D15.6))
            ...
            END IF
      C
      C Return from the calculation, after branching to label 220 for an&
     &other
