! This is the intersection-format version of cobylb.f.
...
! on 03-Aug-2020.
     ...
     &,SIM,
           1 SIMI,DATMAT,A,VSIG,VETA,SIGBAR,DX,W,IACT,F,INFO,FTARGET,RES&
     &MAX)
            IMPLICIT REAL(KIND(0.0D0)) (A-H,O-Z)
            IMPLICIT INTEGER (I-N)
            LOGICAL BETTER
            PARAMETER (NSMAX = 1000)
      C NSMAX is the maximal number of "dropped X" to save (see comments&
     &below
      C line number 480)
            PARAMETER (CTOL = EPSILON(1.0D0))
      C CTOL is the tolerance for consraint violation. A point X is cons&
     &idered
      C to be feasible if its constraint violation (RESMAX) is less than&
     &CTOL.
      C EPSILON(1.0D0) returns the machine epsilon corresponding to 1.0D&
     &0,
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &CCCCC
      C 1 A(N,*),VSIG(*),VETA(*),SIGBAR(*),DX(*),W(*),IACT(*)
           ...
           1 CONSAV(MPP),XSAV(N,NSMAX),DATSAV(MPP,NSMAX),XDROP(N),DATDRO&
     &P(MPP)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      C
      C Set the initial values of some parameters. The last column of SI&
     &M holds
      C the optimal vertex of the current simplex, and the preceding N c&
     &olumns
      C hold the displacements from the optimal vertex to the other vert&
     &ices.
      C Further, SIMI holds the inverse of the matrix that is contained &
     &in the
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C instructions are also used for calling CALCFC during the iterati&
     &ons of
      C the algorithm.
      C
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &CCCCC
      ...
      C CON, however, will be changed during the calculation (see the li&
     &nes
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &CCCCC
      ...
      C CSUM containts the sum of the absolute value of the constraints &
     &to
      C check whether it contains a NaN value.
            ...
            IF (CSUM /= CSUM) THEN
                ...
                GOTO 600
            END IF
      C If the objective function value or the constraints contain a NaN&
     &or an
      C infinite value, the algorithm stops.
            IF (F /= F .OR. F > ALMOST_INFINITY) THEN
                ...
                GOTO 600
            END IF
      C If the objective function achieves the target value at a feasibl&
     &e
      C point, then exit.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &.
      C This led to a bug, because F may not equal F(X) if the subroutin&
     &e
      C exits due to NFVALS .GE. MAXFUN (X is updated but F is not evalu&
     &ated
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
      C Set the recently calculated function values in a column of DATMA&
     &T. This
      C array has a column for each vertex of the current simplex, the e&
     &ntries of
      C each column being the values of the constraint functions (if any&
     &)
      C followed by the objective function and the greatest constraint v&
     &iolation
      C at the vertex.
      C
            DO K=1,MPP
      ...
      C
      C Exchange the new vertex of the initial simplex with the optimal &
     &vertex if
      C necessary. Then, if the initial simplex is not complete, pick it&
     &s next
      ...
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
     &CCCCC
      C When NFVALS<=N, it is necessary to update X(JDROP) because next &
     &X will
      C be calculated based on the current one (see the code below line &
     &number
      C 120). The purpose of this update is to make this X hold the vari&
     &able that
      C has the smallest function value up to now. The next X is defined&
     &as
      C a perturbation of this optimal X, which is reasonable.
      C However, this update leads to inconsistency between X and [F, RE&
     &SMAX],
      C meaning that X is not necessarily the X corresponding to [F, RES&
     &MAX].
      C This can cause COBYLA return inconsistent [X, F, RESMAX].
      C Fortunately, this is not a problem if NFVALS <= N. Because, if C&
     &OBYLA
      C returns with a NFVALS <= N, then X contains NaN or F = NaN or ne&
     &arly
      C Inf or the constraint contains NaN, all of which would lead to a&
     &n
      C immediate jump to line 600 without coming here. Therefore, if we
      C restrict the update to only the case with NFVALS <= N, ther will&
     &be no
      C inconsistency at return.
      C With the original code, if COBYLA returns with NFVALS = NP (this&
     &can
      C happen if the initial TR problem constantly produces too short s&
     &teps
      C so that RHO is reduced to RHOEND without producing any acceptabl&
     &e trial
      C step; see the code below line number 380), then, when the code a&
     &rrives
      C at line number 600, X and [F, RESMAX] may be inconsistent. Howev&
     &er,
      C recall that the inconsistency occurs only if SIM(:, NP) has a lo&
     &wer
      C function value than X (before updated). Therefore, as long as th&
     &e code
      C takes SIM(:, NP) instead of X, no harm would be done. It is the &
     &case
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END DO
      C
      C Switch the best vertex into pole position if it is not there alr&
     &eady,
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END IF
      C
      C Make an error return if SIGI is a poor approximation to the inve&
     &rse of
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END IF
      C
      C Calculate the coefficients of the linear approximations to the o&
     &bjective
      C and constraint functions, placing minus the objective function g&
     &radient
      C after the constraint gradients in the array A. The vector W is u&
     &sed for
      C working space.
      C
            DO K=1,MP
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END DO
      C
      C Calculate the values of sigma and eta, and set IFLAG=0 if the cu&
     &rrent
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END DO
      C
      C If a new vertex is needed to improve acceptability, then decide &
     &which
      ...
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
     &CCCCC
      C Zaikun 20190822: If VETA or VSIG become NaN due to rounding erro&
     &rs,
      C JDROP may end up being 0. If we continue, then a Segmentation Fa&
     &ult
     ...
     &CCCCC
      C Zaikun 20190820: See the comments below line number 480
            DO I = 1, N
                XDROP(I) = SIM(I, NP) + SIM(I, JDROP) ! JDROP<NP is guar&
     &anteed
            END DO
            DO K = 1, MPP
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            GOTO 40
      C
      C Calculate DX=x(*)-x(0). Branch if the length of DX is less than &
     &0.5*RHO.
      C
        370 IZ=1
            IZDOTA=IZ+N*N
      ...
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
     &CCCCC
      C Zaikun 2019-08-29: For ill-conditioned problems, NaN may occur i&
     &n the
      C models. In such a case, we terminate the code. Otherwise, the be&
     &havior
      C of TRSTLP is not predictable, and Segmentation Fault or infinite
      C cycling may happen. This is because any equality/inequality comp&
     &arison
      C involving NaN returns FALSE, which can lead to unintended behavi&
     &or of
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END IF
      C
      C Predict the change to F and the new maximum constraint violation&
     &if the
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            END DO
      C
      C Increase PARMU if necessary and branch back if this change alter&
     &s the
      C optimal vertex. Otherwise PREREM and PREREC will be set to the p&
     &redicted
      C reductions in the merit function and the maximum constraint viol&
     &ation
      C respectively.
      C
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      C
      C Calculate the constraint and objective functions at x(*). Then f&
     &ind the
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            END IF
      C
      C Begin the operations that decide whether x(*) should replace one&
     &of the
      C vertices of the current simplex, the change being mandatory if T&
     &RURED is
      C positive. Firstly, JDROP is set to the index of the vertex that &
     &is to be
      C replaced.
      C
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &CCCCC
      C Zaikun 20190820:
      C When JDROP=0, the algorithm decides not to include the trust-reg&
     &ion
      C trial point X into the simplex, because X is not good enough acc&
     &ording
      C to the merit function PHI = F + PARMU*RESMAX. In this case, X wi&
     &ll
      ...
      C SIM(:, NP), which is supposed to be the best point in the simple&
     &x.
      C For this reason, we save the to-be-discarded X in XSAV and compa&
     &re
      C them with SIM(:, NP) right before exiting. If a vector in XSAV t&
     &urns
      C out better than SIM(:, NP), we replace SIM(:, NP) by this vector
      C
      C When JDROP > 0, SIM(:, JDROP) will be removed from the simplex
      C according to PHI with the current PARMU. Similar to X, SIM(:, JD&
     &ROP)
      C may turn out better when PARMU is updated. Therefore, XSAV also &
     &takes
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            IF (JDROP == 0) GOTO 550
      C
      C Revise the simplex by updating the elements of SIM, SIMI and DAT&
     &MAT.
      C
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
     &CCCCC
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        550 IF (IFLAG == 0) THEN
            ...
            END IF
      C
      C Otherwise reduce RHO if it is not at its least value and reset P&
     &ARMU.
      C
            IF (RHO > RHOEND) THEN
      CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &CCCCC
      ...
      C in DATMAT(:, NFVALS-1). However, when the code arrives at line 6&
     &00,
     ...
     &)
      ...
      C NFVALS-2 instead of NFVALS-1.
        600 DO K = 1, M
            ...
            IF (NFVALS >= 2) THEN ! See the comments above for why NFVAL&
     &S>2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &CCCCC
      C Zaikun 20190820: See the comments below line number 480
           ...
           + NSMAX, CTOL)
      C This subroutine saves XDROP in XSAV and DATDROP in DATSAV, unles&
     &s
      C XDROP is dominated by a vector in XSAV(:, 1:NSAV). If XDROP domi&
     &nates
      C some vectors in XSAV(:, 1:NSAV), then these vectors will be remo&
     &ved.
      C If XDROP does not dominate any of XSAV(:, 1:NSAV) but NSAV=NSMAX&
     &,
      C then we remove XSAV(:,1), which is the oldest vector in XSAV(:, &
     &1:NSAV).
      C
      C When COBYLA calls this subroutine, XDROP is a vector to be "drop&
     &ped",
      C and DATDROP contains its function/constraint inforation (in part&
     &icular,
      C DATDROP(MP) = F(XDROP), and DATDROP(MPP) = RESMAX(X)). XSAV and &
     &DATSAV
      C save at most NSMAX vectors "dropped" by COBYLB and their functio&
     &n/constraint
      C information. Only XSAV(:, 1:NSAV) and DATSAV(:, 1:NSAV) contains&
     &such
      C vectors, while XSAV(:, NSAV+1:NSMAX) and DATSAV(:, NSAV+1:NSMAX)&
     &are
      C not initialized yet.
      C
      C Note: X dominates Y if and only if the function/constraint of X &
     &is
      C better than the function/constraint of Y accoring to the ISBETTE&
     &R
      C subroutine with PARMU = -1.0D0. Indeed, PARMU can be any negativ&
     &e
      ...
      C Fot this reason, it is sufficient to save all the "dropped" X th&
     &at are not
      ...
      C
            ...
            REAL(KIND(0.0D0)), INTENT(IN) :: XDROP(N), DATDROP(M+2), CTO&
     &L
      ...
      C NREMOVE: number of vectors to remove from XSAV
            DO J = 1, NSMAX
      C It is not enough to initialize IREMOVE(1:NSAV), because NSAV may&
     &be
      C incremented by 1 latter, and then IREMOVE(NSAV+1) will be access&
     &ed.
                IREMOVE(J) = -1
            END DO
            NREMOVE = 0
      ...
      C because XDROP should not be inluded into XSAV.
           ...
           1 DATSAV(MPP, I), PARMU, CTOL, BETTER)
                IF (BETTER) RETURN
      C If XDROP dominates XSAV(:, I), then increment NREMOVE by 1 and s&
     &ave
     ...
     &),
      ...
      C The code did not return and NREMOVE=0 (no vector to remove from &
     &XSAV).
      C If NSAV=NSMAX, then remove XSAV(:, 1); otherwise, increment NSAV&
     &by
      C 1 and then "remove" XSAV(:, NSAV) (though there is no vector sav&
     &ed there)
            IF (NREMOVE == 0) THEN
                IF (NSAV == NSMAX) THEN
                    IREMOVE(1) = 1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &CCCCC
      ...
      C This subroutine compares whether (F, R) is (strictly) better tha&
     &n
      C (F0, R0) in the sense of decreasing the merit function PHI = F +&
     &PARMU*R.
      C It takes care of the cases where some of these values are NaN or&
     &Inf.
      ...
      C 3. If A = NaN, then any comparison (except .NE.) with another nu&
     &mber B
      ...
      C
            ...
            LOGICAL :: F0INFNAN, FINFNAN, R0INFNAN, RINFNAN, FLE, FLT,RL&
     &E,RLT

            BETTER = .FALSE.

      C As values of F0, R0, F, and R, we regard Inf and NaN being equiv&
     &alent
      ...
      C When PARMU >= 0 and F + PARMU*R < F0 + PARMU*R0 and R < CTOL (fe&
     &asible),
      C then (F, R) is better than (F0, R0).
      C Note that we should not set BETTER=FALSE even if this inequlity &
     &does not
      ...
      C If R < CTOL and F is not Inf or NaN while (R0 < CTOL) is false (&
     &may
      C be because R0 is NaN), then (F, R) is better than (F0, R0). We p&
     &refer
      C feasible points (i.e., constraint violation is less than CTOL) t&
     &o
      C insfeasible ones.
      ...
      C If F0 or R0 is Inf/NaN while neither F nor R is Inf/NaN, then (F&
     &, R)
     ...
     &THEN
            ...
            FLT = (F0INFNAN .AND. (.NOT. FINFNAN)) .OR. (F < F0) ! F < F&
     &0?
            FLE = (F0INFNAN .AND. FINFNAN) .OR. (F <= F0) .OR. FLT! F <=&
     &F0?
            RLT = (R0INFNAN .AND. (.NOT. RINFNAN)) .OR. (R < R0) ! R < R&
     &0?
            RLE = (R0INFNAN .AND. RINFNAN) .OR. (R <= R0) .OR. RLT! R <=&
     &R0?

      C If (F < F0 and R <= R0) or (F <= F0 and R < R0) in the sense def&
     &ined
      C above, the (F, R) is better than (F0, R0).
            IF ((FLT .AND. RLE) .OR. (FLE .AND. RLT)) BETTER = .TRUE.

      C If one of F and R is -Inf and the other is not Inf/Nan, while ne&
     &ither
