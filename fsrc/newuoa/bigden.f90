...
nptm=npt-n-1
!
!     Store the first NPT elements of the KNEW-th column of H in WCHECK(N+1)
!     to WCHECK(N+NPT).
!
do k=1,npt
    wcheck(n+k)=zero
...
do j=1,nptm
    ...
    if (j < idz) temp=-temp
    do k=1,npt
        wcheck(n+k)=wcheck(n+k)+temp*zmat(k,j)
    end do
end do
alpha=wcheck(n+knew)
!
...
do k=1,npt
    sum=zero
    do j=1,n
        sum=sum+xpt(k,j)*wcheck(j)
    end do
    temp=(tau*wcheck(n+k)-alpha*vlag(k))*sum
    do i=1,n
