...
\begin{algorithm}[htbp!]
    ...
    \begin{algorithmic}[1]
        ...
        \State Define~$x_k = \argmin\{f(x) \mathrel{:} x\in \Int_k\}$, and calculate
            \begin{equation}
                \label{eq:}
        ...
        %$y_k^{+} \approx \argmin\{\kappa(\Int_k, y, y_k^{-}) \mathrel{:} \|y-x_k\|\le \Delta_k\}$
        \State
        If $\|x_k^+-x_k\| \ge \alpha \Delta_k$, then
        \begin{align}
            \label{eq:}
            x_k^{-} &\approx \argmin\{\kappa(\Int_k, x_k^+, x) \mathrel{:} x \in \Int_{k}\},\\
            \rho_k &= [f(x_k) - f(x_k^+)]/[Q_k(x_k) - Q_k(x_k^+)].
        \end{align}
        \State If~$\|x_k^+-x_k\|<\eta \Delta_k$ or $\rho_k \le 0$ and~$\kappa_k \ge \kappa_0$, then
        set~$\Int_{k+1} = \Int_k \cup\{x_k^+\}\setminus\{x_k^{-}\}$. Otherwise,
        set
        $y_k^{-} = \argmax\{\|y-x_k\| \mathrel{:} y \in \Int_k\}$,
        $y_k^{+} = \argmin\{\kappa(\Int_k, y, y_k^{-}) \mathrel{:} \|y-x_k\|\le \Delta_k\}$,
