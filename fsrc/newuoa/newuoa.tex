...
\begin{algorithm}[htbp!]
    ...
    \begin{algorithmic}[1]
        ...
        \If{$\|x_k^+-x_k\| \ge \eta \Delta_k$}
        \State $x_k^{-}= \argmin\{\kappa(\Int_k, x_k^+, x) \mathrel{:} x \in \Int_{k}\setminus\{x_k\}\}$
        \State $\kappa_k = \kappa(\Int_k,x_k^{+}, x_k^{-})$
        \State $\rho_k = [f(x_k) - f(x_k^+)]/[Q_k(x_k)- Q_k(x_k^+)]$
        \State Update~$\Delta_k$ to~$\Delta_{k+1}$ according~to~$\rho_k$
        \Else
        \State $\kappa_k = \infty$
        \State $\rho_k = -\infty$
        \State $\Delta_{k+1} = \theta\Delta_k$
        \EndIf
        \State If~$\rho_k > 0$ or~$\kappa_k < \kappa_0$, then
