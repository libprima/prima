...
\begin{algorithm}[htbp!]
    ...
    \begin{algorithmic}[1]
        ...
        \begin{equation}
            \label{eq:updateq1}
            {Q}^{\new}_k = \argmin\{\|Q-Q_k\|_\fro \mathrel{:} Q\in \Qua~\text{and}~Q(x) = f(x) \text{ for all } x \in
            \Int^{\new}_k\}.
        \end{equation}
        \State \textbf{Geometry improvement}.
        If~$\shortd = 1$ or~$r_k < 1/10$, then set
          ...
          \end{equation}
          If~$\|y_k^{\drop}-\xopt{\Int^{\new}_k}\| \ge 2\Delta_{k+1}$, then
          define~$\Delta^{\new}_k = \max\{\rho_k,\, \min\{\|y_k^{\drop} - \xopt{\Int^{\new}_k}\|/10,\, \Delta_{k+1}/2\}\}$,
          calculate
          \begin{equation}
              \label{eq:ygetn}
