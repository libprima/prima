    ...
    \begin{algorithmic}[1]
        ...
        \begin{equation}
         \label{eq:xgetn}
          x_k^\get \approx \argmin\{Q_k(x)\mathrel{:} \|x-x_k\|\le \Delta_k\}.
        \end{equation}
        Set~$\shortd_k = \ind(\|x_k^\get-x_k\| < \rho_k/2)$, $\redrho_k =  \ind(\shortd_k=1\text{
        and the errors in recent models are small})$.
        If~$\shortd_k=1$ and~$\redrho_k = 0$, then set~$\Delta_{k+1} = \max\{\Delta_k/10, \rho_k\}$.
        If~$\shortd_k = 0$, then evaluate $r_k = [f(x_k)-f(x_k^\get)]/[Q_k(x_k)-Q_k(x_k^\get)]$,
        ...
        \begin{equation}
            ...
            \Int_k\},
        \end{equation}
        and take~$\hat{\Int}_{k} = \Int_k\cup x_k^\get\setminus x_k^\drop$
        ...
        \end{equation}
        \State \textbf{Geometry improvement}.
        If~$\redrho_k = 0$ and either~$\shortd_k = 1$ or~$r_k < 1/10$, then set
          \begin{equation}
              \label{eq:ydropn}
