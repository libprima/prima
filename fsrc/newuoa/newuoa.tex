...
\begin{algorithm}[htbp!]
    \caption{label{alg:newuoa}}
        \INPUT: $x_0\in \RR^n$, $\Delta_0\in (0,+\infty)$, $m\in \{n+2, n+3, \dots, (n+1)(n+2)/2\}$.
    \begin{algorithmic}[1]
        \State \textbf{Initialization}.
        Choose~$\Int_0\subset \RR^n$ such that~$x_0\in \Int_0$ and~$|\Int_0|=m$. Set~$Q_{-1} = 0$
        and~$k=0$.
        \State \textbf{Model construction}.
        \begin{equation}
            %\label{eq:}
            \nonumber
        x_k^* = \argmin_{x\in\Int_k} f(x),
        \quad Q_k = \argmin_{Q\in\Qua}\{\|\nabla^2 Q - \nabla^2Q_{k-1}\|_\fro \mathrel{:} Q(x)
        = f(x)~\text{for}~x\in \Int_k\}.
        \end{equation}
    \end{algorithmic}
\end{algorithm}
