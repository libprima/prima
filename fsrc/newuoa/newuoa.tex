...
\begin{algorithm}[htbp!]
    ...
    \begin{algorithmic}[1]
        ...
        if $\rho_k>0$ or $\kappa(\Int_k\cup x_k^\get \setminus x_k^\drop) \le \kappa_0$.
        \State \textbf{Geometry improvement}.
        If~$\|x_k^\get-x_k\|< \alpha \Delta_k$, or~$\|x_k^\get-x_k\| > \alpha \Delta_k$
        but~$\rho_k\le 0$ and $\kappa(\Int_k \cup x_k^\get\setminus x_k^\drop) > \kappa_0$, then calculate
          \begin{align}
              \label{eq:ydrop}
    ...
    \begin{algorithmic}[1]
        ...
        \begin{equation}
         \label{eq:xgetn}
          x_k^\get \approx \argmin\{Q_k(x)\mathrel{:} \|x-x_k\|\le \Delta_k\}.
        \end{equation}
        Let~$\shortd_k = \ind(\|x_k^\get-x_k\| < \rho_k/2)$, then set~$\Delta_{k+1} = \theta \Delta_k$.
        Otherwise,
        update~$\Delta_k$ to~$\Delta_{k+1}$ according to~$\rho_k = [f(x_k) - f(x_k^\get)]/[Q_k(x_k)
        - Q_k(x_k^\get)]$.
        \State \textbf{Interpolation set update}.
