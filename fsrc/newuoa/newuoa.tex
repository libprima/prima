...
\begin{algorithm}[htbp!]
    ...
    Input $\Delta_0\in (0,+\infty)$, $\tau>0$, $m\in \{n+2, n+3, \dots, (n+1)(n+2)/2\}$,
    and~$\Int_0\subset \RR^n$ with~$|\Int_0|=m$ and~$\kappa(\Int_0) \le \kappa_0$.
    Define~$Q_0 = \argmin\{\|\nabla^2 Q\|_\fro \mathrel{:} Q\in \Qua~\text{and}~Q(x)
    = f(x)~\text{for}~x\in \Int_0\}$.
    Set~$k=0$.
    \begin{algorithmic}[1]
        \State \textbf{Trust-region step evaluation}.
        Set
        ...
        If~$\shortd = 0$, then evaluate $r_k = [f(\xopt{\Int_k})-f(x_k^\get)]/[Q_k(\xopt{\Int_k})-Q_k(x_k^\get)]$
        ...
        \begin{equation}
            ...
            \Int_k\},
        \end{equation}
        and let~$\Int^{\new}_{k} = \Int_k\cup x_k^\get\setminus x_k^\drop$
        if~$r_k>0$ or $\kappa(\Int_k\cup x_k^\get \setminus x_k^\drop) \le \kappa_0$. In
        any other case,~$\Int^{\new}_k=\Int_k$.~Set%
        \begin{equation}
            \label{eq:updateq1}
            {Q}^{\new}_k = \argmin\{\|\nabla^2Q-\nabla^2Q_k\|_\fro \mathrel{:} Q\in \Qua~\text{and}~Q(x)
        = f(x)~\text{for}~x \in \Int^{\new}_k\}.
        \end{equation}
        \State \textbf{Geometry improvement}.
        ...
        calculate
          ...
          \begin{align}
              ...
              {Q}_{k+1} & = \argmin\left\{\|\nabla^2Q-\nabla^2Q_k^\new\|_\fro \mathrel{:}
              Q=f~\text{on}~\Int_{k+1},\, Q\in \Qua\right\}.
          \end{align}
          Otherwise, $\Int_{k+1} = \Int_k^\new$ and~$Q_{k+1} = Q_k^{\new}$.
          If~$\max\{\xdist(\Int_k^{\new})/2, `\}\Delta_{k+1}$ and either~$\shortd=1$ or~$r_k\le 0$,
