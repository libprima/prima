...
\begin{algorithm}[htbp!]
    ...
    \begin{algorithmic}[1]
        ...
        \State If~\red{$\|\nabla Q_k(x_k)\| \le \eta \Delta_k$}, then set~$\kappa_k = \infty$,~$\rho_k = -1$,
        and~$\Delta_{k+1} = \Delta_k/2$. Otherwise, set
        $x_k^+ = \argmin\{Q_k(x)\mathrel{:} \|x-x_k\|\le \Delta_k\}$,
        $x_k^{-}= \argmin\{\kappa(\Int_k, x_k^+, x) \mathrel{:} x \in \Int_{k}\setminus\{x_k\}\}$,
        $\kappa_k = \kappa(\Int_k,x_k^{+}, x_k^{-})$, $\rho_k = [f(x_k) - f(x_k^+)]/[Q_k(x_k)
        - Q_k(x_k^+)]$, and~$\Delta_k$ according to~$\rho_k$.
