...
\begin{algorithm}[htbp!]
    \caption{\label{alg:iopt}OPTimization based on Interpolation Models (OPTIM)}
    Input $\Delta_0\in (0,+\infty)$, $\tau>0$, $m\in \{n+2, n+3, \dots, (n+1)(n+2)/2\}$,
    and~$\Int_0\subset \RR^n$ with~$|\Int_0|=m$ and~$\kappa(\Int_0) < \kappa_0$. Set~$Q_{-1} = 0$ and~$k=0$.
    \begin{algorithmic}[1]
        ...
        \end{equation}
        If~$\|x_k^\get-x_k\|\le \alpha\Delta_k$, then set~$\Delta_{k+1} = \theta \Delta_k$.
        Otherwise,
        update~$\Delta_k$ to~$\Delta_{k+1}$ according to~$\rho_k = [f(x_k) - f(x_k^\get)]/[Q_k(x_k)
        - Q_k(x_k^\get)]$.
        \State \textbf{Interpolation set update}.
        If~$\|x_k^\get-x_k\| \ge \alpha\Delta_k$, then calculate
        \begin{equation}
            \label{eq:xdrop} x_k^\drop \approx \argmin\{\kappa(\Int_k\cup x_k^\get\setminus x) \mathrel{:} x \in
            \Int_k\},
        \end{equation}
        and set~$\Int_{k+1} = \Int_k\cup x_k^\get\setminus x_k^\drop$
        if $\rho_k>0$ or $\kappa(\Int_k\cup x_k^\get \setminus x_k^\drop) < \kappa_0$.
        \State \textbf{Geometry improvement}.
        If~$\|x_k^\get-x_k\|\le \alpha \Delta_k$, or~$\|x_k^\get-x_k\| > \alpha \Delta_k$
        but~$\rho_k\le 0$ and $\kappa(\Int_k \cup x_k^\get\setminus x_k^\drop)\ge \kappa_0$, then calculate
          \begin{align}
              \label{eq:ydrop}
