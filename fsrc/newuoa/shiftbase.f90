subroutine shiftbase(n, npt, idz, xopt, pq, bmat, zmat, gq, hq, xpt, info)

    implicit none
    integer, parameter :: dp = kind(0.0d0)
    real(kind = dp), parameter :: almost_infinity = huge(0.0d0)/2.0d0, half = 0.5d0, zero = 0.0d0

    integer, intent(in) :: idz, n, npt
    integer, intent(out) :: info

    real(kind = dp), intent(in) :: xopt(n), pq(npt)
    real(kind = dp), intent(inout) :: bmat(npt + n, n), zmat(npt, npt - n - 1), gq(n), hq((n*(n + 1))/2), xpt(npt, n)

    integer :: i, ih, j, k
    real(kind = dp) :: summation, summationz, temp, tempq, vlag(n), w(2*npt), xoptsq
    ...
    do k = 1, npt
        ...
        do i = 1, n
            ...
            do j = 1, i
                bmat(npt + i, j) = bmat(npt + i, j) + vlag(i)*w(j) + w(i)*vlag(j)
            end do
        end do
    end do
    ...
    ! The following instructions complete the shift of XBASE, including the changes to the parameters of the quadratic model.
    ih = 0
    do j = 1, n
        w(j) = zero
        do k = 1, npt
