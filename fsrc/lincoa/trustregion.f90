...
!
! Last Modified: Tuesday, November 15, 2022 AM11:18:40
!--------------------------------------------------------------------------------------------------!
...
elseif (ratio <= eta2) then
    ...
    delta = min(max(gamma1 * delta_in, dnorm + dnorm), sqrt(2.0_RP) * delta_in)
    !delta = max(HALF * delta_in, gamma2 * dnorm)  ! Powell's NEWUOA/BOBYQA.
