...
C values. Exit when NCALL is large or NaN detected. 
      IF (NCALL .GT. MIN(10000, 100*(M+1)*N) .OR. ! Note: M can be 0.
     1    ALPHA .NE. ALPHA .OR. ALPHT .NE. ALPHT .OR. 
     1    ALPHM .NE. ALPHM .OR. DGD .NE. DGD .OR. DG .NE. DG .OR. 
     1    SS .NE. SS .OR. SNSQ .NE. SNSQ .OR. REDUCT .NE. REDUCT) THEN
          GOTO 320
      END IF
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
