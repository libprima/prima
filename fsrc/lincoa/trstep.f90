!*==trstep.f90  processed by SPAG 7.50RE at 00:12 on 26 May 2021
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
! Zaikun 19-03-2020: B is never used
!      SUBROUTINE TRSTEP (N,NPT,M,AMAT,B,XPT,HQ,PQ,NACT,IACT,RESCON,
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      SUBROUTINE TRSTEP(N,Npt,M,Amat,Xpt,Hq,Pq,Nact,Iact,Rescon,Qfac,   &
     &                  Rfac,Snorm,Step,G,Resnew,Resact,D,Dw,W)
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         CALL GETACT(N,M,Amat,Nact,Iact,Qfac,Rfac,Snorm,Resnew,Resact,G,&
     &               Dw,W,W(N+1))
         IF ( W(N+1)==zero ) EXIT
...
!
            ...
            IF ( M>0 ) THEN
               DO j = 1 , M
                  IF ( Resnew(j)>zero ) Resnew(j)                       &
     &                 = DMAX1(Resnew(j)-alpha*W(j),tiny)
               ENDDO
            ENDIF
...
! values. Exit when NCALL is large or NaN detected.
            IF ( ncall>MIN(10000,100*(M+1)*N) .OR. alpha/=alpha .OR.    &
     &           alpht/=alpht .OR. alphm/=alphm .OR. dgd/=dgd .OR.      &
     &           dg/=dg .OR. ss/=ss .OR. snsq/=snsq .OR.                &
     &           reduct/=reduct ) GOTO 100     ! Note: M can be 0.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
