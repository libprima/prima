...
if ~isempty(requirements.list)
    ...
    case {'newuoa', 'newuoan'}
        ...
        blacklist = [blacklist, {'HYDC20LS'}]; % Takes too long to solve
    case {'bobyqa', 'bobyqan'}
    ...
    case {'cobyla', 'cobylan'}
        blacklist = [blacklist, {'POLAK2'}]; % Cannot pass A = B^{-1}.
        ...
        blacklist = [blacklist, {'PRODPL0', 'DEGENLPB', 'ANTWERP', 'SIPOW3'}]; % Takes long to solve
...
if (ismember('newuoa', solvers) || ismember('uobyqa', solvers)) && fx1 == fx2 && norm(x1 - x2) > 0 ...
        && output1.funcCount == output2.funcCount ...
        && length(output1.fhist) == length(output2.fhist) && all(output1.fhist == output2.fhist)
    x1 = x2;
